...
\end{equation}
%
\en{which minimizes the Kullback-Leibler divergence between the true and the approximate distribution. This kind of method allow performing Bayesian inference in situations that otherwise would be impossible.}
...
%
\en{As we need to know how skills change over time, this technique allows to efficiently apply equation~\eqref{eq:posterior_win} iteratively over a sequence of observations.}
...
%
\en{The posterior at any given time is:}
...
% \end{figure}
%
\en{By applying the rules of probability over this dynamical model, the historical information naturally propagates throughout the system, solving the issues caused by the filtering approach.}
...
\end{lstlisting}
%
\en{TrueSkill Through Time not only returns correct estimates (the same for all players), they also have less uncertainty.}
...
\es{Ahora analizamos un ejemplo más complejo en el que un jugador nuevo se integra a comunidad grande de jugadores ya conocidos.}
%
...
%
\en{The community is generated by ensuring that each opponent has skill similar to the target of the target player, following its evolution.}
\es{La comunidad se genera asegurando que cada oponente tenga una habilidad similar a la del jugador objetivo, siguiendo su evolución.}
%
...
%
\begin{lstlisting}[backgroundcolor=\color{python}, label=lst:simulated_skill, caption={\en{Simulation of the target learning curve and the community of opponents}\es{Simulación de la curva de aprendizaje objetivo y la comunidad de oponentes}}, belowskip=0.1cm, aboveskip=0.1cm]
...
\end{lstlisting}
%
\en{Here we only include the \proglang{Python} version (\proglang{Julia} and \proglang{R} versions can be found in the appendix~\ref{AGREGA-CODE}).}
\es{Aquí incluimos s\'olo la versi\'on de \proglang{Python} (las versiones de \proglang{Julia} y \proglang{R} se encuentran en el anexo \ref{AGREGA-CODIGO}).}
%
...
%
\en{To generate the list \texttt{results} we sample the performance of the players from the synthetic skills: the player with the highest performance wins.}
\es{Para generar la lista \texttt{results} muestreamos el rendimiento de los jugadores a partir de las habilidades sintéticas: gana el jugador con mayor rendimiento.}
%
\en{The variable \texttt{time} is a list of integer values ranging from 0 to 999 representing the time for each game: the class \texttt{History} uses it to determine how much dynamic uncertainty ($\gamma^2$) must be applied between games as we move forward in time.}
\es{La variable \texttt{time} es una lista de valores enteros que van de 0 a 999, representando el tiempo de cada partida: la clase \texttt{Historia} la utiliza para determinar cuánta incertidumbre dinámica ($\gamma^2$) debe aplicarse entre juegos a medida que avanzamos en el tiempo.}
%
The variable \texttt{priors} is a dictionary that can be used to customize player characteristics.
\en{The variable \texttt{priors} is a dictionary that can be used to customize the features of the players.}
\es{La variable \texttt{priors} es un diccionario que se puede utilizar para personalizar las caracterísiticas de los jugadores.}
%
\en{In this example, we assign low uncertainty to the opponents' priors to simulate that we know their skill beforehand.}
\es{En este ejemplo, asignamos baja incertidumbre a los priors de los oponentes para simular que conocemos su habilidad de antemano.}

% Parrafo

\en{The class \texttt{History} receives those four parameters, and initializes the rest of the players (in this case only the target player) using the default values and a dynamic factor \texttt{gamma=0.015}.}
\es{La clase \texttt{History} recibe los 4 parámetros, e inicializa al resto de los jugadores (en este caso sólo el agente objetivo) con los valores por defecto y un factor dinámico \texttt{gamma=0.015}.}
%
...
%
\en{Figure~\ref{fig:logistic} shows the evoltion of the synthetic (solid line) and estimaed (dashed line) learning curve of the target player's.}
\es{La figura~\ref{fig:logistic} muestra la evolución de la curva de aprendizaje sintética (línea sólida) y estimada (línea punteada) del jugador objetivo.}
%
\begin{figure}[H]
    \centering
    ...
    \end{subfigure}
    \caption{
    \en{Synthetic and estimated learning curve of a new player that joining a large community of already known players.}
    ...
    %
    \en{The solid line represents the synthetic skill of the target player, while the dashed line shows the mean estimate.}
    \es{La línea continua representa la habilidad sintética del jugador objetivo, mientras que la línea discontinua muestra la estimación media.}
    %
    \en{The dark (light) gray area shows once (twice) the uncertainty of the estimate.}
    \es{La zona gris oscura (clara) iulstra una vez (dos veces) la incertidumbre de la estimación.}
    }
    \label{fig:logistic}
\end{figure}
%
\en{In the Figure~\ref{fig:logistic_mu} we see that all the means of the estimated learning curves always remain close to the actual skill during the whole evolution.}
...
%
\en{In Figure~\ref{fig:logistic_sigma} we see that the real learning curve is always contained in the uncertainty interval of any of the estimates.}
\es{En la figura~\ref{fig:logistic_sigma} vemos que la curva real esta siempre contenida en el itervalo de incertidumbre de cualquiera de las estimaciones.}
%
\en{With this example we can see that at all times TrueSkill Through Time correctly estimates the real skills of new players in communities of already known players.}
\es{Con este ejemplo podemos ver que en todo momento TrueSkill Through Time estima correctamente las habilidades reales de los jugadores nuevos que se incoporan a comunindades de jugadores ya conocidos.}
...
% 	\node[const, above=of nfr] (dfr) {\large $\mathbb{I}(d >0)$}; %
    ...
    %\node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     \caption{
     \en{Graphical way of representing the factorization of the basic causal model (equation~\eqref{eq:factorization}).}
     \es{Forma gráfica de representar la factorizaci\'on del modelo causal básico (ecuación~\eqref{eq:factorization}).}
     %
