...
%
\en{In our case, in which we need to know how skills change over time, this technique allow us to efficiently apply the equation~\ref{eq:posterior_win} iteratively over a sequence of observations.}
\es{En nuestro caso, en el que necesitamos conocer cómo cambian las habilidades a trav\'es del tiempo, está t\'ecnica nos permite aplicar eficientemente la ecuación~\ref{eq:posterior_win} iterativamente sobre una secuencia de observaciones.}
...
\es{Exluyendo el aspecto dinámico, $\gamma = 0$, el prior de un agente $i$ en el $t$-\'esimo evento es el producto del todas sus verosimilitudes, salvo la del $t$-\'esimo evento.}
%
...
\es{Esto produce una mutua dependencia entre verosimilitudes, obligándonos a implementar un algoritmo iterativo para resolver la inferencia (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
