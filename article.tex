...
%
\en{We will start first with the ``downstream'' messages, from the priors to the result, until compute the evidence.}
\es{Empezaremos primero con los mensajes ``descendentes'', desde los priors hasta el resultado, para calcular la evidencia.}
%
\en{Then we will continue with the ``upward'' messages to compute the posterior of an agent from the observed result and the prior of the rest of the agents.}
...
\end{equation}
%
\en{De la diferencia esperada $\delta$ remplazamos la estimación media $\mu_1$ por la hipótesis $s_1$.}
...
\end{lstlisting}  
%
\en{Assuming that the skill is known, its own uncertainty must be removed from the total uncertainty.}
\es{Al suponer conocida la habilidad, su propia incertidumbre debe ser removida de la incertidumbre total.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:theta, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
...
\end{paracol}
%
\en{This last message, $m_{f_{p_1} \rightarrow s_1}(s_1)$, is the exact likelihood and computes the ``probability of winning given the $s_1$ hypothesis''.}
\es{Este último mensaje, $m_{f_{p_1} \rightarrow s_1}(s_1)$, es el likelihood exacto y computa la ``probabilidad de ganar dada la hipótesis $s_1$''.}
%
\en{In other words, this is the a prior probability of a winning result if the player's true skill was $s_1$.}
\es{En otras palabras, esto es la probabilidad a priori de un resultado ganador si la verdadera habilidad del jugador fuera $s_1$.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:exact_lhood, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
  delta1 = delta - mu1 + s1 
...
\subsection{Approximate close posterior} \label{sec:approximate _posterior}

\en{En la sección~\ref{sec:exactSolution} hemos visto como encontrar la solución analítica de un posterior.}
%
