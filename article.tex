...
%
\en{Since the first generative models, proposed almost a century ago~\cite{glickman2013-zermelo1928}, it is assumed that the probability of observing a positive result, $r=1$, depends on the \emph{difference in strength} between the agent and their target, $d_{ab} = s_a - s_b$.}
\es{Ya los primeros modelos generativos, propuestos hace casi un siglo~\cite{glickman2013-zermelo1928}, suponen que la probabilidad de observar un resultado positivo, $r=1$, depende de la \emph{diferencia de escala} entre el agente y su objetivo, $d_{ab} = s_a - s_b$.}
...
%
\en{The performance that agents exhibit in each event is assumed to be random, centered around the real unknown skill $\N(s,\beta^2)$.}
...
%
\en{According to the model, the results are a direct consequence of the hidden returns.}
\es{Según el modelo, los resultados son consecuencias directa de los rendimientos ocultos.}
%
\en{Therefore, by observing a positive result we can infer, given the hypotheses, that the hidden performance of the agent in that event was indeed superior to that of his opponent.}
\es{Por eso, al observar un resultado positivo podemos inferir, dadas las hipótesis, que rendimiento oculto del agente en ese evento fue efectivamente superior al de su oponente.}
%

% The model assumes that, in each game, the players exhibit a performance, a hidden random variable normally distributed, $p_i\sim N(s_i , \beta^2)$, centered at their unknown true skill value with some constant noise.
% Under these assumptions, we can infer in each game who had the highest performance by observing the game outcome (win/lose).
% Then, the probability that player $i$ wins is $P(p_i>p_j \mid s_i, s_j) = P(p_i - p_j > 0 \mid s_i, s_j)$.
...
\en{Probability Theory provides us with a consistent mathematical framework  for quantifying the uncertainty of our beliefs, and to update them optimally given a model and the data~\cite{bishop2006-patternRecognitionMachineLearning}.}
...
\centering \small
    ...
    \node[const, left=of r, xshift=-1.25cm] (r_name) {\small Resultado observado}; 
    \node[const, right=of r] (dr) {\normalsize $ r_{ij} = (d_{ij}>0)$}; 

    \node[latent, above=of r, yshift=-0.45cm] (d) {$d_{ij}$} ; %
    \node[const, right=of d] (dd) {\normalsize $ d_{ij}=p_i-p_j$}; 
    \node[const, left=of d, xshift=-1.55cm] (d_name) {\small Diferencia oculta};
    ...
    \node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     \end{subfigure}
     \caption{
    \es{Modelo generativo de resultados observables ($r$) a partir de habilidades ocultas ($s$), a trav\'es de la diferencia ($d$) de rendimientos aleatorios ocultos ($p$).}
    \es{Los c\'irculos representan variables continuas, los cuadrados variables discretas.}
