...
\en{In this section we show how to use \proglang{Julia}, \proglang{Python} and \proglang{R} packages to solve a single event, a sequence consisting of three events, and the real-life history of professional ATP tennis players.}
...
%
\en{As we present three different versions of the tool using different programming languages, we select the following color convention to identify each one:}
...
\end{paracol}
%
where grey is used for all the common parts of the code (i.e. same syntax in the three languages), \proglang{Julia} code is colored with pink, \proglang{Python} with cyan, and \proglang{R} with light green.

\subsection{Single event} \label{sec:singleEvent}

\en{The class \texttt{Game} is used to model an event.}
...
%
\en{We assign the default values to the parameters with code~\ref{lst:parameters}.}
\es{Antes de empezar, ofrecemos algunos comentarios respecto de los valores de los parámetros.}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:parameters, caption={\en{Packages' default values}\es{Par\'ametros}},aboveskip=0.1cm]
mu = 0.0; sigma = 6.0; beta = 1.0; gamma = 0.03; p_draw = 0.0
\end{lstlisting}  
%
\en{The initial value of \texttt{mu} can be selected by the user as it does not model any feature by itself, the difference between \texttt{mu} of the different players is what really matters.}
...
\es{El desvío est\'andar del prior \texttt{sigma} debe ser suficientemente grande para no dejar por fuera posibles hip\'otesis de habilidad.}
%
\en{The parameter \texttt{beta} ($\beta$) works as the scale for the estimation.}
...
%
\en{A difference of one $\beta$ between two actual skills, $ s_i - s_j = \beta $, is equivalent to 76\% probability of winning independently of the chosen \texttt{beta}.}
...
%
\en{The default value for \texttt{beta} is \texttt{1.0} which is also used for this example.}
...
%
\en{The rule of thumb to initialize the draw probability (\texttt{p\_draw}) proposes to use the observed frequency of draws.}
...
\end{lstlisting}  
%
\en{In this example, we instantiate four players using the same initial parameters. For the first player (\texttt{p1}), we use a call to the constructor explicitly including the initial values while \texttt{p2}, \texttt{p3}, and \texttt{p4} are initialized using default values.}
...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={\en{Initialization of teams and a game}\es{Inicializaci\'on de los equipos y el juego}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
...
%
\en{Posteriors can be found by manually multiplying the likelihoods and priors, or we can call the method \texttt{posteriors()} of class \texttt{Game} to directly compute them.}
...
\end{lstlisting}
%
\en{The lists of likelihoods and posteriors are returned keeping the order in which players and teams were loaded during the initialization of the class \texttt{Game}. HABLAR DEL RESULTADO Y DAR INTUICION.}
...
\en{At this point, we analyze a more complex example in which the same four players participate in a single team game. 
The players are organized in three teams of different size (two teams with only one player and the other with two players).
The result is a single winning team and a tie between the other two losing teams.}
...
%
\en{Unlike the previous example, we use a draw probability different from the default.}
\es{A diferencia del ejemplo anterior, ahora necesitamos especificar el resultado y la probabilidad de empate del juego.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={\en{Game with different sized teams}\es{Inicializaci\'on de los equipos y el juego}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
ta = [p1]
...
\end{lstlisting}
%
\en{where \texttt{teams} includes all the participants in the game while \texttt{result} has the score obtained by each team following the same order used previously.
The team with the highest value is the winner and the teams with the same value are tied.
In this way you can specify any outcome of a team game including global ties.}
...
\subsection{Sequence of three events}
...
%
\en{In this example, we instantiate class \texttt{History} containing three players (\texttt{\small "a", "b", "c"}) and three games.}
...
%
\en{To access them we can use the method \texttt{learning\_curves()}, which returns a dictionary that is indexed by the names of the agents.}
