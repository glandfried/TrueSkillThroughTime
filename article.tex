...
\begin{equation}\label{eq:dynamic_factor}
 p(s_{i_t}) = \N(s_{i_t} | \mu_{i_{t-1}}, \sigma_{i_{t-1}}^2 + \gamma^2 )
 \end{equation}
...
\en{To solve this problems, we need a better representation of the temporal process: a single causal model that links all historical activities.}
\es{Para resolver estos problemas, necesitamos una mejor representaci\'on del proceso temporal: un único modelo causal que vincule todas las actividades hist\'oricas. }
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
  ...
  \caption{
  \en{Single causal model for the entire history of events.}
  \es{ modelo causal para toda la historia de eventos.}
  %
  \en{La aplicación de las reglas de la probabilidad sobre esta distribución de probabilidad conjunta de como resultado el approach known as \emph{smoothing} represents the entire history of activities through a single causal model.}
  \es{La aplicación de las reglas de la probabilidad sobre esta distribución de probabilidad conjunta de como resultado el approach known as \emph{smoothing}.}
  %
...
%
\en{The general solution can be found in section~\ref{sec:throguthTime}.}
\es{La soluci\'on general se encuentra en la secci\'on~\ref{sec:throguthTime}.}
%
\en{As an instance, we show the solution with $\gamma = 0$.}
\es{Para ejemplificar, mostramos la soluci\'on con $\gamma = 0$.}
%
\en{The prior of an event $t$ is the product of all, past, present and future likelihoods.}
\es{El prior en un evento $t$ es la multiplicaci\'on de todas las verosimilitudes pasadas y futuras.}
%
...
%  \text{Posterior}_{i,t} = \text{Prior}_{i,0} \underbrace{\prod_{k = 1}^{t-1} \text{\en{Likelihood}\es{Verosimilitud}}_{i,k}}_{\text{Past information}} \cdot \underbrace{  \text{\en{Likelihood}\es{Verosimilitud}}_{i,k}}_{\text{Future information}} \cdot \underbrace{\prod_{k = t+1}^{T} \text{\en{Likelihood}\es{Verosimilitud}}_{i,k}}_{\text{Future information}} 
% \end{equation}
Considerar todas las observaciones en un único modelo probabilísitico genera una mutua dependencia entre las verosimilitudes, lo que impone la necesidad de propoagar la información por todo el sistema.
%
...
\es{Esto se logra con un algoritmo iterativo eficiente, conocido como \emph{loopy belief propagation} (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
Al empezar, las verosimilitudes que aun no están definidas se inicializan con un valor neutro o simplemente se las excluyé.
%
Las verosimilitudes se reculalcular hasta alcanzar convergencia.
%
...
\begin{figure}[h!]
  ...
  \caption{
  Funcionamiento de TrueSkill Throguth Time en una historia que contiene tan solo dos jugadores, $a$ y $b$, y dos partidas.
  %
  La primera partida la gana el jugador $a$, $p_{a,1} > p_{b,1}$, y la segunda la gana el jugador $b$, $p_{a,2} < p_{b,2}$.
  %
  Notar que con tan solo dos partidas, al final de la convergencia el posterior se centra en 0 para ambos jugadores, indicando que tienen misma habilidad.
  %
  }
