...
\title{Harnessing the skill estimation using True Skill Through Time: the \proglang{Julia}, \proglang{Python} and \proglang{R} packages}
\Plaintitle{Harnessing the skill estimation using True Skill Through Time: the Julia, Python and R packages}
...
%
\en{Because new agents start with arbitrary estimates (the same initial value is used for all individuals), they tend to generate greater surprise values and abruptly modify estimates that had already converged.}
...
%
\en{The weak point is to not have into account the notion of uncertainty.}
\es{El punto d\'ebil se debe a la falta de la noción de incertidumbre.}
%
...
\en{An improved alternative consists in quantifying the uncertainty associated with each of the skill hypotheses instead of selecting a single value.}
...
%
\en{All inference, independently of its complexity, can be solved by the rules of probability: the~\ref{eq:sum_rule} and the~\ref{eq:product_rule}.}
...
%
\en{The \ref{eq:sum_rule} states that any marginal distribution can be obtained by integrating, or summing up, the joint probability distribution:}
\es{La \ref{eq:sum_rule} afirma que cualquier distribuci\'on marginal se puede obtener integrando o sumando la distribuci\'on de probabilidad conjunta.}
%
...
\todo[inline]{Arreglar la siguiente frase, incerteza sobre la estimaci\'on y sobre la performace}
\en{The difference of performances, in these cases, can also be expressed as a Gaussian distribution, centered on the difference of the prior estimate means ($\mu_i -\mu_j$), with a variance that incorporates the uncertainty of both estimates ($\sigma$) and both performances ($\beta$), $\N( d \, | \, \mu_i -\mu_j \, ,\ 2\beta^2 + \sigma_i^2 + \sigma_j^2 \,)$.}
\es{La diferencia de rendimientos, en estos casos, tambi\'en se puede expresar como una Gaussiana centrada en la diferencia de las medias de las estimaciones a priori ($\mu_i - \mu_j$), con una varianza que incorpora la incertidumbre de ambas estimaciones ($\sigma$) y de ambos rendimientos ($\beta$), $\N(\, d \, | \, \mu_i -\mu_j \, ,\ 2\beta^2 + \sigma_i^2 + \sigma_j^2 \,)$.}
%
...
\es{En nuestro caso, en el que necesitamos conocer cómo cambian las habilidades a trav\'es del tiempo, está t\'ecnica nos permite aplicar la ecuación~\ref{eq:posterior_win} iterativamente sobre una secuencia de observaciones.}
%
...
\es{Debido a que el enfoque de filtrado no se surge de ningún modelo probabil\'istico, sufre de un serie de problemas, todos relacionados con el hecho de que la información se propaga en una sola dirección a trav\'es del sistema.}
%
...
\es{Exluyendo el aspecto dinámico, $\gamma = 0$, el prior del agente $i$ en el $t$-\'esimo evento es el producto del todas sus verosimilitudes, salvo la del $t$-\'esimo evento.}
%
...
\es{Pero tambi\'en lo serán las verosimilitudes calculadas en la segunda pasada, debido a que su resutado dependerá parcialmente de las verosimilitudes calculadas en la pasada anterior.}
%
