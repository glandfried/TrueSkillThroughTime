...
%
\begin{equation}\label{eq:integral_con_dirac} \tag{\text{\en{Delta de Dirac function}\es{Función delta de dirac}}}
\begin{split}
...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:performance, caption={\en{Computing the priori performance}\es{Computando el desempeño a priori}}, belowskip=-1.0 \baselineskip, aboveskip=0.0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
p1 = performance(a1)
...
\end{paracol}
%
\en{The message sent by the team factors $f_{t_e}$ to the team variable $t_e$ is an integral over all the individual performance variables with their sum being equal to a constant $t_e$ (constraint imposed by the Dirac delta function).}
\es{El mensaje que envían los factores equipos $f_{t_e}$ a la variable equipo $t_e$ es una integral sobre todas las variables de rendimiento individuales siendo su suma igual a una constante $t_e$ (restricción impuesta por la función delta de Dirac).}
%
...
\end{equation}
%
\en{Thanks to the property of \ref{sum of normals}, the integral has an analytical solution being again a Gaussian distribution.}
\es{Gracias a la propiedad de \ref{suma de normales}, la integral tiene solución analítica siendo a su vez una distribución Gaussiana.}
%
