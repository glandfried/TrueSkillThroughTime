...
\en{Knowing how individual skills change over time and relating this changes with their possible causes is essential in sports and educational contexts.}
...
%
\en{However they are hidden variables, which means that the best we can do is estimating them based on their direct observable consequences: the outcome of problem solving and competitions.}
\es{Sin embargo ellas son variables ocultas, por lo que en la pr\'actica lo mejor que podemos hacer es estimarla a partir de sus consecuencias observables directas: el producto de resoluci\'on de problemas y competencias.}
%
\en{One naive solution is keeping track of positive results frequency, but this is not an informative measure because it depends also on the difficulty of the challenges.}
...
%
\en{For this reason, the estimation of skill is based on pairwise comparisons.}
\es{Por esta raz\'on, toda soluci\'on exitosa conocida se basa en comparaciones por pares.}
%
...
\es{Se diferencian s\'olo en la metodología usada para resolverlo.}
% 
\begin{figure}[h!]
\centering \small
    \tikz{         
...
\es{La soluci\'on metodol\'ogica de Elo es extremadamente simple y astuta: empezar con estimaciones arbitrarias, y luego de ver nuevos datos, actualizarlas en base a la sorpresa.}
%
...
\es{Cualquier par de estimaciones previas, $s_{i_\text{old}}$ y $s_{j_\text{old}}$, induce una predicci\'on a priori que, dependiendo del resultado, generan una determinada sorpresa.}
%
\begin{equation} \label{eq:elo_delta}
 \Delta_i = \underbrace{y_{ij}}_{\hfrac{\text{\en{Direction}\es{Direcci\'on}}}{\text{(Sign\es{o})}}}\underbrace{\left(1-P(\,r_{ij}\,|\,s_{i_\text{old}},s_{j_\text{old}}\,)\right)}_{\hfrac{\text{Magnitud}}{\text{(\en{Surprise}\es{Sorpresa})}}}
\end{equation}
%
...
\es{Pero es también una elecci\'on sustentada te\'oricamente.}
%
...
\es{A diferencia de la estadística clásica, que optimiza alguna funci\'on de costo ad-hoc para seleccionar una única hip\'otesis, la inferencia Bayesiana, aplicando estrictamente las reglas de la probabilidad, integra la distribuci\'on de probabilidad conjunta para cuantificar la incertidumbre asociada a cada hip\'otesis.}
...
\es{Para reducir la incertidumbre epistémica sobre la habilidad oculta, usando la informaci\'on que nos ofrece el resultado observado y el modelo causal descrito, necesitamos resolver la siguiente ecuaci\'on.}
%
...
\es{Esta clase de métodos, que minizan la divergencia Kullback-Leibler entre la distribuci\'on verdadera y aproximada, permiten realizar Inferencia Bayesiana en situaciones que de otra forma no sería posibles.}
%
...
\es{Impulsado por la creciento en la capacidad de c\'omputo ocurrida, en las últimas cinco décadas los investigadores han desarrollado miles de algoritmos exitosos de aprendizaje de automático, hechos a medida de conjuntos de problemas específicos.}
%
...
\es{Del mismo modo, los algoritmos empleadas varían de aplicaci\'on en aplicaci\'on, obligándonos a aprender una gran variedad de métodos.}
%
...
\es{En la secci\'on~\ref{sec:propiedades} enumeramos las propiedades que necesitaremos para derivar las distribuciones marginales de interés.}
%
...
% %
% 
% \en{Dado que la forma exacta se parece a una Gaussiana (figura~\ref{fig:posterior_win}), usar una Gaussiana para representar el posterior no parecería ser tan grave en términos de aproximaci\'on y en cambio nos permitiría realizar estimaciones sucesivas.}
% 
% % Parrafo
...
%\es{Los métodos para aproximar una distribuci\'on intratable mediante una distribuci\'on perteneciente a una familia conocida se conocen en inferencia bayesiana como ``variacionales''.}
%
...
\es{Y las verosimilituides se computan siguiendo la secci\'on~\ref{sec:iterative_posterior}, usando como prior toda la informaci\'on salvo la de la partida de interés.}
%
 \begin{equation}
