...
%
\begin{equation}\label{eq:integral_con_indicadora} \tag{\text{prop 4}}
\begin{split}
...
\begin{equation}\label{eq:simetria} \tag{\text{prop 5}}
 \N(x|\mu,\sigma^2) = \N(\mu|x,\sigma^2) = \N(-\mu|-x,\sigma^2) = \N(-x|-\mu,\sigma^2) 
\end{equation}
%
\en{The Gaussian standardization,}
\es{La estandizarización de la Gaussiana,}
\begin{equation}\label{eq:estandarizar} \tag{\text{prop 6}}
...
\begin{equation}\label{eq:phi_norm} \tag{\text{prop 7}}
...
\begin{equation}\label{eq:phi_simetria} \tag{\text{prop 8}}
...
% %
% \begin{lstlisting}[backgroundcolor=\color{all},label=lst:N1_N2, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
% \end{lstlisting}
...
\begin{split}
\iint \delta(t=x + y) \N(x|\mu_1, \sigma_1^2)\N(y|\mu_2, \sigma_2^2) dxdy = \N(t|\mu_1+\mu_2,\sigma_1^2 + \sigma_2^2)
\end{split}
\end{equation}
%
\vspace{-0.5cm}
...
\end{equation}
%
\en{We have access to this message by calling the \texttt{prior} attribute of the initialized \texttt{p} players, \texttt{p.prior}.}
\es{Tenemos acceso a este mensaje llamando al atributo \texttt{prior} de los jugadores \texttt{p} que se tengan inicializados, \texttt{p.prior}.}
%
...
\en{We know that}\es{Sabemos que}
\begin{equation}
t_n = \sum_{i=1}^n x_i \sim \int \dots \int \delta(t_n= \sum_{i=1}^n x_i ) \left( \prod_{i=1}^n \N(x_i|\mu_i,\sigma_i^2) \right) dx_1 \dots dx_n = \N(t|\sum_{i=1}^n \mu_i,\sum_{i=1}^n \sigma_i^2 )
\end{equation}
%
\en{We want to demonstrate by induction.}\es{Queremos demostrar por inducci\'on.}
\begin{equation}
 P(n):= \int \dots \int \delta(t_n= \sum_{i=1}^n x_i ) \left( \prod_{i=1}^n \N(x_i|\mu_i,\sigma_i^2) \right) dx_1 \dots dx_n \overset{?}{=} \N(t|\sum_{i=1}^n \mu_i,\sum_{i=1}^n \sigma_i^2 )
\end{equation}
...
\end{equation}
%
\en{Therefore $P(1)$ is true.}
\es{Luego $P(1)$ es verdadera.}
\begin{equation}
 \begin{split}
P(2) & := \iint \delta(t_2 = x_1 + x_2) \N(x_1|\mu_1, \sigma_1^2)\N(x_2|\mu_2, \sigma_2^2) dx_1dx_2 \\
 ...
 \end{equation}
%
 \en{Where $\overset{*}{=}$ is valid by de proof by the section~\ref{multiplicacion_normales}.}
 \es{Donde $\overset{*}{=}$ vale por la demostraci\'on de miltiplicaci\'on de normales en la secci\'on~\ref{multiplicacion_normales}.}
 %
