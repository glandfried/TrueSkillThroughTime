...
\es{El parámetro $\beta^2$, al ser el mismo para todos los agentes, actúa como la escala de las estimaciones: habilidades separada por un $\beta$ mantienen siempre la misma probabilidad de ganar.}
...
\en{In this section we show how to use \proglang{Julia}, \proglang{Python} and \proglang{R} packages to solve a single event, a short history of 3 events and the real history of professional ATP tennis players.}
\es{En esta sección mostramos cómo usar los usando los paquetes de\proglang{Julia}, \proglang{Python} y \proglang{R} para resolver un evento, una pequeña historia de 3 eventos y finalmente la historia real de jugadores profesional de tenis ATP.}
%
\en{We will show both TrueSkill and TrueSkill Through Time solutions, the method to obtain individual learning curves, and the a priori prediction of the observed data (i.e. evidence) that serve as a measure to compare models.}
\es{Mostraremos la soluciones TrueSkill y TrueSkill Through Time, el método para obtener curvas de aprendizaje individuales, y la predicción a priori (i.e. evidencia) del dato observado que sirven como medida para comparar modelos.}
%
\en{Finally we will make a time complexity comparison between the three packages presented in this paper and the \texttt{trueskill} package of presented in this paper \proglang{Python}.}
\es{Por último haremos un comparación de complejidad temporal entre los tres paquetes que presentamos en este trabajo y el paquete \texttt{trueskill} de \proglang{Python}.}
...
%
\en{In the presence of teams, the observed result is modeled as a direct consequence of the difference of the team performance, $d = \sum_{i \in A} p_i - \sum_{i \in B} p_i$ (see details at section \ref{sec:2vs2}).}
\es{En presencia de equipos, el resultado observado se modela como consecuencias directa de la diferencia de los rendimientos de cada equipo, $d = \sum_{i \in a} p_i) - \sum_{i \in b} p_i)$ (detalles en la sección \ref{sec:2vs2}).}
%
...
\end{lstlisting}
%
\en{where \texttt{result} indicates the observed order (the teams with the lowest number are the winners), and \texttt{praw = 0.0} indicates that the probability of a draw is zero for that type of game.}
\es{donde \texttt{result} indica el orden observado (los equipos con menor número son los ganadores), y \texttt{p\_draw = 0.0} indica que la probabilidad de empate nula para ese tipo de juegos.}
%
\en{The attributes of the \texttt{Game()} class are the \texttt{teams}, the \texttt{result}, the approximate \texttt{likelihoods} (one for each agent) and the \texttt{evidence}.}
...
\end{paracol}
%
\en{The evidence is $0.5$, indicating that the prior probability of the observed result was the same for both teams.}
...
\es{Algunos comentarios respecto a la elección de los parámetros.}
%
...
\es{El valor que se elija como media del prior \texttt{mu} no afecta las estimaciones porque lo único que importa es la diferencia de habilidad entre agentes.}
%
\en{The standard deviation of the prior \texttt{sigma} must be large enough not to leave out any possible hypothesis.}
\es{El desvío estandar del prior \texttt{sigma} debe ser suficientemente grande de modo de no dejar por fuera ninguna posible hipótesis.}
%
...
\es{El factor dinámico \texttt{gamma} es en general una pequeña fracción de \texttt{beta}.}
%
\en{Its optimal value can be obtained by maximizing the a priori prediction (i.e. evidence).}
\es{Su valor óptimo se puede obtener maximizando la predicción a priori (i.e. evidencia).}
%
\en{The rule of thumb proposes to initialize \texttt{p\_draw} with the empirical proportion of draws.}
\es{La regla general propone inicializar \texttt{p\_draw} con la proporción empírica de empates.}
%
...
\es{El parámetro \texttt{epsilon} indica la condición de convergencia, el cambio máximo en todos los posteriors antes y después de una iteración.}
%
\en{Finally, the number of iterations \texttt{iter} functions as a cut-off condition in case convergence has not been reached.}
\es{Por último, la cantidad de iteraciones \texttt{iter} funciona como condición de corte en caso de no haber alcanzado la convergencia.}
...
\es{Donde la variable \texttt{e1} indica la composición de la primer partida usando los nombres de los agentes; la variable \texttt{events} contiene la composición de las tres partidas; y la variable \texttt{res} contiene los resultados de las tres partidas (el menor valor representa el ganador).}
%
...
\es{Inmediatamente después de que se inicializa la historia, se imprime un resumen de los datos.}
%
...
\es{La inicialización de la historia también activa el computo de las estimaciones TrueSkill.}
%
...
\es{El primer elemento indica el tiempo, y el segundo indica la estimación correpondiente a ese tiempo.}
%
...
\es{A pesar de que lo razonable sería tener la misma estimación para todos los agentes (poque se ganan mutuamente), se puede ver que los posteriors reportados por TrueSkill difieren mucho entre jugadores y varían mucho para uno mismo jugador.}
%
...
\es{Impulsado por la crecimiento en la capacidad de c\'omputo ocurrida, en las últimas cinco d\'ecadas los investigadores han desarrollado miles de algoritmos exitosos de aprendizaje de autom\'atico, hechos a medida de conjuntos de problemas espec\'ificos.}
%
