...
\en{The analysis is performed using a low-end workstation with \SI{3.33}{\giga\byte} of RAM and an AMD processor (A4-6210, \SI{1.00}{\giga\hertz}, cache \SI{2048}{\kilo\byte}).}
...
\begin{table}[H] \centering
    \begin{tabular}{ccc} 
        \en{Language}\es{Lenguaje} & \en{Seconds}\es{Segundos} & \en{Improvement}\es{Mejora} \\ \hline
        Julia.1.5.0 &  $0.000064$ & $\times22.7$\\ \hline
        Python.3.6.9 & $0.00014$ & $\times10.4$ \\ \hline
        R.3.4.4   & $0.0371$ & $\times0.04$ \\ \hline
    \end{tabular}
    \caption{
...
\en{In this case, we perform the analysis using an additional workstation to show the impact of different hardware in the execution time while treating a realistic case: Workstation \textbf{A} has \SI{15.6}{\giga\byte} of RAM and an Intel processor (i5-3330, \SI{3.00}{\giga\hertz}, total cache \SI{6144}{\kilo\byte}), and workstation \textbf{B} (same as before) has \SI{3.33}{\giga\byte} of RAM and an AMD processor (A4-6210, \SI{1.00}{\giga\hertz}, cache \SI{2048}{\kilo\byte}), which represents a low-end computer with few resources.}
\es{El análisis fue realizado en dos computadoras representativas de uso domiciliario.}


\en{The total runtime for initialization of the \texttt{History} class and the \texttt{convergence()} method are,}
