...
%
\begin{lstlisting}
N1, N2 = Gaussian(mu=1, sigma=1), Gaussian(1, 2)  
\end{lstlisting}
%
...
\subsection{Analytical likelihood} \label{sec:analyticalSolution}

Las partidas con 2 equipos tienen solución analítica.
Podemos calcular el posterior exacto.
\todo[inline]{Crear la función, posterior exacto, que devuelva una función parametrizada.}

El posterior exacto sin embargo no pertenece a una distrbución conocida, por lo que se la aproxima mediante la Gaussiana que más se le parece.
...
team_b = [ r3, r4 ]
observed_result = [1,0]
g = Game(teams = [team_a, team_b], result = observed_result)
\end{lstlisting}
%
\begin{lstlisting}
[ s1 , s2 ], [ s3 , s4 ] = g.exact_posterior()
hipotesis = ...
plt.plot(s1(hipotesis))
\end{lstlisting}
%
\begin{figure}[H]
  \centering
...
\end{figure}
%
%\paragraph{Inferencia} Mensajes descendentes
%
...
vartheta = differencia.sigma
evidencia = cdf(delta/vartheta) 
\end{lstlisting}
%
