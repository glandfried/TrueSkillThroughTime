...
\es{El uso de la teoría de la probabilidad para manejar la incertidumbre, conocida como inferencia bayesiana, está hoy adopta de manera generalizada en el ámbito de la inteligencia artificial y el aprendizaje automático fundamentalmente porque ha mostrado ser un instrumento sumamente útil en la práctica.}
%
\en{It is not however an ad-hoc choice.}
\es{No es sin embargo una elección ad-hoc.}
%
...
\es{Cualquier sistema que represente grados de creencia mediante un valor numérico, consistente con la lógica deductiva clásica y un conjunto de axiomas que codifican el razonamiento de sentido común,  será necesariamente isomórfico a la teoría de la probabilidad~\citep{cox1946-reasonableExpectation,vanHorn2003-guideCoxTheorem}.}
%
...
\es{Y toda inferencia probabilística, no importa cuan compleja ella sea, puede ser resuelta a través de dos simples ecuaciones: la~\ref{eq:sum_rule} y la~\ref{eq:product_rule}.}
%
...
\es{Esto significa que cualquier distribución marginal $p(x)$ puede ser obtenida integrando la distribución conjunta $p(x,y)$, en este caso sumando todos los valores de $Y$.}
%
...
\es{Además, cualquier distribución conjunta puede ser expresada como el producto de distribuciones condicionales uni-dimensionles.}
%
\en{This rule, together with the symmetry property $p(x,y) = p(y,x)$, we can immediately obtain the~\ref{eq:bayes_theorem},}
\es{Esta regla, junto con la propiedad de simetría $p(x,y) = p(y,x)$, podemos obtenemos inmediatamente el~\ref{eq:bayes_theorem},}
%
...
\es{El uso inferencial del teorema de bayes juega un rol central en las técnicas modernas de aprendizaje estadísitico.}
%
\en{This formula allows us to optimally update our beliefs about a set of hypotheses, given a model and the data.}
\es{Esta fórmula nos permite actualizar de forma óptima nuestras creencia sobre ciertas hipótesis, dado un modelo y los datos.}
%
\begin{equation}\label{eq:inference}
 \underbrace{p(\text{\en{Hypotesis}\es{Hipótesis}}|\text{\en{Data}\es{Datos}})}_{\text{Posterior\es{i}}} = \frac{\overbrace{p(\text{\en{Data}\es{Datos}}|\text{\en{Hypotesis}\es{Hipótesis}})}^{\text{\en{Likelihood}\es{Verosimilitud}}}\overbrace{p(\text{\en{Hypotesis}\es{Hipótesis}})}^{\text{Prior\es{i}}}}{\underbrace{p(\text{\en{Data}\es{Datos}})}_{\text{Evidenc\en{e}\es{ia}}}}
\end{equation}
%
\en{Where the role played by the model is left implicit.}
\es{Donde el lugar que ocupa el modelo queda implícito.}
%

% Nuevo Párrafo


\en{In our case, the hypotheses are the ability of the people, the data are the results, and the model is defined by the figure~\ref{fig:generative_model}.}
\es{En nuestro caso, las hipótesis son las habilidades de las personas, los datos son los resultados y el modelo está definido por la figure~\ref{fig:generative_model}.}
%
\en{By defining a probability distribution that represents our a priori beliefs about people's abilities, we can obtain the exact posterioror belief distribution using only the sum and product rules (detalles en~\ref{}).}
\es{Definiendo una distribución de probabilidad que represenste nuestraa creenccias a priori sobre las habilidad de las personas, podemos obtener la distribución de creencias a posterori exacta usando tan solo las reglas de la suma y el producto (detalles en~\ref{}).}
%
\en{}
...
\en{Consider a game in which $N$ players $\{1, \dots, N\}$ are compete over a period of $T$ time steps or temporal batch (e.g. day, week, month, year).}
\es{Consideremos un juego que invulucra $N$ jugadores $\{1, \dots, N\}$, es completado en pasos o baches temporales $T$ (e.g. dia, semana, mes, años, etc).}
\en{Denote the series of games outcomes between two players $i$ and $j$ within a temporal batch $t$ by $\vm{y}^t_{ij}(k)$, where $k \in \{1,\dots,K^t_{ij}\}$, with $K^t_{ij}$ the number of games outcomes available for that pair of players in that temporal batch.}
\es{Denotamos al resultado de un juego entre dos jugadores $i$ y $j$ dentro de un bache temporal $t$ como $\vm{y}^t_{ij}(k)$, donde $k \in \{1,\dots,K^t_{ij}\}$, con $K^t_{ij}$ el numero de resultados de juegos disponibles para ese par de jugadores dentro de ese bache tempraol.
}
...
\es{\begin{itemize}
		\item Inferencia entre un bache temporal (día, semana, mes) depende del orden aleatorio elegido para la actualización. De todas formas el resultado deberia ser independiente del orden de los juegos dentro de un bache temporal.
		\item La información a través de un bache es solo propagado de manera temporal hacia adelante, no pudiendo propagarla en sentido contrario.
\end{itemize}}

\en{The neighbor factor graph of a temporal skill variable $s_i^t$, is}
\es{El grafico de factores de una variable de habilidad temporal $s_i^t$, es}
\begin{center}
\tikz{ %
...
\es{Los nombres fueron elegidos solamente para simplificar la notación.}
...
\es{Mediante el algoritmo de sum-product, sabemos que la distribución marginal de cualquier variable, es el producto de los mensajes, que esta variable recibe de sus vecinos,}
\begin{equation}
...
\es{Remplazando los mensajes por los nombres seleccionados, esta distribución se puede expresar como,}
\begin{equation}
...
\es{Existe una dependencia muta entre los mensajes hacia adelante y hacia atras, haciendo imposible una inferencia de única iteración.}
...
\es{La idea básica es la de actualizar repetidas veces los mensajes hacia adelante y hacia atras hasta su convergencia, asegurándonos que el efecto de la actualización previa sea removida antes de que el nuevo efecto se agregue.}
...
\es{Los mensajes que no están todavía definidos, por ejemplo el prior inverso en la primer pasada hacia adelante, es remplazado por una forma neutral como un escalar unidad o una distribución gausiana con infinita varianza.}
\vspace{0.3cm}
...
\es{La actualización para un resultado de un juego $y^t_i(k)$, se realiza de la misma manera que TrueSkill original, usando como priori el \emph{withinPrior}$_i(t,k)$ y guardando el \emph{likelihood}$_i(t,k)$.}
...
\es{En cada pasada hacia atrás computamos el mensaje hacia atrás, i.e. \emph{inversePrior}$_i(t-1)$.}
...
The first examples is the most basic one we figure out. Its one of the first examples in the official python package of Trueskill. 
...
One of the advantage of \textbf{TTT}, its the way that the algorithm pass information to the past to the future and vice versa, getting a more accurate estimation. 
In addition, it can define a temporal batch were its suppose that in there the order of the games it doesn't matter. 
