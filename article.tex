...
%
\en{Any point estimate, $s_i$ and $s_j$, determines a prediction probability for each possible outcome.}
\es{Cualquier estimación puntual, $s_i$ y $s_j$, determina una probabilidad de predicción de cada posible resultado.}
%
...
\end{equation}
%
\en{where $y_{ij}$ is $1$ for winners and $-1$ for losers, and the probability of the observed result given the old estimates can be derived from Fi.~\ref{fig:generative_model} (more details in section~\ref{sec:exactSolution}).}
...
%
\todo[inline]{Esteban arreglala un poquito para que sea m\'as simple}
...
\begin{equation}\label{eq:filter} %\tag{\text{filtering}}
 \text{Posterior}_t \propto \text{Likelihood}_t  \overbrace{\text{Likelihood}_{t-1} \dots \text{Likelihood}_{2} \underbrace{\text{Likelihood}_{1} \text{Prior}_1}_{\widehat{\text{Posterior}}_{1} \text{ \en{as}\es{como} } \text{Prior}_{2}} }^{\widehat{\text{Posterior}}_{t-1} \text{ \en{as}\es{como} } \text{Prior}_{t}} %= \text{Prior}_1 \prod_{i=1}^t \text{Likelihood}_i 
\end{equation}

\en{Where {\footnotesize $\widehat{\text{Posterior}}_i$} represents the approximation made with the equation~\ref{eq:approx} at the $i$-est event.}
\es{Donde {\footnotesize $\widehat{\text{Posterior}}_i$} representa la aproximación hecha con la ecuaci\'on~\ref{eq:approx} en el $i$-ésimo evento.}
%
...
\begin{equation}\label{eq:dynamic_factor}
 p(s_{i_t}) = \N(s_{i_t} | \widehat{\mu}_{i_{t-1}}, \widehat{\sigma}_{i_{t-1}}^2 + \gamma^2 )
 \end{equation}
