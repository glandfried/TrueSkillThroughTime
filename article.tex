...
%
\en{Since it is a complex variable, it is generally impossible to measure it directly (aka hidden variable).}
\es{Al ser una variable compleja, generalmente es imposible medirla directamente (aka variable oculta).}
%
\en{In practice, the best we can do is estimate it from its most direct observable consequences.}
\es{En la práctica, lo mejor que podemos hacer es estimarla a partir de sus consecuencias observables m\'as directas.}
%
...
\es{Las fuentes de información más utilizadas en educación y deportes provienen del producto de resolución de problemas y de competencia entre oponentes, respectivamente.}
%
...
\es{La solución ingenua, frecuencia de resultados positivos, no es un buen indicador fundamentalmente porque depende también de la dificultad de los desafíos.}
%
\en{For that reason, all conventional models are underpinned by pairwise comparisons.}
\es{Por esa razón, todos los modelos convencionales se basan en \emph{pairwise comparisons}.}
%
\en{Since the first generative models, proposed almost a century ago~\cite{glickman2013-zermelo1928}, it is assumed that the probability of observing a positive result, $r=1$, depends on the \emph{difference in strength} between the agent and their target, $d_{ij} = s_i - s_j$.}
\es{Ya los primeros modelos generativos, propuestos hace casi un siglo~\cite{glickman2013-zermelo1928}, suponen que la probabilidad de observar un resultado positivo, $r=1$, depende de la \emph{diferencia de escala} entre el agente y su objetivo, $d_{ij} = s_i - s_j$.}
% NO TOCAR: no debe haber espacio entre ecuacion y texto
\begin{equation}\label{eq:pairwise-comparision}
p(\,r=1 \,|\, d_{ij}\,)
\end{equation}
% NO TOCAR: no debe haber espacio entre ecuacion y texto
\en{After the seminal works of~\cite{thurstone1927-comparativeJugement} and~\cite{zermelo1929-translation}, the field was revitalized 25 years later by the publications of~\cite{bradley1952-pairedComparisions} and~\cite{mosteller1951-remarks}.}
\es{Luego de los trabajos fundamentales de~\cite{thurstone1927-comparativeJugement} y~\cite{zermelo1929-translation}, el campo revitalizó 25 años después gracias a las publicaciones de \cite{bradley1952-pairedComparisions} y~\cite{mosteller1951-remarks}.}
%
\en{The most widely used skill estimators today are those proposed by~\cite{elo1961-uscf},~\cite{glikman_gliko_2},~\cite{herbrich2006-trueskill}, and some of the Item-Response Theory variants compiled by~\cite{vanDerLinden2016-handbookIRT_1} and~\cite{fox2010-bayesianIRT}.}
\es{Los estimadores de habilidades más utilizados en la actualidad son los propuestos por~\cite{elo1961-uscf},~\cite{glikman_gliko_2},~\cite{herbrich2006-trueskill}, y algunas de las variantes de la\emph{Item-Response Theory} recopiladas por~\cite{vanDerLinden2016-handbookIRT_1} y~\cite{fox2010-bayesianIRT}.}
%
...
%
\en{In respect to their representations, the main difference is limited to the choice of one of two very similar cumulative probability distributions to define the equation~\ref{eq:pairwise-comparision}: the logistic function or the cumulative Gaussian one.}
\es{Respecto de su representación, la principal diferencia se limita a la elección de una de dos distribuciones de probabilidad acumulativa, muy similares entre sí, para definir la ecuación~\ref{eq: pairwise-comparision}: la función logística o la Gaussiana acumulativa.}
%
...
\es{Las diferencias sustantivas son diferencias metologógicas: métodos alternativos para resolución de el modelo probabilístico.}
...
\centering \small
    ...
    %\node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     \caption{
     \en{Generative graphical model representing the joint probability distribution, in which hidden skills ($s$) cause the observable results ($r$) through a random difference ($d$) of hidden performances ($p$).}
    \es{Modelo gráfico generativo que representa la distribución de probabilidad conjunta, en la que las habilidades ocultas ($s$) causan los resultados observables ($r$) a través de una diferencia aleatoria ($d$) de las actuaciones ocultas ($p$).}
    %
    \en{--.}
    \es{Las variables observables se pintan de gris y las variable ocultas se dejan en blanco.}
    %
    \en{--.}
    \es{Las flechas representan relaciones causales y las funciones definen esas relaci\'on causales.}
    %
    \en{--.}
    \es{Los puntos negros representan valores constantes.}
    %
    \en{--.}
    \es{Los c\'irculos y cuadrados representan variables aleatorias continuas y discretas respectivamente.}   
    }
...
\es{Un gran avance produjo la metodolog\'ia desarrollada por~\cite{elo1961-uscf} para la Federación de Ajedrez de los Estados Unidos (USCF), adoptada hasta el día de hoy por la Federación Internacional De Ajedrez (FIDE).}
%
\en{Figure~\ref{fig:generative_model} provides a graphical representation of the generative model, which details the causal relationship that hidden skills ($s$) have on the observable results ($r$), through the random difference ($d$) of hidden performances ($p$).}
\es{La Figura~\ref{fig:generative_model} ofrece una representación gráfica del modelo generativo, en la que se detalla la relación causal que las habilidades ocultas ($s$) tienen sobre los resultados observables ($r$), a través de la diferencia aleatorias ($d$) de rendimientos ocultos ($p$).}
%
\en{The performance that agents exhibit in each event is assumed to be random, centered around the real unknown skill $\N(p\,|\,s,\beta^2)$.}
\es{El desempeño que exhiben los agentes en cada evento se supone aleatorio, centrado en torno a la verdadera habilidad desconocida, $\N(s,\beta^2)$.}
%
\en{According to the model, the results are a direct consequence of the hidden performances.}
\es{Según el modelo, los resultados son consecuencias directa de los rendimientos ocultos.}
%
...
\es{Al observar un resultado podemos deducir, dadas las hipótesis, el orden de los rendimientos ocultos que en efecto ocurrió en ese evento, y su probabilidad a priori.}
%
\en{Elo's methodological solution for estimating skills from results is extremely simple and astute: update the current estimate based on our surprise.}
\es{La solución metodológica de Elo para estimar habilidades a partir de resultados es extremadamente sencilla y astuta: actualizar la estimación actual en función de nuestra sorpresa.}
%
\begin{equation}
...
\es{Los resultados poco probables indicarían que las estimaciones actuales no son del todo correctas y deberían actualizarse en mayor medida que si hubieran ocurrido como se esperaba.}
%
...
\es{Esta solución puede recuperar la escala relativa de los agentes, partiendo de de valores iniciales arbitrarios.}
%
...
\es{Debido a la falta de una noción de incertidumbre sobre las estimaciones, lo que un agente pierde en la ecuación~\ref{eq:elo_update} lo adquiere el otro.}
%
\en{As new estimates tend to generate high surprise, their presence can abruptly modify estimates that had already converged correctly.}
\es{Como las estimación nuevas tienden a generan alta sorpresa, su presencia puede modificar bruscamente estimaciones que ya habían convergido correctamente.}
%
\en{The ad-hoc solution proceeds by filtering the impact of $\Delta$ as a function of the times an agent was seen, which breaks the symmetry of equation~\ref{eq:elo_update}.}
\es{La solución ad-hoc procede filtrando el impacto de $\Delta$ en función de las veces que un agente fue visto, lo que rompe la simetría de la ecuación~\ref{eq:elo_update}.}

% Cambio de parrafo


\en{Probability Theory provides us with a consistent framework for quantifying the uncertainty of our beliefs, and to update them optimally given a model and the data~\cite{bishop2006-patternRecognitionMachineLearning}.}
\es{--.}
%
\en{The use of probabilty to represent uncertainty, however, is not an ad-hoc choice.}
\es{--.}
%
\en{Cox~\cite{cox} showed that if numerical values are used to represent degrees of belief, then a simple set of axioms encoding ``common sense'' that leads uniquely to a set of rules for manipulating degrees of belief that are equivalent to the sum and product rules of probability.}
\es{--.}
