...
\en{The parameter $\beta^2$ is the same for all the agents and acts as the scale of the estimations: skills separated by a $\beta$ keep always the same probability of winning.}
...
%
\en{The difference of performances, $d=p_i-p_j$, can also be expressed as a Gaussian distribution, centered on the difference of the prior estimate means ($\mu_i -\mu_j$), with a variance that incorporates the uncertainty of both estimates ($\sigma_i$ and $\sigma_j$) and the variance of both performances ($\beta$), $\N( d \, | \, \mu_i -\mu_j \, ,\ 2\beta^2 + \sigma_i^2 + \sigma_j^2 \,)$.}
...
\es{En la primera pasada, utilizamos las versolimulitudes pasadas para calcular las priores de cada evento.}
%
...
\en{In this section we show how to use \proglang{Julia}, \proglang{Python} and \proglang{R} packages to solve a single event, a short history of three events, and the real-life history of professional ATP tennis players.}
\es{En esta secci\'on mostramos c\'omo usar los usando los paquetes de\proglang{Julia}, \proglang{Python} y \proglang{R} para resolver un evento, una peque\~na historia de tres eventos y finalmente la historia real de jugadores profesional de tenis ATP.}
%
\en{We show TrueSkill and TrueSkill Through Time solutions, how to obtain the individual learning curves, and the prior prediction of the observed data (i.e. evidence) that is also used as a measure to compare models.}
\es{Mostraremos la soluciones TrueSkill y TrueSkill Through Time, el m\'etodo para obtener curvas de aprendizaje individuales, y la predicci\'on a priori (i.e. evidencia) del dato observado que sirven como medida para comparar modelos.}
%
% \en{Finally, we make a time complexity comparison between the three packages presented in this paper and the \texttt{trueskill} package of presented in this paper \proglang{Python}.}
% \es{Por \'ultimo haremos un comparaci\'on de complejidad temporal entre los tres paquetes que presentamos en este trabajo y el paquete \texttt{trueskill} de \proglang{Python}.}

\subsection{Single Event} \label{sec:singleEvent}

\en{To solve a single event we will use the class \texttt{Game()}, although the class \texttt{History()} (which we will present later) also allows to solve single event histories.}
\es{Para resolver un único evento utilizareomos la clase \texttt{Game()}, a pesar de que la clase \texttt{History()} (que presentaremos más adelante) también permita resolver historias de eventos únicos.}
%
\en{To perform the analysis using the class \texttt{Game()} we first need to define the characteristics of the participating players.}
\es{Para hacer el análisis mediante la clase \texttt{Game()} necesitamos definir primero las caracterísiticas de los jugadores participantes.}
%
\en{The class \texttt{Player()} has three attributes: the prior Gaussian distribution (characterized by the mean \texttt{mu} and the standard deviation \texttt{sigma}), the standard deviation of the performance (\texttt{beta}), and the dynamic factor of the skill (\texttt{gamma}).}
\es{La clase \texttt{Player()} tiene tres atributos: la distribuci\'on gaussiana a priori (caracterizada por la media \texttt{mu} y el desv\'io est\'andar \texttt{sigma}); el desv\'io est\'andar de los rendimientos (\texttt{beta}); y el factor din\'amico de la habilidad (\texttt{gamma}).}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:player, caption={\en{Initialization of players}},aboveskip=0.1cm]
...
\end{lstlisting}  
%
\en{In this example, we instantiate four players using the same initial parameters.}
\es{En este ejemplo instanciamos cuantro jugadores usando los mismos parámetros iniciales.}
%
...
\es{En presencia de equipos, the observed result depends on the sum of the performances of each team, $d = \sum_{i \in a} p_i) - \sum_{i \in b} p_i)$ (detalles en la secci\'on \ref{sec:2vs2}).}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={Initialization of teams and games}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
...
%
\en{During the initialization, the class \texttt{Game()} computes the prior prediction (\texttt{evidence}) of the observed result and the approximate \texttt{likelihoods}, one for each player.}
\es{Duarante la inicialización, la clase \texttt{Game()} calcula la predicción a priori (\texttt{evidence}) del resultado observado y las verosimilitudes aproximadas (\texttt{likelihoods}), una por cada jugador.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:evidence_likelihoods, caption={Evidence and likelihoods queries}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
lhs = g.likelihoods
...
\end{lstlisting}
%
\en{In this case, the evidence is $0.5$ indicating that both teams had the same probability of winning given the previous estimates.}
\es{En este caso, la evidencia es de $0,5$ indicando que ambos equipos tenian la misma probabilidad de ganar dadas las estimaciones previas.}
%
\en{Posteriors can be found by multiplying the likelihoods and priors, or we can call the method \texttt{posteriors()} to solve it for us.}
\es{Los posteriors se pueden obtener multiplicando a mano las verosimilitudes y los priors, o podemos llamar al método \texttt{posteriors()} para que lo resuelva por nosotros.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game_posterior, caption={Posterior query}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
pos = posteriors(g)
...
\end{lstlisting}
%
\en{The likelihoods and posteriors are returned respecting the order in which players and teams were loaded in the \texttt{Game()} class.}
\es{Las verosimilitudes y posteriors se devuelven respetando el orden en el que los jugadores y equipos fueron cargados en la clase \texttt{Game()}.}

% Parrafo

\en{Let's suppose now that the same four players participate in a game organized in 3 teams in which two of them draw and one wins.}
\es{Supongamos ahora que esos mismo cuatro jugadores participan en una partida organizados en 3 equipos en el que dos de ellos empatan y uno gana.}
%
\en{Unlike the previous example, now you need to specify the result and the probability of a tie associated with these types of games.}
\es{A diferencia del ejemplo anterior, ahora necesitas especificar el resultado y la probabilidad de empatar asociada a ese tipo de juegos.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={Initialization of teams and games}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
...
\end{lstlisting}
%
\en{Now the outcome of the game is explicitly defined: the team with the highest value beats the rest, and the teams with the same value tie.}
\es{Ahora el resultado de la partida se define explícitamente: el equipo con el mayor valor le gana al resto, y los equipos con mismo valor empatan.}
%
\en{When considering the possibility of a tie, it is necessary to indicate the approximate typical draw frequency for this type of game.}
\es{Al considerar la posibilidad de empate se hace necesario indicar la frecuencia de empate típica aproximada que tienen ese tipo de juegos.}
%
...
\todo[inline]{Que julia acepte resultados con enteros}
\todo[inline]{Verificar que si hay un empate el bico chille.}


\subsection{Environment}

\en{Before solving a history of events it is necessary to review the set of hyperparameters of the model (\texttt{Environment()}): the mean \texttt{mu} and standard deviation \texttt{sigma} of the priors; the standard deviation of the performances \texttt{beta}; the dynamic factor \texttt{gamma}; the draw probability \texttt{p\_draw}; and the convergence conditions \texttt{epsilon} and \texttt{iter}.}
\es{Antes de resolver una historia de eventos es necesario revisar el conjunto de hiperparámetros del modelo (\texttt{Environment()}): la media \texttt{mu} y el desvío estandar \texttt{sigma} de los priors; el desvío estandar de los desempe\~nos \texttt{beta}; el factor dinámico \texttt{gamma}; la probabilidad de empate \texttt{p\_draw}; y las condiciones de convergencia \texttt{epsilon} e \texttt{iter}.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:environment, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{lstlisting}[backgroundcolor=\color{all}]
env = Environment(mu=0., sigma=6., beta=1., gamma=.05, p_draw=0., epsilon=1e-6, iter=10)
env == Environment()
\end{lstlisting}
%
\en{The values used in the code \ref{lst:environment} are the default values of the \texttt{Environment()} class.}
\es{Los valores usados en el c\'odigo \ref{lst:environment} son los valores por defecto de la clase \texttt{Environment()}.}
%
\en{Some comments regarding the choice of parameters.}
\es{Algunos comentarios respecto a la elecci\'on de los parámetros.}
%
\en{The value chosen as the mean of the prior \texttt{mu} does not affect the estimates because the only thing that matters is the difference in skill between agents.}
\es{El valor que se elija como media del prior \texttt{mu} no afecta las estimaciones porque lo \'unico que importa es la diferencia de habilidad entre agentes.}
%
\en{The standard deviation of the prior \texttt{sigma} must be large enough not to leave out any possible hypothesis.}
\es{El desvío estandar del prior \texttt{sigma} debe ser suficientemente grande de modo de no dejar por fuera ninguna posible hip\'otesis.}
%
\en{This value depends on the scale imposed by the standard deviation of the performances \texttt{beta}.}
\es{Ese valor depende de la escala que imponga el desvío estandar de los rendimientos \texttt{beta}.}
%
\en{A skill difference of one $\beta$, $ s_i - s_j = \beta $, represents a 76\% probability of winning.}
\es{Una diferencia de habilidad de un $\beta$, $s_i - s_j = \beta$, representa 76\% de probabiilidad de ganar.}
%
\en{The other factor to take into account when choosing \texttt{sigma} is the nature of the problem, the number of ``levels'' that may exist in the system to be analyzed.}
\es{El otro factor a tener en cuenta para elegir el valor de \texttt{sigma} tiene que ver con la naturaleza del problema, la cantidad de ``niveles'' que pueden exisitir en el sistema a analizar.}
%
...
%
\en{Its optimal value can be obtained by maximizing the a priori prediction (i.e. evidence).}
\es{Su valor \'optimo se puede obtener maximizando la predicci\'on a priori (i.e. evidencia).}
%
\en{The rule of thumb proposes to initialize \texttt{p\_draw} with the empirical proportion of draws.}
\es{La regla general propone inicializar \texttt{p\_draw} con la proporci\'on empírica de empates.}
%
...
\en{The class \texttt{History()} is used to compute the posteriors and evidence of a history of events such as described in Figure~\ref{fig:smooth_example}.}
\es{La clase \texttt{History()} se usa para computar los posteriors y la evidencia de una historia de eventos como la descrita en la figura~\ref{fig:smooth_example}.}
%
\en{As an example, we initialize a history containing three players (\texttt{\small "a", "b", "c"}) with three games.}
\es{Como ejemplo, inicializamos una historia que contiene tres jugadores (\texttt{"a","b","c"}) con tres partidas.}
%
...
%
\en{In other words, all agents win one game and lose the other.}
\es{Es decir, todos los agentes ganan una partida y pierden la otra.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:history, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
...
res=list(c(0,1),c(0,1),c(0,1))
\end{lstlisting}
\end{paracol}
%
\en{where the variable \texttt{e1} indicates the composition of the first game using the names of the agents; the variable \texttt{events} contains the composition of the three games; and the variable \texttt{res} contains the results of the three games (the smallest value represents the winner).}
\es{Donde la variable \texttt{e1} indica la composici\'on de la primer partida usando los nombres de los agentes; la variable \texttt{events} contiene la composici\'on de las tres partidas; y la variable \texttt{res} contiene los resultados de las tres partidas (el menor valor representa el ganador).}
%
\en{Assuming that the agents' skill does not change between games, \texttt{gamma = 0.0}, the state of knowledge about their skills (i.e. the estimates) should be the same after observing the data.}
\es{Suponiendo que la habilidad de los agentes no cambia entre partidas, \texttt{gamma = 0.0}, el estado de conocimiento sobre sus habilidades (i.e. las estimaciones) debería ser la misma luego de observar los datos.}
%
\begin{lstlisting}[backgroundcolor=\color{all}]
env = Environment(gamma = 0.0)
h = History(events = events, result = res, env = env)
...
\en{History initialization also triggers the computation of TrueSkill estimates.}
\es{La inicializaci\'on de la historia tambi\'en activa el computo de las estimaciones TrueSkill.}
%
\en{We can access them using the method \texttt{learning\_curves()}, which returns a dictionary that is accessed with the name of the agents.}
\es{Podemos acceder a ellas mediante el m\'etodo \texttt{learning\_curves()}, que devuelve un diccionario de al que se accede con el nombre de los agentes.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:trueskill, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
...
\end{lstlisting}
%
\en{The individual learning curves are a list of tuples.}
\es{Las curvas de aprendizaje individuales son una lista de tuplas.}
%
\en{The first element indicates the batche number, and the second indicates the estimate corresponding to that batche.}
\es{El primer elemento indica el tiempo, y el segundo indica la estimaci\'on correpondiente a ese tiempo.}
%
\en{Although it would be reasonable to have the same estimate for all agents (because they earn each other), it can be seen that the posteriors reported by TrueSkill differ greatly between players and vary greatly for the same player.}
\es{A pesar de que lo razonable sería tener la misma estimaci\'on para todos los agentes (poque se ganan mutuamente), se puede ver que los posteriors reportados por TrueSkill difieren mucho entre jugadores y varían mucho para uno mismo jugador.}
%
...
\es{La \'unica diferencia es que el sistema reporta una \'unica estimaci\'on por bache.}
