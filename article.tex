...
\title{All the concepts behind TrueSkill Through Time: the \proglang{R}, \proglang{Python} and \proglang{Julia} packages}
\Plaintitle{All the concepts behind TrueSkill Through Time: the R, Python and Julia packages}
\Shorttitle{TrueSkill Through Time: the \proglang{R}, \proglang{Python} and \proglang{Julia} packages}
...
%
\en{This is made possible by a special integration of biological, cognitive and social processes~\citep{_Koster2020}.}
...
\en{Knowing how people's abilities change over time is crucial in many areas.}
...
%
\en{The main difference betweem them are methodological: the procedure to solve the shared probabilistic model.}
...
%
\en{Although agents may have a specific skill the performance they exhibit in an event depends on an undetermined number of unknown factors, which are modeled with a constant noise, $\N(p\,|\,s,\beta^2)$.}
\es{Si bien los agentes tienen una habilidad, el desempe\~no que exhiben en cada evento depende de una cantidad indeterminada de factores desconocidos, que se modelan con un ruido constante, $\N(p\,|\,s,\beta^2)$.}
%
\en{The observed result is a direct consequence of the difference of those hidden performances, $d_{ij}=p_i - p_j$.}
\es{El resultado observado es consecuencias directa de la diferencia de esos rendimientos ocultos, $d_{ij}=p_i - p_j$.}
%
...
\end{equation}
%
\en{Where $y_{ij}$ is $1$ for winners and $-1$ for losers, and the probability of the observed result given the a priori estimates are derived from Figure~\ref{fig:generative_model} (details at section~\ref{}).}
\es{Donde $y_{ij}$ es $1$ para ganadores y $-1$ para perdedores y la probabilidad del resultado observado dadas las estimaciones a priori se deriva de la figura~\ref{fig:generative_model} (detalles en la sección~\ref{}).}
%
...
%
\en{An ad-hoc solution was proposed to break the symmetry of equation~\ref{eq:elo_update}: reduce the impact of $\Delta$ based on how many times the agent was already seen by the system.}
...
%
\en{In section~\ref{} we will see in detail how equation~\ref{eq:inference} is solved in general.}
\es{En la sección~\ref{} veremos en detalle cómo se resuelve la ecuación~\ref{eq:inference} en general.}
%
...
%
\en{The mass that is assigned a posteriori to each hypothesis $s_i$ is proportional to the product of the prior and the likelihood.}
...
%
\en{Therefore, the likelihood compute all the mass up to the value 0 of a Gaussian distribution centered on the difference between the skill hypothesis and the opponent's mean estimate, ($\delta(s_i) = s_i - \mu_j$), with a variance that includes all uncertainties except the one of $s_i$, ($\vartheta_i^2$).}
...
%
\en{The posterior receives the mass of the prior no filtered by the likelihood.}
\es{El posterior recibe la masa del prior no filtrada por la verosimilitud.}
%
\en{For each hypothesis $s_i$, the filtered mass is exactly proportional to the magnitude of the surprise, defined as the complement of the likelihood.}
...
%
\en{At the region of very high skill hypotheses, where the winning result would have generated almost no surprise, the posterior receives almost all the mass of the prior.}
...
%
\en{Conversely, at the region of very low skill hypotheses, where the winning result would have generated a lot of surprise, the posterior receives almost no mass from the prior.}
...
%
\en{Since the shape of the exact posterior can be approximated well enough with a Gaussian, the computational cost can be avoided by implementing deterministic approximation techniques.}
\es{Dado que la forma del posterior exacto puede ser aproximado suficientemente bien con una Gaussiana, el costo computacional puede ser evitado implementando técnicas de aproximación determinística.}
%
...
%
\en{Methodological details are described in the section~\ref{}.}
\es{Los detalles metodológicos se describen en la sección~\ref{}.}
%
...
%
\en{In this way, information from the past propagates to future estimates.}
\es{De esta forma, la información del pasado propaga hacia las estimaciones futuras.}
%
...
%
\en{However, the imperfect representation of the temporal problem, as a sequence of independent events, introduces a series of problems.}
\es{Sin embargo, la representación imperfecta del problema temporal, como secuencia de eventos independientes, introduce una serie de problemas.}
%
\en{All weaknesses arise from the fact that in the "filtering approach" the information propagates in the system in only one direction.}
\es{Todas las debilidades surgen por el hecho de que en el "enfoque de filtrado" la información propaga en el sistema en un solo sentido.}
%
\en{The most obvious problem is that the beginning of any estimation sequence always has high uncertainty, which makes them useless.}
\es{El problema más evidente es que el inicio de toda secuencia de estimaciones siempre tiene alta incertidumbre, lo que las hace inservibles.}
%
\en{There are, however, a number of less obvious problems to keep in mind. }
\es{Existe sin embago una serie de problemas menos evidentes muy importantes a tener en mente.}
%
\en{In general terms, filtering approach estimates can suffer from ``temporary'' and ``spatial'' decoupling.}
\en{En términos generales, las estimaciones del enfoque de filtrado pueden sufrir desacoplamientos ``temporales'' y ``espaciales''.}
%
\en{To understand them it is important to remember that the observed results depend only on the difference in skills, and not on their absolute value.}
...
%
\en{Similarly, it may happen that within well-connected communities the relative difference is correct, but not between poorly connected communities, preventing comparability between communities.}
\es{De modo similar puede ocurrir que al interior de comunidades bien conectadas la diferencia relativa sea correcta, pero no entre comunidades poco conectadas, impidiendo comparabilidad entre comunidades.}
...
%
\en{It is difficult to imagine the complex structure that a graphical model of this nature can acquire.}
\es{Es difícil imaginar la compleja estructura que un modelo gráfico de esta naturaleza puede adquirir.}
%
\en{Having a single model allows historical information to propagate throughout the system, which definitely solves the decoupling problems mentioned above.}
\es{Contar con un único modelo permite que la información histórica propage hacia todo el sistema, lo que resuelve defnitivamente los problemas de desacoplamiento antes mencionados.}
%
...
\end{figure}
%
\en{Inference in this single causal model, as in any other model, is made by the only two rules of reasoning with uncertainty: the rule of addition and the rule of product.}
\es{La inferencia en este modelo causal único, como en cualquier otro modelo, se realiza mediante la únicas dos reglas del razonamiento con incertidumbre: la regla de la suma y la regla del producto.}
%
\en{However, due to the complexity of its structure, it is necessary to implement an iterative solution known as loopy belief propagation.}
\es{Sin embargo, por lo complejo de su estructura es necesario implementar un algoritmo iterativo eficiente, conocido como \emph{loopy belief propagation}.}
%
\en{This procedure converge with only a few linear iterations over the data, scaling to millions of observations in a few minutes.}
\es{Este procedimiento converge con sólo unas pocas iteraciones lineales sobre los datos, escalando a millones de observaciones en pocos minutos.}
%
\en{TrueSkill Through Time outperforms TrueSkill, and others competing models, in terms of model evidence.}
\en{TrueSkill Through Time supera a TrueSkill, y otros modelos alternativos, en términos de evidencia.}
%
En la siguiente figura se compara el comportamiento de las estimaciones de TTT y TrueSkill en dos escenarios sintéticos sencillos.
%

\begin{figure}[h!]
    \centering
    \en{\includegraphics[page={1},width=.4\linewidth]{figures/estimates.pdf}
    \includegraphics[page={2},width=.4\linewidth]{figures/estimates.pdf}}
    \caption{Comportamiento de las estimaciones de habilidad utilizando el TrueSkill Through Time, basado en el enfoque \emph{smoothing}, y TrueSkill, basado en el enfoque \emph{filtering}.
    %
    El escenario de la izquierda 3 jugadores con misma habilidad compiten entre sí y se ganan mutuamente, primero A le gana a B, B le gana a C y C le gana a A.
    %
    En el escenario de la derecha, dos comunidades separadas de 3 jugadores, cada una con misma habilidad al interior, pero entre ellos hay 2/3 de probabilidad de ganar se conectan al final.
    }
    \label{ejemplos}
\end{figure}
...
%
En estos dos ejemplos se puede ver claramente que las estimaciones TTT son mejores que las de TrueSkill.
%
