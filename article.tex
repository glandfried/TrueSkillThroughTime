...
\en{}
\en{El \emph{sum-product algorithm}~\cite{Kschischang2001} aprovecha la estructura del modelo causal involucrado para aplicar eficientemente las únicas dos reglas de inferencia: la~\ref{eq:sum_rule} y la~\ref{eq:product_rule}}
%
\en{}
\en{Los modelos causales, representados como distribuciones de probabilidad conjunta, pueden factorizarse en el producto de probabilidades condicionales simples.}
%
\en{}
\en{Nuestro modelo generativo $p(\bm{s},\bm{p},d,r)$ puede factorizarse como $p(s_1)p(s_2)p(p_1|s_1)p(p_2|s_2)p(d|\bm{p})P(r|d)$.}
%
\en{}
\en{En la figura~\ref{fig:factor_graph} mostramos su representación gr\'afica, conocida como \emph{factor graph}.}
%
\begin{figure}[t]
\centering \small
    \tikz{         
...
% 	\node[const, above=of nfr] (dfr) {\large $\mathbb{I}(d >0)$}; %
    ...
    %\node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     \caption{
     \en{}
     \en{Factorizaci\'on gráfica (o \emph{factor graph}) del modelo genertivo (Fig.~\ref{fig:generative_model}).}
     %
     \en{}
...
\en{}
\en{Los \emph{factor graph} son grafos bipartitos entre nodos variable $v$ y nodos factor $f$, en el que los ejes representan la relaci\'on matem\'atica ``el factor $f$ tiene como argumento el nodo $v$''.}
%
\en{}
\en{Sobre este tipo de estructuras se  cualquier marginal de la distribución de probabilidad conjunto, como por ejemplo la distribución a posteriori de las habilidades.}
%
\en{}
\en{Los pasos pueden }
%
\begin{mdframed}
El mensaje enviado desde un v\'ertice $v$ a trav\'es de un eje $e$ es el producto de la funci\'on local de $v$ (funci\'on indicadora si $v$ es un nodo variable) con todos los mensajes recibidos en $v$ a trav\'es de ejes \emph{distintos} de $e$, integrados para la variable asociada con $e$.
\end{mdframed}
%
...
N1 = Gaussian(mu=1., sigma=1.); N2 = N2 =Gaussian(1., 2.)  
\end{lstlisting}
%
...
\subsection{Exact posterior} \label{sec:exactSolution}
%
Las partidas con 2 equipos tienen solución exacta.
%
\begin{paracol}{3}
...
\end{paracol}
%
En términos gráficos, la partida se representa como 
%
\begin{figure}[H]
  \centering
