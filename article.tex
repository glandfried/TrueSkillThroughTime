...
%
\en{This produces a mutual dependence between estimates that forces us to iteratively use the last available likelihoods until convergence is reached (details in section~\ref{sec:throguthTime}).}
\es{Esto produce una mutua dependencia entre estimaciones que nos obliga a usar iterativamente las últimas verosimilitudes disponibles hasta alcanzar convergencia (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
\begin{figure}[ht!]
  \centering
  \scalebox{.9}{
...
%
\en{The information is stored in a single \texttt{csv} file\footnote{File available at \nolinkurl{https://github.com/glandfried/tennis_atp/releases/download/atp/history.csv.zip}. An updated data source at \nolinkurl{https://github.com/JeffSackmann/tennis_atp}.} including both single and double matches: if the column \texttt{double} has the letter \texttt{t}, the game is a double match.}
...
%
\begin{lstlisting}[backgroundcolor=\color{julia},label=lst:atp, caption={\en{The history of the Association of Tennis Professionals}\es{La historia de la Asociación de Tenistas Profesionales}}, belowskip=0.1cm, aboveskip=0.1cm]
...
\es{En otros ejemplos, donde la multidimensionalidad de las habilidades sea más relevante, es posible que convenga separar las estimaciones en diferentes componentes.}
%
...
%
\en{TrueSkill Through Time generates a unique causal model in which all historical activities are linked.}
\es{TrueSkill Through Time genera un modelo causal único en el que todas las actividades históricas quedan vinculadas.}
%
...
\caption{
\en{Nodes of the factor graph of a history of events that are neighbors to an agent's skill variable $i$ at time step $t$.}
\es{Nodos del grafo de factorizaci\'on de la historia de eventos que son vecinos a la variable de habilidad de un agente $i$ en el paso temporal $t$.}
%
...
%
\begin{equation}
 p(s_{i_t}) = \emph{prior}(t) \cdot \emph{inversePrior}(t) \cdot \prod_{k=1}^{K_{i_t}} \emph{likelihood}(t,k)
\end{equation}
%
\en{Where $K_{i_t}$ is the number of events in which agent $i$ participates in time step $t$.}
\es{Donde $K_{i_t}$ es la cantidad de eventos en los que participa el agente $i$ en el paso temporal $t$.}
%
\en{The \emph{likelihood} messages are the likelihoods of the events and were described in sections \ref{sec:approximate_posterior} and \ref{sec:iterative_posterior}, and the \emph{prior} and \emph{inversePrior} messages are the neighboring skill estimates, to which some uncertainty $\gamma$ is added due to the time step.}
\es{Los mensajes \emph{likelihood} son las verosimilitudes de los eventos descritas en la secciones \ref{sec:approximate_posterior} y \ref{sec:iterative_posterior}, y los mensajes \emph{prior} e \emph{inversePrior} son las estimaciones de habilidad vecinas, a las que se le agrega cierta incertidumbre $\gamma$ por el paso temporal.}
%
\begin{equation*}
\emph{prior}_i(t) = \N(s_{i_t}|s_{i_{t-1}}, \gamma^2) \ \ \ \ \ \ \emph{inversePrior}_i(t) = \N(s_{i_t}|s_{i_{t+1}}, \gamma^2)
\end{equation*}
%
\en{The message \emph{prior} matches the definition of prior in the basic TrueSkill model: it is the \emph{posterior} of the previous step after the dynamic uncertainty $\gamma$ is added.}
\es{El mensaje \emph{prior} coincide con la definción de prior que tiene el modelo básico de TrueSkill: es el \emph{posterior} del paso anterior luego de que se le agrega la incertidumbre dinámica $\gamma$.}
%
\en{This coincidence arises from applying the sum-product algorithm.}
\es{Esta coincidencia surge de aplicar el \emph{sum-product algorithm}.}
%
\en{But from its application also emerges the existence of the message \emph{inversePrior}, which can be seen as the posterior of the next event after the dynamic uncertainty $\gamma$ is added to it.}
\es{Pero de su aplicación también aparece la existencia del mensaje~\emph{inversePrior}, que puede ser visto como el posterior del siguiente evento luego de que se le agrega la incertidumbre dinámica $\gamma$.}
%
\en{These three messages, multiplied, compute the marginal distribution $p(s_{i_t})$.}
\es{Estos tres mensajes, multiplicados, computan la distribución marginal $p(s_{i_t})$.}
...
%
 \begin{equation}
 \emph{withinPrior}_i(t,k) = \emph{prior}_i(t) \cdot \emph{inversePrior}_i(t) \cdot \prod_{\hfrac{q=1}{q\neq k}}^{K_{i_t}} \emph{likelihood}_i(t,q) = \frac{p(s_{i_t}) }{\emph{likelihood}_i(t,k)}
 \end{equation}
%
\en{The \emph{withinPrior} contain the same information as the maringal distribution $p(s_{i_t})$ except that which comes from the likelihood of the event for which they are priors.}
\es{Los \emph{withinPrior} contienen toda la informaci\'on de la distribución marginal $p(s_{i_t})$ salvo la que proviene de la verosimilitud de la partida para la que son priors.}
%
...
\end{equation*}
%
\en{The messages that are not yet defined, for example the \emph{inversePrior} in the first forward pass, are replace it by a neutral form like a Gaussian distribution with infinite variance.}
\es{Los mensajes que no est\'an todav\'ia definidos, por ejemplo el \emph{inversePrior} en la primer pasada, son remplazados por una forma neutral como la distribución gaussiana con varianza infinita.}
%
\en{To solve the mutual dependency between likelihoods, the idea is to update repeatedly forward and backward until convergence.}
...
%
\en{At each forward pass we keep track the last forward message of each agent \emph{prior}$_i(t+1)$, and at each backward pass we keep track the last backward message of each agent, \emph{inversePrior}$_i(t-1)$.}
\es{En cada pasada hacia adelante guardamos el último mensajes que los agentes envían para adelante, \emph{prior}$_i(t+1)$, y en cada pasada hacia atrás guardamos el último mensajes que los agentes envían para atrás, \emph{prior}$_i(t+1)$.}
%
\en{This algorithm requires only a few linear iterations on the data and allows scaling to millions of observations in a few seconds.}
\es{Este algoritmo requiere s\'olo unas pocas iteraciones lineales sobre los datos y permite escalar a millones de observaciones en pocos minutos.}
