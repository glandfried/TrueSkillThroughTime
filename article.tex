...
%    
\en{At the region of very high skill hypotheses, where the winning result would have generated almost no surprise ($\lim_{s_i \to \infty}P(r|s_i) = 1$), the posterior receives most of the prior's density.}
...
%
\en{At the region of very low skill hypotheses, where the winning result would have generated huge surprise ($\lim_{s_i \to -\infty}P(r|s_i) = 0$), the posterior does not receive density from the prior.}
...
\en{It is important to stress that the posterior, although similar, is not a Gaussian distribution, initially preventing us from using equation~\ref{eq:posterior_win} iteratively.}
...
%
\en{We show both TrueSkill and TrueSkill Through Time solutions, how to obtain the individual learning curves, and the prior prediction of the observed data (i.e. evidence) that is also used as a measure to compare models.}
...
\todo[inline]{Contar que para poder hacer el analisis de un evento, necesitamos instanciar ratings (o como se llame) y decir en esta oracion qu\'e se modela con esta clase, y mas o menos para que se usa}
...
\es{La clase \texttt{Player()} se representa con cuatro atributos: la media (\texttt{mu}) y el desv\'io est\'andar (\texttt{sigma}) de la distribuci\'on gaussiana a priori; el desv\'io est\'andar de los rendimientos (\texttt{beta}); y el factor din\'amico de la habilidad (\texttt{gamma}).}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:rating, caption={\en{Example of initialization of players}}]
...
\end{lstlisting}  
%
\en{In this example, we instantiate four players using the same initial parameters. The \texttt{Gaussian()} class is used to model the standard operations of Gaussian distributions including multiplication, summation, division, and differentiation. The next step is using the instantiated players in a game.}
\es{Los rating tienen tres atributos.}

% Parrafo

\en{The class \texttt{Game()} storages the game details (i.e. teams and result) and computes the likehood and the evidence.}
...
%
\en{When dealing with teams and following a natural extension to the single player model, the observed result is a direct consequence of the difference of the team performance (see details at section \ref{sec:2vs2}).}
\es{En presencia de equipos, el resultado observado se modela como consecuencias directa de la diferencia de los rendimientos de cada equipo, $d = \sum_{i \in a} p_i) - \sum_{i \in b} p_i)$ (detalles en la secci\'on \ref{sec:2vs2}).}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={Example of teams initialization and creation of a game}, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
...
g = Game(teams, result, p_draw = 0.0)
\end{lstlisting}
%
\todo[inline]{Arreglar como se informa el resultado del partido}
\en{In this example, \texttt{result} has the outcome of the game, and \texttt{p\_draw} is used to consider the probability of a draw, when this parameter is zero or is omitted there is no chance of a draw.}
\es{donde \texttt{result} indica el orden observado (los equipos con menor n\'umero son los ganadores), y \texttt{p\_draw = 0.0} indica que la probabilidad de empate nula para ese tipo de juegos.}
%
\en{The approximate \texttt{likelihoods} (one for each agent) and the \texttt{evidence} are computed during the initialization stage.}
\es{Los atributos de la clase \texttt{Game()} son los \texttt{teams} y el \texttt{result}, los \texttt{likelihoods} aproximados (uno por cada agente) y la \texttt{evidencia}.}
%
\en{This class enables to query about the prior probability of the observed result:} 
\es{La evidencia es $0.5$, indicando que la probabiilidad a priori del resultado observado era la misma para ambos equipos.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game_evidence, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=-0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
ev = g.evidence
...
\en{In this case, the evidence is $0.5$ showing that the both teams have the same previous skill estimation.}
\es{GUSTAVO COMPLETA}
%
\en{This class also provides the posteriors for each agent (they are also Gaussian distributions) which can be computed by calling a method:}
\es{Los posteriors de cada agente son gaussianas que pueden computarse de los likelihoods y priors llamando a un m\'etodo.}
%
...
\end{lstlisting}
\todo[inline]{Explicar como se accede a los posteriors y que hace este codigo}
