...
%
\en{However, because of the form that the exact posterior takes, a Gaussian would be a very good approximation.}
\es{Sin embargo, por la forma que toma el posterior exacto, una Gaussiana sería una muy buena aproximación.}
%
\en{One of the main contributions of~\cite{herbrich2006-trueskill} was to offer an efficient method to find the Gaussian that best approximates the exact posterior.}
\es{Uno de los principales aportes de~\cite{herbrich2006-trueskill} fue ofrecer un método eficiente para calcular la Gaussiana que mejor aproxima a la posterior exacta.}
%
%A diferencia de los métodos de sampleo, muy costosos computacionalmente, los métodos variacionales ofrecen soluciones analíticas aproximadas muy eficientes. 
%
...
%
\en{The simplest approach treat a sequence of observations as an ordered list of independent causal models, as shown in the Figure~\ref{fig:filter}.}
\es{El enfoque más sencillo trata las secuencia de observaciones como una lista ordenada de modelos causales indepedientes, como se muestra en la Figura~\ref{fig:filter}.}
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
  ...
  \caption{
  \en{The filter approach treat the sequence of observations as an ordered list of independent events.}
  \es{El enfoque de filtro considera la secuencia de observaciones como una lista ordenada de eventos independientes.}
  %
...
\end{figure}
%
\en{Even the graphical model assumes independent events, the \emph{filter approach} uses the posterior estimates at time $t-1$ as prior of time $t$.}
\es{Aunque el modelo gráfico asume eventos independientes, el \emph{enfoque de filtro} usa las estimaciones a posteriori del tiempo $t-1$ como como prior del tiempo $t$.}
%
...
\end{equation}
%
\en{If the likelihood are filters for the priors, the estimates made at a time $t$ can be seen as the accumulation of all previous filters.}
\es{Si la verosimilitud es un filtro para el prior, las estimaciones hechas en el tiempo $t$ pueden ser vistas como la acumulación de todos los filtros anterios.}
%
...
%
\en{In our case, as skills are dynamic variables it is important to add uncertainty in each step.}
\es{En nuestro caso, como las habilidades son variables dinámicas es importante agregar incertidumbre en cada paso.}
%
\begin{equation}\label{eq:dynamic_factor}
 \text{Prior}^t \sim \N(\text{Posterior}^{t-1}, \gamma^2)
 \end{equation}
%
La solución variacional de filtro dinámico, conocida como TrueSkill~\cite{herbrich2006-trueskill}, mostró resultados superiores a sus antecesores.
%
Superó al sistema Glicko, que se había presentado como una mejora respecto de Elo gracias a que incorporaba la noción de incertidumbre de la estimación y ofrecía estimaciones que convergían al verdadero valor con menos cantidad de datos.
%Sin embargo, el sistema Glicko utiliza una función logísitica para representar la probabilidad de ganar dado la diferencia de rendimientos de modelo.
%
De modo similar a TrueSkill, Glickman ofreció un método para aproximar la posterior mediante una Gasussiana.
%
Sin embargo, la metodología no ofrece garantías de que la distribución aproximada sea la que mejor aproxime a la posterior exacta.
%
% Además el sistema Glicko requiere una cierta cantidad de observaciones antes de implementar el proceso de inferencia luego de tener .
% %
Por otra parte, TrueSkill superó las versions Bayesianas de Item-Response Theory descritas por FOX debido a que resuelven el modelo gr\'afico mediante técnicas de sampleo muy costosas computacionalmente.
%
%Si bien las metodologías de sampleo tienen la propiedad de converger a la distribución exacta con muestra de tamaño infinito, debido a que el posterior exacto es parecido a una Gaussiana la aproximación Gaussiana es lo suficientememnte buena y no tiene costo computacional asoaciado.

% Parrafo

Todas estas solcuiones, TrueSkill, Glicko, BIRT, aplican el abordaje de filtro.
%
Su representación imperfecta del problema temporal tiene asociada una serie de debilidades.
%asociados con el hecho de que la secuancia de estimaciones van acumulanndo información del pasado, como si la información propagara del pasado al futuro
%
El problema más evidente es la alta incertidumbre que tienen las primeras estimaciones de la secuencia, lo que impide generalmente tenerlas en cuenta para los análisis.
%
Sin embargo existe una serie de problemas menos evidentes pero igual de importantes a tener en cuenta. 
%
En términos generales, las estimadciones puede sufrir desacoplamientos ``temporales'' y ``espaciales''.
%
...
%
Puede ocurrir que entre estimaciones contemporáneas la diferencia relativa sea correcta, pero que en el tiempo el valor absoluto se desliza en direcciones arbitrarias.
%
También puede ocurrir al interior de comunidades bien conectadas la diferencia relativa sea correcta, pero que entre comunidades poco conectadas los valores absolutos no son comparables entre sí.
%
...
Estos problemas se solucionan si en vez de trabajar con una lista ordenada de modelos causales indepedientes, se trabajar con un único modelo causal que vincule en un solo sistema de inferencia todas las actividades históricas.
%
