...
\en{A superior solution, instead of selecting a single value as an estimate, involves quantifying the ``relative certainty'' associated with each of the skill hypotheses.}
\es{Una solución superior, a la de seleccionar una único valor como estimación, consiste en cuantificar la ``certidumbre relativa'' asociada a cada una de las hipótesis de habilidad.}
%
\en{This approach, known as Bayesian inference, which has proven successful in practice~\citep{bishop2006-PRML}, ensures consistent reasoning in handling this kind of uncertainty (plausible beliefs)~\citep{jaynes2003-bookProbabilityTheory,VanHorn2003}.}
\es{Este enfoque conocido como inferencia Bayesiana, que ha demostrado ser exitoso en la práctica~\citep{bishop2006-PRML}, garantiza un razonamiento consistente en el manejo de este tipo de incertidumbre (creencias plausibles)~\citep{jaynes2003-bookProbabilityTheory,VanHorn2003}.}
%
\en{All inference, no matter how complex it is, can be solved by the rules of probability: the~\ref{eq:sum_rule} and the~\ref{eq:product_rule}.}
...
%
\en{In our case, to reduce the uncertainty about the hidden skill using the information provided by the observed result and the described causal model, we need to solve:}
...
\underbrace{p(\,s_i\, | \, r \, )}_{\text{Posterior}} \propto \underbrace{1-\Phi(0 \, |  \overbrace{s_i^{\textcolor{white}{2}} - \mu_j}^{\hfrac{\text{\en{Expected}\es{Diferencia}}}{\text{\en{difference}\es{esperada}}}} , \, \overbrace{2\beta^2 + \sigma_j^2}^{\hfrac{\text{\en{Total}\es{Incertidumbre}}}{\text{\en{uncertainty}\es{total}}}} )}_{\text{\en{Likelihood}\es{Verosimilitud}}} \,  \underbrace{\N(s_i \, | \, \mu_i,\, \sigma_i^2)}_{\text{Prior}} 
\end{equation}
%
\en{Where the normalized posterior can be found dividing the right hand by the evidence ${P(r)}$.}
\es{Donde el el posterior normalizado se obtiene dividiendo el lado derecho por la evidencia, $P(r)$.}
%
\en{The likelihood is just the cumulative density of all positive values of a Gaussian distribution centered at the difference between the hypothesis $s_i$ and the opponent's mean estimate $\mu_j$, with a variance that includes all uncertainties except the one of $s_i$.}
\es{La verosimilitud es la densidad acumulada de los valores positivos de una distribuci\'on Gaussiana centrada en la diferencia entre la hip\'otesis $s_i$ y la estimaci\'on media del oponente $\mu_j$, $\delta(s_i)$, con una varianza que incluye todas las incertidumbres salvo la de la propia hip\'otesis $s_i$, $\vartheta_i^2$. }
%
