...
%
\en{Suppose we track the new player for 1000 games, and during that time his actual skill increases by two points ($\beta$) following a logistic function.}
\es{Supongamos que le hacemos seguimiento al nuevo jugador durante 1000 partidas, y que durante ese tiempo su habilidad real aumenta dos puntos ($\beta$) siguiendo una función logísitica.}
%
\en{Here we only show the \proglang{Python} version.}
\es{Aquí mostramos solo la versión de \proglang{Python}.}
%
\en{The codes for \proglang{Julia} and \proglang{R} can be found in the appendix \ref{AGREGA-CODE}.}
\es{Los códigos de \proglang{Julia} y \proglang{R} se encuentran en el anexo \ref{AGREGA-CODIGO}.}
%
\begin{lstlisting}[backgroundcolor=\color{python}, label=lst:synthetic_skill, caption={\en{Synthetic learning curve}\es{Curva de aprendizaje sintética}}, belowskip=0.0, aboveskip=0.1cm]
from numpy import exp
def skill(experience, middle, maximum, slope):
    return maximum/(1+exp(slope*(-experience+middle)))
mu_agent = [skill(i, 500, 2, 0.0075) for i in range(1000)]
\end{lstlisting}
%
\en{In this example we want to infer this learing curve from the results of the games.}
\es{En este ejemplo queremos inferir esta curva de aprendizaje a partir de los resultados de las partidas.}
%
\en{}
\es{Supongamos que los oponentes tienen habilidad similar, se encuentran alrededor de la verdadera habilidad del nuevo jugador.}
%
\en{}
\es{Los resultados van a depender de los rendimientos ambos jugadores}
\begin{lstlisting}[backgroundcolor=\color{python}]
from numpy.random import normal, seed
seed(999)
mu_other = normal(mu_agent,scale=0.5,size=1000)
def perf(mu, beta=1.0):
    return normal(mu,beta)
\end{lstlisting}
