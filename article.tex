...
\todo[inline]{Poner un poco m\'as de venta de la importancia, el inter\'es y el potencial uso de saber la habilidad de alguien}
%
...
\es{Luego de los trabajos fundamentales de~\cite{thurstone1927-comparativeJugement} y~\cite{zermelo1929-translation}, el campo revitaliz\'o 25 a\~nos despu\'es gracias a las publicaciones de \cite{bradley1952-pairedComparisions} y~\cite{mosteller1951-remarks}.}
%
...
\es{Los estimadores de habilidades m\'as utilizados en la actualidad son los propuestos por~\cite{elo1961-uscf},~\cite{glikman_gliko_2},~\cite{herbrich2006-trueskill}, y algunas de las variantes de la\emph{Item-Response Theory} recopiladas por~\cite{vanDerLinden2016-handbookIRT_1} y~\cite{fox2010-bayesianIRT}.}
%
...
\centering \small
    ...
    \es{Modelo gr\'afico generativo que representa la distribuci\'on de probabilidad conjunta, en la que las habilidades ocultas ($s$) causan los resultados observables ($r$) a trav\'es de una diferencia aleatoria ($d$) de las actuaciones ocultas ($p$).}
    %
    \en{--.}
...
\es{La Figura~\ref{fig:generative_model} ofrece una representaci\'on gr\'afica del modelo generativo, en la que se detalla la relaci\'on causal que las habilidades ocultas ($s$) tienen sobre los resultados observables ($r$), a trav\'es de la diferencia aleatorias ($d$) de rendimientos ocultos ($p$).}
%
...
\es{El desempe\~no que exhiben los agentes en cada evento se supone aleatorio, centrado en torno a la verdadera habilidad desconocida, $\N(s,\beta^2)$.}
%
...
\es{La soluci\'on metodol\'ogica de Elo para estimar habilidades a partir de resultados es extremadamente sencilla y astuta: actualizar la estimaci\'on actual en funci\'on de nuestra sorpresa.}
%
\begin{equation}
...
\es{La soluci\'on ad-hoc procede filtrando el impacto de $\Delta$ en funci\'on de las veces que un agente fue visto, lo que rompe la simetr\'ia de la ecuaci\'on~\ref{eq:elo_update}.}
