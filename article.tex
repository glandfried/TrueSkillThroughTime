...
%
\en{however they are hidden variable, so in practice the best we can do is estimating it from its direct observable consequences: the outcome of problem solving and competitions.}
...
%
\en{The naive solution, frequency of positive results, is not an informative measure because it depends on the difficulty of the challenges.}
\es{La soluci\'on ingenua, frecuencia de resultados positivos, no es un métrica informativa porque depende de la dificultad de los desaf\'ios.}
%
\en{For this reason, any solution should rely on pairwise comparisons.}
\es{Por esta razón, cualquier solución debe basarse en comparaciones por pares.}
%
...
% 
\en{A breakthrough took place with the methodology developed for the US Chess Federation (USCF)~\cite{Elo2008}, which is still used by the International Chess Federation (FIDE).}
\es{Un gran avance produjo la metodolog\'ia desarrollada para la Federaci\'on de Ajedrez de los Estados Unidos (USCF)~\cite{Elo2008}, adoptada hasta el d\'ia de hoy por la Federaci\'on Internacional de Ajedrez (FIDE).}

% Parrafo

\en{Nowadays, the most widely used skill estimators are based on the Elo ranking system~\cite{Elo2008,Glickman1999,Herbrich2007} while a similar approach is used by some variants of the Item-Response Theory~\cite{VanDerLinden2016,Fox2010}.}
\es{Actualmente, los estimadores de habilidades m\'as utilizados son los propuestos por~\cite{Elo2008,glikman_gliko_2,Herbrich2007}, y algunas de las variantes de la\emph{Item-Response Theory} recopiladas por~\cite{VanDerLinden2016,Fox2010}.}
%
...
% %
\en{All them shared the same probabilistic model.}
\es{Todos ellos compartían el mismo modelo probabilístico.}
%
...
%
\en{Given current estimates, $s_i$ and $s_j$, each possible future outcome has an associated probability, which will generate a certain surprise when observing the result.}
\es{Dadas las estimaciones actuales, $s_i$ y $s_j$, cada posible resultado futuro tiene una probabilidad asociada, lo que al observar el resultado nos generará una sorpresa.}
%
...
%s
\begin{equation} \label{eq:elo_delta} \tag{\text{Elo's update}}
...
%
\begin{equation}\label{eq:elo_update} \tag{\text{Elo's skill}}
...
%
\en{To break the symmetry of equation~\ref{eq:elo_update}, an ad-hoc solution was proposed: reduce the impact of $\Delta$ based on how many times the agent was already seen by the system.}
...
%
\en{Any system that represent degrees of belief by a numerical value, consistent with classical deductive logic and a set of axioms encoding commonsense reasoning, will necessarily be isomorphic to Probability Theory~\citep{Cox1946,VanHorn2003}.}
\es{Cualquier sistema que represente grados de creencia mediante un valor num\'erico, consistente con la l\'ogica deductiva cl\'asica y un conjunto de axiomas que codifican el razonamiento de sentido común,  ser\'a necesariamente isom\'orfico a la Teor\'ia de la Probabilidad~\citep{Cox1946,VanHorn2003}.}
%
...
%
\begin{equation}\label{eq:event_inference} \tag{\text{\en{event's }inferenc\en{e}\es{ia de eventos}}}
...
%
\en{In section~\ref{sec:analyticalSolution} we will see in detail how an~\ref{eq:event_inference} is solved in general.}
\es{En la secci\'on~\ref{sec:analyticalSolution} veremos en detalle c\'omo la~\ref{eq:event_inference} se resuelve en general.}
%
...
%
\begin{equation}\label{eq:posterior_win} \tag{\text{\en{winner's exact posterior}\es{posterior exacto del ganador}}}
...
%
\en{Therefore, the likelihood for a winner agent is the density corresponding to the positive values of Gaussian distribution centered at the difference between their skill hypothesis and the opponent's mean estimate, ($\delta(s_i) = s_i - \mu_j$), with a variance that includes all uncertainties except the one of $s_i$, ($\vartheta_i^2$).}
\es{Por lo tanto, la verosimilitud del agente ganador es la densidad correpondiente a los valores positivos de una distribuci\'on Gaussiana centrada en la diferencia entre la hip\'otesis de su habilidad $s_i$ y la estimaci\'on media del oponente $\mu_j$, ($\delta(s_i)$), con una varianza que incluye todas las incertidumbres salvo la de la propia hip\'otesis $s_i$, ($\vartheta_i^2$).}
%
...
%
\en{This problem can be addressed through Markov chain, as~\cite{Fox2010} does to solve the Bayesian Item-Response Theory (BIRT) variants.}
\es{Este problema puede ser tratado mediante cadenas de Markov, tal como hace~\cite{Fox2010} para resolver las variantes Bayesianas de Item-Response Theory (BIRT).}
%
\en{While sampling methods allow intractable integrals to be solved, the associated computational complexity imposes severe limits to the applications.}
\es{Si bien las métodos de sampleo permiten resolver cualquier integrales intratables, la complejidad computacional asociada impone severos límites las aplicaciones.}
%
...
%
\en{Due the shape of the exact posterior, a Gaussian seem to be a good enough approximation, allowing us to avoid the computational cost of the sampling methodologies.}
...
%
\begin{equation} \label{eq:approx} \tag{\text{\en{approximate posterior}\es{posterior aproximado}}}
 \widehat{p}(s_i| r_{ij}, s_j) = \underset{\mu, \sigma}{\text{ arg min }} \ \ \text{KL}(\, p(s_i| r_{ij}, s_j) \, || \,  \N(s_i|\mu, \sigma^2)\, )
\end{equation}
%
...
%
\en{The approach adopted by TrueSkill, Glicko, and BIRT to treat a sequence of observations, known as \emph{filtering}, uses each \ref{eq:approx} as the prior for the next event.}
\es{El enfoque adoptado por TrueSkill, Glicko y BIRT para tratar una secuencia de observaciones, conocido como \emph{filtering}, utiliza cada \ref{eq:approx} como prior del siguiente evento.}
%
...
\es{Si consideramos la verosimilitud como un filtro del prior, las estimaciones hechas en el tiempo $t$ pueden ser vistas como la acumulaci\'on de todos los filtros anterios.}
%
