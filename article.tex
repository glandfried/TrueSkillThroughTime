...
\todo{R debe admitir \texttt{list("a","b")}}
...
\end{paracol}
\begin{lstlisting}[backgroundcolor=\color{all}]
h = History(events = events, env = Environment(gamma = 0.0))
> History(Events=3, Batches=3, Agents=3)
\end{lstlisting}
\todo{History debe admitir result por defecto}

La carga de los datos genera el resultado TrueSkill
...
Todos ganan contra todos, pero se puede ver que los posterior del jugador \texttt{a} varía mucho, cuando lo que esperábamos sería que todos tengan misma habilidad.
%
Para correr TTT necesitamos converger el modelo
