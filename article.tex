...
%
\en{The information is obtained and stored in a single \texttt{csv} file\footnote{Available at \url{https://github.com/glandfried/tennis_atp/releases/download/atp/history.csv.zip}. Additional data at Jeff Sackmann's site: \url{https://github.com/JeffSackmann/tennis_atp}.} including both single and double matches: if the column \texttt{double} has the letter \texttt{t}, the game is a double match.}
...
%
\en{Here we only show the \proglang{Julia} code because it is far more efficient than \proglang{Python} and \proglang{R} versions (in section~\ref{sec:computationDetails} we show performance comparison of the three versions) which can also be found in the appendix~\ref{sec:appendix_atp_code}.}
...
%
\en{We assume the skill of each player does not change during a tournament and we add uncertainty (based on $\gamma$ parameter) before each tournament according to the elapsed time between competitions.}
...
\en{}
\es{En esta sección discutiremos sobre como evaluar modelos.}
%
\en{}
\es{Qusieramos tener una medida que nos permita decidir si TrueSkill Through Time es mejor a TrueSkill, pero también que nos permita decidir los valores de los parámetros $\sigma$ y $\gamma$, o que nos permita decidir si conviene modelar habilidades multi-dimensionales, como hicimos en el código \ref{}, o no.}
%
\en{}
...
%
\en{}
\es{donde $P(\text{Modelo})$ es el prior de los modelos, que definimos nosotros, y $P(\text{Datos}|\text{Modelo})$ es la evidencia, el denominador del teorema de bayes cuando evaluamos hipótesis, que en un evento de TrueSkill coincide con la ecuación \ref{eq:evidence}.}
%
\en{}
\es{Si ambas expresiones podemos calcularlas para todo el espacio de modelos alternativos que estamos evaluando, entonces también podemos calcular el denominador $P(\text{Datos})$.}
%
\begin{equation}
...
% Parrafo
\en{}
\es{La probabilidad del modelo $P(\text{Modelo}|\text{Datos})$ solo puede computarse cuando tenemos acceso a todo el espacio modelos.}
%
\en{}
\es{Cuando nos falta alguno, podemos comparar modelos.}
%
\begin{equation}
...
%
\en{}
\es{En el caso especial en el que no tengamos preferencia a priori sobre ningún modelo, comparar modelos se reduce a comparar predicciones a priori de los modelos.}
%
\begin{equation}
...
%
\en{}
\es{Usando la regla del producto, sabemos que siempr podemos descomponer la probabilidad conjunta de los datos como el producto de probabilidades condicionales unidimensionales.}
%
...
%
\en{}
\es{Para evaluar esta expresión conviene calcular su valor en escala logarítmica.}
%
...
%
\en{}
\es{En TrueSkill, esta expresión se obtiene sumando el logaritmo de todas las predicción a priori que se hacen siguiendo la ecuación \ref{eq:evidence} usando el último posterior como prior de esa partida.}
%
\en{}
\es{En TrueSkill Through Time, en cambio, computar esta exresión resulta computacionalmente más complicado.}
%
\en{}
\es{En la sección \ref{sec:throughTime} vimos que cuando modelamos correctamente la historia, el prior de los eventos se corresponde con el mensaje \emph{whitinPrior}, que contienen la información histórica del resto de los eventos, pasados y futuros.}
%
\en{}
\es{Entonces, todas las predicciones a priori calculada con esos mensajes están condicionadas sobre el resto de los datos, $p(d_i| d_1 \dots d_{i-1}, d_{i+1} \dots d_n , M)$, por lo que su multiplicación no se corresponde con la probabilidad conjunta de los datos.}
%
\begin{equation} \label{eq:loocv}
 P(\text{D}|\text{M}) \neq \prod_i p(d_i|\wedge_{j\neq i} d_j , M) = \text{LOOCV}
\end{equation}
%
\en{}
...
%
\en{}
\es{Esta expresión \ref{eq:loocv} coincide con una técnica de validación cruzada conocida como \emph{leave-one-out croos-validation} (LOOCV).}
%
\en{}
\es{Para computar en TrueSkill Through Time la predicción a priori usando sólo la información pasada sería necesario subir los datos de a uno y converger cada vez.}
%
\en{}
\es{Si bien esto se puede resolver en principio, su complejidad computacional es factorial por lo que es prohibitivo en bases de datos grandes.}
%
\en{}
\es{Por este motivo decidimos que el método \texttt{log\_evidence()} de la clase \texttt{History}, que usamos para evaluar modelos, devuelva el valor calculado en la parte derecha de la ecuación \ref{eq:loocv}.}
%
\en{}
\es{Si bien el producto de las validaciones cruzadas \emph{leave-one-out} no se correponde con la predicción conjunta de los datos, funciona muy bien en la práctica para seleccionar modelo.}
