...
%
\en{The community is generated by ensuring that each opponent has a skill similar to the target player and follows its evolution.}
...
%
\begin{lstlisting}[backgroundcolor=\color{python}, label=lst:simulated_skill, caption={\en{Initialization of the target's learning curve and the community of opponents}\es{Simulación de la curva de aprendizaje objetivo y la comunidad de oponentes}}, belowskip=0.1cm, aboveskip=0.1cm]
...
%
\en{To generate the list \texttt{results}, we sample the performance of the players using the previously generated skills: the outcome of each game depends on the highest performance.}
...
%
\en{The variable \texttt{time} is a list of integer values ranging from 0 to 999 representing the date of each game: the class \texttt{History} uses it to add dynamic uncertainty ($\gamma^2$) proportionally to the days between each game.}
...
%
\en{The variable \texttt{priors} is a dictionary containing the customization of the players' features.}
\es{La variable \texttt{priors} es un diccionario que se puede utilizar para personalizar las características de los jugadores.}
%
\en{In particular, we assign a low uncertainty to the opponents' priors considering we know their skill beforehand.}
...
\en{The class \texttt{History} receives those four parameters and initializes the target player using the default values except for the dynamic factor \texttt{gamma=0.015}.}
\es{La clase \texttt{History} recibe los cuatro parámetros, e inicializa al resto de los jugadores (en este caso sólo el agente objetivo) con los valores por defecto y un factor dinámico \texttt{gamma=0.015}.}
%
\en{Then, we use the method \texttt{convergence()} to obtain the TrueSkill Through Time estimates and the target's learning curve.}
...
%
\en{To generate the data for Figure~\ref{fig:logistic}, we repeat this procedure and shows the evolution of the true (solid line) and estimated (dashed line) learning curves of the target player's.}
...
\begin{figure}[H]
    ...
    %
    \en{The dark and light gray areas show once and twice the uncertainty of one of the estimates.}
    \es{La zona gris oscura (clara) ilustra una vez (dos veces) la incertidumbre de la estimación.}
    }
    \label{fig:logistic}
\end{figure}
%
...
%
\en{For the Fig~\ref{fig:logistic_sigma}, we select one of the estimated learning curves and present its uncertainty to show that the actual learning curve is contained in the uncertainty interval.}
...
%
\en{This example shows that TrueSkill Through Time correctly estimates and follows the skill evolution of a new player.}
\es{Con este ejemplo podemos ver que en todo momento TrueSkill Through Time estima correctamente las habilidades reales de los jugadores nuevos que se incorporan a comunidades de jugadores ya conocidos.}
...
%
\en{Here we only show the \proglang{Julia} version because it is more efficient than \proglang{Python} and \proglang{R} versions, which can be also found in the appendix \ref{AGREGA-CODE}.}
...
%
\en{Times are defined as days elapsed since a reference date to the tournament start.}
\es{Los tiempos los definimos como días transcurridos desde una fecha base (cualquiera sea) hasta el inicio del torneo.}
%
...
% %
\en{To generate the list \texttt{composition} we differentiate if the games are doubles or singles based on the column \texttt{double}.}
...
%
\en{When initializing the class \texttt{History} we also modify the default values of \texttt{sigma} and \texttt{gamma} based on an optimization procedure previously performed (more details next).}
\es{Al inicializar la clase \texttt{History} modificamos los valores por defecto de \texttt{sigma} y \texttt{gamma}.}
%
\en{Finally, we use the \texttt{convergence()} method to obtain TrueSkill Through Time estimates explicitly selecting the convergence criterion: when the change between iterations is less than $0.01$ or when $10$ iterations are performed.}
\es{Finalmente, convergemos la historia para obtener las estimaciones de TrueSkill Through Time, indicando explícitamente el criterio de corte: cuando la modificación entre iteraciones sea menor a $0.01$, o cuando se alcanzan las $10$ iteraciones.}
%
\en{Figure~\ref{fig:atp} presents the learning curves of eight players of the ATP's history.}
\es{En la siguiente figura graficamos las curvas de aprendizaje de ocho jugadores importantes de la historia de la ATP.}
%
\begin{figure}[h]
    \centering
