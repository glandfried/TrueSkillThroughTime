...
\begin{algorithm}[H]
  ...
  \lstinputlisting{code/ejemplo.py}
\end{algorithm}
...
\end{leftbar}
This tutorial will guide you through some of typicals \textbf{TTT} package application.
...
The first examples is the most basic one we figure out. Its one of the first examples in the official python package of Trueskill. Ower library, its almost four times faster than the official.

\lstinputlisting{./code/Example1.py}

\subsection{Example 2: Trueskill dataset}
For this example  we want to show hoy trueskill can be used with a data set. For this we will use de as input Table \ref{tab:csvExample2}. Its importan to de Result intput as shown. In this case we will use the libraty Pandas, but it can be done with out it, the only important its how to upload the data set and loop with it.
...
One of the advantage of \textbf{TTT}, its the way that the algorithm pass information to de past to the future and vice versa, getting a more accurate estimation. In adition, it can define a temporal batch were its supose that in there the order of the games it doesn't matter. For that, we use the next example. Three player, three games, that each one wins one game and lose the other one. So if we do trueskill, the order of how we compute the games will have importance, making at the end three player with three different skill. With \textbf{TTT} this kind of problem its solve. 
