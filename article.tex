...
\en{In this section we show how to use \proglang{Julia}, \proglang{Python} and \proglang{R} packages to solve a single event, a sequences of three events, the skill evolution of a player, and the analysis of the historical players of the Association of Tennis Professionals (ATP).}
...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:history, caption={\en{Initialization of a \texttt{History}'s instance using a sequence of three events}\es{Historia inicializada con una secuencia de tres eventos}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia},belowskip=-0.77 \baselineskip]
c1 = [["a"],["b"]]
...
\end{lstlisting}
%
\en{where the variables \texttt{c1}, \texttt{c2}, and \texttt{c3} model the composition of the each game using the names of the participating agents (i.e. identifiers), the variable \texttt{composition} is a list containing all the events to be considered in the analysis, and the parameter \texttt{gamma} determines the change of one agent's skill between games (in this case 0 means no change).}
\es{donde la variable \texttt{c1} indica la composici\'on de la primer partida usando los nombres (i.e. identificadores) de los agentes, la variable \texttt{composition} es una lista que contiene la composición de la tres partidas y el par\'ametro \texttt{gamma=0.0} indica que la habilidad de los agentes no cambia entre partidas.}
%
...
\en{After initialization, the \texttt{History} class immediately triggers the computation of TrueSkill estimates: it instantiates a new player for each name, initializes a game for each event, and applies the dynamic factor to the players' estimates between events.}
...
\es{Las curvas de aprendizaje individuales son listas de tuplas: el primer elemento de cada tupla indica el tiempo, y el segundo la estimaci\'on correspondiente a ese tiempo.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:trueskill, caption={\en{Obtaining the learning curves for the players participating in a sequence of events}\es{Consulta de curvas de aprendizaje}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}, belowskip=-0.77 \baselineskip]
...
\end{lstlisting}
%
\en{The learning curves of players \texttt{"a"} and \texttt{"b"} contain one tuple per game but the initial prior is omitted.}
\es{Las curvas de aprendizaje de ambos jugador \texttt{"a"} y \texttt{"b"} contienen solamente dos tuplas, una por partida.}
%
\en{As every player wins one game and loses the other, we expect to obtain the same skill estimate for all agents. 
But in this case, the final skill reported by TrueSkill differ between players. Moreover, the skill estimate for each player has strong variations between events.}
...
\en{For example, the first tuple of both players corresponds to the first game between them in which \texttt{"a"} beat \texttt{"b"}.}
\es{La primera tupla de ambos se corresponde con el tiempo 1, la partida en la que \texttt{"a"} le gana a \texttt{"b"}.}
%
\en{Both estimates have the same uncertainty. They also have the same mean absolute value, but positive for the winner and negative for the loser.}
...
%
\en{The second tuple for each player includes the result of their game against \texttt{"c"}. In both cases, the posteriors show a decrease in the uncertainty and the mean closer to zero.}
\es{La segunda tupla de cada jugador tiene menor incertidumbre y la media se encuentra más cerca de cero.}
%
\en{We expected the final estimates to be equal for all players, but we can see strong differences.}
...
%
\en{The TrueSkill Through Time model solves this problem by propagating the information through all the system.}
