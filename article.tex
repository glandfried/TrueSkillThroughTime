...
%
\en{Life histories include a long period of childhood, short intervals between births, and a long post-reproductive life span~\citep{Jones2011}.}
\es{Las historias de vida incluyen un largo período de infancia, intervalos cortos entre nacimientos, y una prolongada esperanza de vida post-reproductiva~\citep{Jones2011}.}
%
\en{The brains develop first followed by the body because this sequence allows a longer period of learning~\citep{GonzalezForero2018}.}
\es{Los cerebros se desarrollan primero, seguidos por el cuerpo porque esta secuencia permite un período más largo de aprendizaje~\citep{GonzalezForero2018}.}
%
...
%
\en{Our cognitive ability to acquire cultural information makes human adaptation a population-based process~\citep{Herrmann2007}.}
\es{Nuestra capacidad cognitiva para adquirir información cultural hace que la adaptación humana sea un proceso poblacional~\citep{Herrmann2007}.}
%
...
%
\en{In respect to their representations, the difference between them is limited to the choice of one distributions from the bell-shape family to define the cumulative probability $P(\, d_{ij} > 0 \,)$.}
\es{Respecto a sus representaciones, la diferencia entre ellas se limita a la elección de una distribución de la familia del funciones \emph{bell-shape} para definir la probabilidad acumulada de $P(\, d_{ij} > 0 \,)$.}
%
\en{The main difference is the methodological one: the procedure used for solving the probabilistic model.}
\es{La principal diferencia es la metodológica: el procedimiento utilizado para resolver el modelo probabilístico.}
% Cambio de parrafo.
\begin{figure}[h!]
\centering \small
    \tikz{         
...
%
\en{Suprise works as a filter for the prior beliefs.}
\es{La sorpresa (1-likelihood) la que filtra las creencias a priori.}
%
% \en{The mass that the proportional posterior receives from the prior is exactly the magnitude of the likelihood.}
% \es{La masa que el posterior proporcional recibe del prior es exactamente la magnitud del likelihood.}
%
