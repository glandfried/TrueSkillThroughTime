...
\es{Nuestra extraordinaria capacidad para imitar, combinada con los largos períodos de aprendizaje juvenil y vida posreproductiva, permite a los humanos aprender de los dem\'as y transmitir las innovaci\'on a trav\'es de la generaciones~\citep{citar}.}
%
...
%
\en{Since they are hidden variables, the best we can do is estimate them based on their direct observable consequences: the outcome of problem solving and competitions.}
...
%
\en{Considering the frequency of positive results as an indicator of the ability of individuals is a misconception, mainly because its value also depends on the difficulty of the challenges.}
\es{Considerar la frecuencia de resultados positivos como indicador de la habilidad de los individuos es un error de concepto, fundamentalmente porque su valor depende tambi\'en de la dificultad de los desaf\'ios.}
%
\en{For this reason, all widely used skill estimators are based on pairwise comparisons.}
\es{Por esta raz\'on, todos los estimadores de habilidad ampliamente usados se basa en comparaciones por pares.}
%
\en{Since the first generative models, proposed almost a century ago by~\cite{Thurstone1927} and~\cite{zermelo1929-translation}, it is assumed that the probability of an observed result, $r$, depends on the performance, $p$, of the agent $i$ and their opponent $j$, expressed as $P(\, r \,|\, p_i, \, p_j \,)$.}
\es{Desde los primeros modelos generativos, propuestos hace casi un siglo por~\cite{Thurstone1927} y~\cite{zermelo1929-translation}, se supone que la probabilidad de un resultado observado, $r$, depende del rendimiento, $p$, del agente $i$ y de su oponente $j$, expresada como $P(\, r \,|\, p_i, \, p_j \,)$.}
%
...
\es{El campo sigui\'o progresando con los trabajos de \cite{Bradley1952} y~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}.}
% 
\en{A breakthrough took place with the methodology developed by~\cite{Elo2008} for the US Chess Federation (USCF), which is still used by the International Chess Federation (FIDE).}
\es{Un gran avance produjo la metodolog\'ia desarrollada por~\cite{Elo2008} para la Federaci\'on de Ajedrez de los Estados Unidos (USCF), adoptada hasta el d\'ia de hoy por la Federaci\'on Internacional de Ajedrez (FIDE).}

% Parrafo

\en{All currently widely used skill estimators share some variant of the probabilistic model proposed by the Elo~\citep{Glickman1999,Herbrich2007,VanDerLinden2016,Fox2010}.}
\es{Todos los estimadores de habilidad ampliamente utilizados actualmente comparten alguna variante del modelo probabilístico propuesto por el Elo~\cite{Glickman1999, Herbrich2007, VanDerLinden2016, Fox2010}.}
%
\begin{figure}[h!]
\centering \small
    \tikz{         
...
%
%
\en{Figure~\ref{fig:generative_model} provides a graphical representation in which skills generates the observable result.}
\es{La figura~\ref{fig:generative_model} ofrece una representaci\'on gr\'afica en la que las habilidades generan el resultado observable.}
%
\en{The model assumes that the result is a direct consequence of the performances, which vary between events around the true skill with some constant noise,  $\N(p\,|\,s,\beta^2)$.}
\es{El modelo supone que el resultado es consecuencia directa de los rendimientos, que varían entre eventos alrededor de la verdadera habilidad con cierto ruido constante, $\N(p\,|\,s,\beta^2)$.}
%
% \en{Then, the observed result is a direct consequence of the difference of the performance of each agent in a particular event (i.e. game), $d_{ij}=p_i - p_j$.}
%
\en{The one with the highest performance wins, $r = (p_i > p_j)$.}
\es{Gana quien haya obtenido mayor rendimiento, $r = (p_i > p_j)$.}

% Parrafo

\en{Elo's methodological solution is simple and astute: update the previous estimates, $s_{i_\text{old}}$ and $s_{j_\text{old}}$, based on the surprise, i.e. the complement of the prediction of the observed result.}
...
%
%empezar con estimaciones arbitrarias
%
...
\end{equation}
%
\en{Where the probability arises from instantiating the previous estimates, $s_{i_\text{old}}$ and $s_{j_\text{old}}$, in the generative model (details in section~\ref{sec:exactSolution}).}
\es{Donde la probabilidad surge de instanciar las estimaciones previas en el modelo generativo (detalles en la sección~\ref{sec:exactSolution}).}
%
\en{The idea is that the surprise, $\Delta$, is related to the quality of the previous estimates and therefore serves to update them.}
\es{La idea es que la sorpresa, $\Delta$, está relacionada con la calidad de las estimaciones previas y por lo tanto sirvan para actualizarlas. }
%
\en{Unexpected results would indicate that current estimates are not entirely correct and should be updated to a greater extent than if they had occurred as expected.}
...
\es{Donde la sopresa, $\Delta$, act\'ua como factor de correcci\'on para ambas estimaciones previas.}
%
...
%
\en{The update rule (Eq.~\ref{eq:elo_update}) is symmetric, so what one agent wins is lost by the other.}
\es{La regla de actualizaci\'on (Eq.~\ref{eq:elo_update}) es sim\'etrica, as\'i que lo que un agente gana lo pierde el otro.}
%
\en{Because new agents start with arbitrary estimates (the same initial value for any individual), they tend to generate high surprise and abruptly modify estimates that had already converged correctly.}
\es{Debido a que a los agentes nuevos comienzan con estimaciones arbitrarias (el mismo valor inicial para cualquier individuo), ellas tienden a generan alta sorpresa y a modificar bruscamente estimaciones que ya habían convergido correctamente.}
%
\en{The weak point of the methodological solution proposed by Elo is the lack of a notion of uncertainty.}
\es{El punto débil de la solución metodológica propuesta por Elo es la falta de una noción de incertidumbre.}
%
\en{To solve this problem, an ad-hoc solution was proposed: reducing the impact of the surprise, $\Delta$, based on the number of times the agent has participated.}
\es{Para resolver este problema, una soluci\'on ad-hoc fue propuesta: reducir el impacto de la sorpresa, $\Delta$, en funci\'on de la cantidad de veces que el agente ha participado.}
%
...
\en{Instead of selecting a single value as an estimate, we would like to quantify the ``relative certainty'' associated with each of the skill hypotheses.}
\es{En vez seleccionar un único valor como estimación, quisiéramos cuantificar la ``certidumbre relativa'' asociada a cada una de las hipótesis de habilidad.}
%
\en{The use of probability theory to handle this type of uncertainty (or beliefs), known as Bayesian inference, has been widely adopted not only because it has been shown to be a successful tool in practice~\citep{bishop2006-PRML}, but also because its rules have been proven to be the only ones that guarantee consistent reasoning~\citep{jaynes2003-bookProbabilityTheory,VanHorn2003}.}
\es{El uso de la teor\'ia de la probabilidad para tratar este tipo de incertidumbre (o creencias), conocida como inferencia Bayesiana, se ha adoptado ampliamente no sólo porque ha mostrado ser una herramienta exitosa en la práctica \citep{bishop2006-PRML}, sino también porque se ha demostrado que sus reglas son las únicas que garantizan un razonamiento consistente~\citep{jaynes2003-bookProbabilityTheory,VanHorn2003}.}
%
\en{All inference, no matter how complex, can be solved in terms of this two simple equations: the~\ref{eq:sum_rule} and the~\ref{eq:product_rule}.}
\es{Toda inferencia, no importa cuan compleja ella sea, puede ser resuelta a trav\'es de estas dos simples ecuaciones: la~\ref{eq:sum_rule} y la~\ref{eq:product_rule}.}
%
...
%
\en{The \ref{eq:sum_rule} state that any marginal distribution can be obtained by integrating, or summing up, the joint distribution.}
...
\end{equation}
%
\en{Additionally, the \ref{eq:product_rule} state that any joint distribution can be expressed as the product of one-dimensional conditional distributions.}
...
%
\en{From these rules, jointly with the symmetry property $p(x,y) = p(y,x)$, we can immediately obtain the~\ref{eq:bayes_theorem}:}
...
%
\en{The \ref{eq:bayes_theorem} allows to optimally update our beliefs about the hypotheses, given a model and the data.}
...
%
\en{In our case, to reduce the uncertainty about the hidden skill using the information provided by the observed result and the described causal model, we need to solve the following equation.}
\es{En nuestro caso, para reducir la incertidumbre sobre la habilidad oculta usando la informaci\'on que nos ofrece el resultado observado y el modelo causal descrito, necesitamos resolver la siguiente ecuaci\'on.}
%
...
\end{equation}
%
\en{Where the hidden skills are the hypotheses, the observed results are the data, and the model is defined by the figure~\ref{fig:generative_model}.}
...
%
\en{The only free variable is the skill hypothesis}
\es{La única variable libre es la hip\'otesis de habilidad.}
%
\en{The prior represents the previous uncertainty, and the posterior is the updated uncertainty after incorporating the information provided by the data.}
\es{El prior representa la incertidumbre previa, y el posterior es la incertidumbre actualizada luego de incorporar la información provista por el dato.}
%
\en{Note that the likelihood and the evidence are both probabilities of the observed result, so they can be seen as predictions.}
\es{Notar que la verosimilitud como la evidencia son probabilidades del resultado observado, por lo que pueden ser vistas como predicciones.}
%
\en{Since the results are discrete variables, those probabilities are written with capital letters.}
\es{Como los resultados son variables discretas, esas probabilidad se escribe con letras mayúsculas.}
%
\en{Evidence, for its part, is a prediction made with all the prior hypotheses, so its value does not change when different hypotheses are evaluated.}
\es{La evidencia, por su parte, es una predicción hecha con todas las hipótesis a priori, por lo que no cambia su valor cuando se evaluan diferentes hipótesis.}
%
\en{Therefore, the only factor that updates our beliefs is the likelihood.}
\es{Es decir, el único factor que actualización nuestras creencias es la verosimilitud.}
%
\en{The posterior uncertainty of each hypothesis is proportional to the product of their prior uncertainty and their likelihood.}
\es{La incertidumbre a posteriori de cada hip\'otesis es proporcional al producto de su incertidumbre a priori y su verosimilitud.}

% Parrafo

\en{As an instance, equation~\ref{eq:posterior_win} shows the belief update for a winning case, using Gaussian priors.}
...
%
\en{In section~\ref{sec:exactSolution} we will see in detail how equation~\ref{eq:event_inference} arises by applying the sum and product rule over the probabilistic model.}
\es{En la secci\'on~\ref{sec:exactSolution} veremos en detalle c\'omo la ecuaci\'on~\ref{eq:event_inference} surge de aplicar las reglas de las suma y el producto sobre el modelo probabil\'istico.}
%
...
\underbrace{p(s_i , \, r \, | \, s_j\, )}_{\hfrac{\text{\scriptsize Proportional}}{\text{\scriptsize posterior}}} = \underbrace{1-\Phi(0 \, |  \overbrace{s_i^{\textcolor{white}{2}} - \mu_j}^{\hfrac{\text{\en{Expected}\es{Diferencia}}}{\text{\en{difference}\es{esperada}}}} , \, \overbrace{2\beta^2 + \sigma_j^2}^{\hfrac{\text{\en{Total}\es{Incertidumbre}}}{\text{\en{uncertainty}\es{total}}}} )}_{\text{\en{Likelihood}\es{Verosimilitud}}} \,  \underbrace{\N(s_i \, | \, \mu_i,\, \sigma_i^2)}_{\text{Prior}} 
\end{equation}
%
\en{Where the proportional posterior is just the unnormalized posterior, $p(s_i| r, s_j) = \frac{p(s_i, r|s_j)}{P(r)}$, and the likelihood is the cumulative density of all positive values of a Gaussian distribution centered at the difference between the hypothesis $s_i$ and the opponent's mean estimate $\mu_j$, with a variance that includes all uncertainties except the one of $s_i$.}
\es{El posterior no es m\'as que el posterior proporcional normalizado por la evidencia, $p(s_i| r, s_j) = \frac{p(s_i, r|s_j)}{P(r)}$, y la verosimilitud es la densidad acumulada de los valores positivos de una distribuci\'on Gaussiana centrada en la diferencia entre la hip\'otesis $s_i$ y la estimaci\'on media del oponente $\mu_j$, $\delta(s_i)$, con una varianza que incluye todas las incertidumbres salvo la de la propia hip\'otesis $s_i$, $\vartheta_i^2$. }
%
\en{In other words, the likelihood is just the prior prediction of the observed winning result, made with all the skill opponent's hypotheses, and assuming true the skill hypothesis of the agent we are evaluating.}
\es{En otras palabras, la verosimilitud no es más que la predicci\'on a priori del resultado observado ganador, realizada con todas las hip\'otesis de habilidad del oponete, y suponiendo verdadera la hipótesis de habilidad del agente que estamos evaluando.}
%
...
\es{Por la forma del posterior exacto, una Gaussiana parece una aproximaci\'on suficientemente buena, que nos permitiría evitar el costo computacional de la metodolog\'ias de sampleo.}
%
...
\es{Esta clase de m\'etodos, que minizan la divergencia Kullback-Leibler entre la distribuci\'on verdadera y aproximada, permiten realizar Inferencia Bayesiana en situaciones que de otra forma no sería posibles.}
%
...
\es{El principal aporte del sistema Glicko~\cite{glikman_gliko_2} fue el desarroll\'o de un m\'etodo eficiente para aproximar la posterior exacta con una distribuci\'on Gasussiana.}
%
...
\es{El \'exito de la soluci\'on TrueSkill~\citep{Herbrich2007} se basa en la aplicaci\'on de m\'etodo eficiente para calcular la Gaussiana que mejor aproxima a la posterior exacta.}
%
...
% \es{Este avance t\'ecnico permiti\'o aplicar la ecuaci\'on~\ref{eq:posterior_win} sobre una secuencia de observaciones.}
...
% \es{Gracias a que en cada paso usamos el posterior aproximado como prior podemos aplicar la ecuaci\'on~\ref{eq:posterior_win} para calcular el posterior exacto y su \ref{eq:approx}.}
%
...
\es{El enfoque de filtrado es muy útil, pero como su soluci\'on no se  corresponde a ningún modelo probabilístico, surge un conjunto de problemas.}
%
...
%Dabido a que los resultados dependen s\'olo de la diferencia de habilidades, y no de su valor absoluto, p
...
\begin{figure}[h!]
  ...
  \es{Representaci\'on esquemática de un único modelo causal para toda la historia de eventos.}
  %
  ...
  \es{La soluci\'on probabilística da como resultado el abordaje conocido como \emph{smoothing}, en el que la informaci\'on hist\'orica fluye por todo el sistema.}
  %
...
\es{La distribuci\'on a prior en un evento $t$, con $\gamma = 0$, es ahora la multiplicaci\'on de toda la informaci\'on pasada y futura.}
%
\begin{equation}\label{eq:smooth_prior}
 \text{Prior}_{i_t} = \text{Prior}_{i_0} \underbrace{\prod_{k = 1}^{t-1} \text{\en{Likelihood}\es{Verosimilitud}}_{i_k}}_{\text{\en{Past information}\es{Informaci\'on pasada}}} \underbrace{\prod_{k = t + 1}^{T} \text{\en{Likelihood}\es{Verosimilitud}}_{i_k}}_{\text{\en{Future information}\es{Informaci\'on futura}}}
\end{equation}
%
...
\es{Al empezar, las verosimilitudes que aun no están definidas se inicializan con un valor neutro o simplemente se las excluy\'e.}
%
...
\begin{figure}[h!]
  ...
  \es{Convergencia de TrueSkill Throguth Time en una historia con s\'olo dos eventos y dos agentes.}
  %
  ...
  \es{La primera (la más clara) se correponde a la soluci\'on TrueSkill.}
  %
  \en{The last one (the darkest) corresponds to the TrueSkill Throguth Time solution.}
  \es{La última (la más oscura) se correponde a la soluci\'on TrueSkill Throguth Time.}
  }
...
\es{Impulsado por la creciento en la capacidad de c\'omputo ocurrida, en las últimas cinco d\'ecadas los investigadores han desarrollado miles de algoritmos exitosos de aprendizaje de automático, hechos a medida de conjuntos de problemas específicos.}
%
\en{In contrast, probabilistic inference offers a generally applicable framework~\citep{Bishop2013}.}
\es{Por el contrario, la inferencia probabilística ofrece un marco de aplicaci\'on general~\citep{Bishop2013}.}
%
...
\es{Del mismo modo, los algoritmos empleadas varían de aplicaci\'on en aplicaci\'on, obligándonos a aprender una gran variedad de m\'etodos.}
%
...
\es{En lugar de tener que transformar el problema para que encaje en algún algoritmo estándar, la inferencia probabilística ofrece los bloques de construcci\'on que permiten resolver cualquier problema específico.}
%
...
\es{Todos los supuestos se hacen explícitos a trav\'es de un modelo generativo (o causal), que puede ser especificado intuitivamente de forma gráfica.}
%
...
\es{En la secci\'on~\ref{sec:sumProductAlgorithm} introduciremos el \emph{sum-product algorithm}, que permite computar eficientemente cualquier distribuciones de probabilidad marginal a partir de una distribuciones de probabilidad conjunta.}
%
...
%
% Afortunadamente, gracias a la distribuci\'on de probabilidad elegida para representar el modelo causal~\ref{modelo_elo}, la distribuci\'on de creencias a posteriori de la habilidad de los agentes y la predicci\'on a priori del resultado observado (aka evidencia) tienen soluci\'on analítica.
%
...
\es{En las secciones \ref{sec:exactSolution}, \ref{sec:empate}, \ref{sec:approximate _posterior}, y \ref{sec:iterative_posterior} mostramos respectivamente c\'omo resolver la probabilidad a priori y el posterior exacto de un evento, modificamos el modelo para que incluya empates, explicamos c\'omo aproximar el posterior y damos la soluci\'on general multi-equipos.}
%
FALTA HISTORY
% %
% En la secci\'on~\ref{history} mostamos los pasos matemáticos requeridos para resolver el modelo TTT.
% %
...
\es{En la figura~\ref{fig:factor_graph} mostramos gráficamente la factorizaci\'on.}
%
...
\es{Los ejes entre variables y funciones representan la relaci\'on matemática ``la variable $v$ es argumento de la funci\'on $f$''.}
%
\begin{figure}[h!]
\centering \small
    \tikz{         
...
% 	\node[const, above=of nfr] (dfr) {\large $\mathbb{I}(d >0)$}; %
    ...
    %\node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     %
     \en{Los cuadrados negros representan las funciones, los círculos blancos representan las variable, y los ejes entre ellos representan la relaci\'on matem\'atica ``la variable es argumento de la funci\'on''.}
     %
     }
...
\es{La estructura codifica la mínima cantidad de pasos que se requieren para calcular cualquier distribuci\'on de probabilidad marginal.}
%
...
\es{Los mensajes que envían los nodos variables codifican una porci\'on de la regla del producto.}
%
...
\es{En pocas palabras, los mensajes que envía una variables $v$ es simplemente la multiplicaci\'on de los mensajes que recibi\'o del resto de sus vecinos $h \in n(v)$ salvo $f$.}
%
...
\subsection{\en{Mathematical properties and notation}\es{Propiedades matemáticas y notaci\'on}}\label{sec:propiedades}
...
\es{La clase sobreescribe los operadores suma (\texttt{+}), resta (\texttt{-}), producto (\texttt{*}) y divisi\'on (\texttt{/}) con las principales propiedades requeridas para computar las distribuciones marginales en el modelo TrueSkill Throguth Time.}
%
...
\es{En la figura~\ref{fig:modelo_trueskill_2vs2} mostramos su factorizaci\'on gráfica.} 
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
...
\es{Debido a las reglas del algoritmo de suma-producto y a la factorizaci\'on del modelo, los mensajes de los factores de habilidad $f_{s_i}$ a su variable $s_i$ no son otra cosa más que el prior.}
%
...
\es{El siguiente mensaje, que la variable $s_i$ le envía al factor rendimiento $f_{p_i}$ tambi\'en será el prior.}
%
...
\es{Recordar que los mensajes que envían las variables es tan solo la multiplicaci\'on de los mensajes que esa variable recibe del resto de los factores.}
%
\en{Since they are trivial, we will not make these types of messages explicit.}
\es{Por ser de trivial soluci\'on, no haremos explícitos este tipo de mensajes.}
%
...
\es{En la figura~\ref{fig:draw_a} se puede ver en t\'erminos gráficos las probabilidades de los tres resultados posibles.}
%
...
% \es{En cualquier caso, una alternativa es usar la estrategia clásica de optimizaci\'on: elegir el valor de $\varepsilon$ que maximiza alguna funci\'on de costo.}
% %
% \begin{equation*}
% \varepsilon = \underset{\varepsilon}{\text{arg max}} \ p(\text{Data}|\text{Model}_{\varepsilon})
% \end{equation*}
% %
\subsection{\en{Optimal approximation of the exact posterior}\es{Aproximaci\'on \'optima del posterior exacto}} \label{sec:approximate _posterior}
...
% %
% 
% \en{Dado que la forma exacta se parece a una Gaussiana (figura~\ref{fig:posterior_win}), usar una Gaussiana para representar el posterior no parecería ser tan grave en t\'erminos de aproximaci\'on y en cambio nos permitiría realizar estimaciones sucesivas.}
% 
% % Parrafo
...
\es{Los paquetes lo resuelven con las siguientes dos líneas de c\'odigo.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{En esta secci\'on veremos como encontrar la mejor aproximaci\'on al posterior exacto en casos en los que tenemos más de dos equipos.}
%
...
\es{Los mensajes que aún no están definidos se inicializan con una forma neutra, como una distribuci\'on gaussiana con varianza infinita.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{En esta secci\'on vamos a ver c\'omo se obtiene la mejor aproximaci\'on al posterior exacto en una historia de eventos en la que $n$ agentes compiten durante $T$ pasos temporales o \emph{batches} (e.g. día, semana, mes, año).}
% 
...
\es{En la figura~\ref{fig:history} mostramos representaci\'on esquemática del grapho de factorizaci\'on de un historia de eventos.}
%
