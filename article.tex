...
%
\en{The \emph{factor graph} are bipartite graphs, consiting of variable nodes $v$ and function nodes (or factors) $f$.}
...
%
\en{In short, the messages sent by a function $f$ to a neighboring variable $v$ is simply the integration over $v$ of the product of itself and all the messages that $f$ receives from the rest of its neighbors $\bm{h}$ except $v$.}
\es{En pocas palabras, los mensajes que enviado por una función $f$ a una variable vecina $v$ es simplemente la integración sobre $v$ del producto de si mismo con todos los mensajes que $f$ recibe del resto de sus vecinos $\bm{h}$ salvo $v$.}
%
...
%
 ...
 %
 \en{En la figura~\ref{modelo_trueskill_2vs2} mostramos su factorización gráfica.} 
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
...
\en{Las partidas con dos equipos tienen solución exacta y los likelihoods exactos pueden computarse en \proglang{Julia}, \proglang{Python} y \proglang{R} mediante el método \texttt{exact\_likelihoods()}.}
%
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
...
\end{paracol}
%
\en{En esta sección vamos a mostrar los pasos para calcular la evidencia  y un likelihood exacto de una partida con dos equipos de dos jugadores.}
%
\en{Empezaremos primero con los mensajes ``descendentes'', desde los priors hasta el resultado, para calcular la evidencia.}
%
\en{Luego seguiremos con los mensajes ``ascendentes'' que permiten calcular el posterior de un agente a partir del resultado observado y el prior del resto de los agentes.}
%
%\en{Para computar los mensajes utilizaremos solamente el \emph{sum-product algoriithm} y las propiedades arriba mencionadas.}
