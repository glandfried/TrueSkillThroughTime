...
\es{Por la forma del posterior exacto, una Gaussiana parece una aproximaci\'on suficientemente buena, que nos permitiría evitar el costo computacional de las metodolog\'ias de sampleo.}
%
...
%
\en{The success of the TrueSkill solution~\citep{Herbrich2007} is based on the application of an efficient method for computing the Gaussian that best approximates the exact posterior (section~\ref{sec:approximate_posterior}).}
...
%
\en{In our case, in which we want to know how skills change over time, this technique allow us to apply the equation~\ref{eq:posterior_win} iteratively over a sequence of observations.}
\es{En nuestro caso, en el que queremos conocer cómo cambian las habilidades a través del tiempo, está técnica nos permite aplicar la ecuación~\ref{eq:posterior_win} iterativamente sobre una secuencia de observaciones.}
%
% Parrafo
%
...
%
\en{The approach adopted by TrueSkill, Glicko, and BIRT to treat the dynamical process, known as \emph{filtering}, uses the posterior as the prior for the next event.}
\es{El enfoque adoptado por TrueSkill, Glicko y BIRT para tratar el proceso din\'amico, conocido como \emph{filtering}, usa el posterior como prior del siguiente evento.}
%
\en{Then, the posterior calculated by TrueSkill at any given time is,}
\es{Luego, el posterior calculado por TrueSkill en un momento es,}
%
...
\begin{equation}\label{eq:dynamic_factor}
 ...
 \end{equation}
 %
\en{Because the filtering approach does not arise from any probabilistic model, it suffers from a number of problems.}
\es{Debido a que el enfoque de filtrado no se surge de ningún modelo probabilístico, sufre de un serie de problemas.}
% %
\en{Thay weaknesses are related to the fact that the information propagates in only one direction through the system.}
\es{Sus debilidades están relacionadas con el hecho de que la informaci\'on propaga en un solo sentido por el sistema.}
%
\en{The beginning of any sequence of estimates always has high uncertainty.}
%, making them useless
\es{El inicio de toda secuencia de estimaciones siempre tiene alta incertidumbre.}
% , lo que las hace inservibles
\en{But it also suffer from ``temporary'' and ``spatial'' decoupling.}
\es{Pero también sufre de desacoplamientos ``temporales'' y ``espaciales''.}
%
\en{Although the relative differences between contemporary estimates within well-connected communities may be correct, estimates separated in time and between poorly connected communities are often incorrect.}
% preventing comparability over time and between communities.
\es{A pesar de que la diferencia relativa entre estimaciones contempor\'aneas al interior de comunidades bien conectadas puedan ser correctas, las estimaciones separadas en el tiempo y entre comunidades poco conectadas suelen ser incorrectas.}
% impidiendo así la comparabilidad temporal y entre comunidades.

% Parrafo

\en{In order to obtain good initial estimates and ensure temporal and spatial comparability we need a causal model of the temporal process that links all historical activities.}
...
%
\en{It is difficult to imagine the complex structure that these types of models can acquire, any video game will typically require a network of millions of events.}
\es{Es dif\'icil imaginar la compleja estructura que pueden adquerir este tipo de modelos gr\'aficos, cualquier videojuego requerirá tipicamente una red de millones de eventos.}
%
...
\end{figure}
%
\en{By applying the rules of probability to this temporal model, historical information naturally propagate throughout the system, solving the problems of the filtering approach.}
...
\es{En las secciones \ref{sec:exactSolution}, \ref{sec:empate}, \ref{sec:approximate_posterior}, y \ref{sec:iterative_posterior} mostramos respectivamente c\'omo resolver la probabilidad a priori y el posterior exacto de un evento, modificamos el modelo para que incluya empates, explicamos c\'omo aproximar el posterior y damos la soluci\'on general multi-equipos.}
%
FALTA HISTORY
% %
