...
\title{Harnessing Skill estimation using Bayesian Inference: the \proglang{Julia}, \proglang{Python} and \proglang{R} packages}
\Plaintitle{Harnessing Skill estimation using Bayesian Inference: the Julia, Python and R packages}
%\title{All the concepts behind TrueSkill Through Time: the \proglang{Julia}, \proglang{Python} and \proglang{R} packages}
%\Plaintitle{All the concepts behind TrueSkill Through Time: the Julia, Python and R packages}
...
%
\en{Since the first generative models, proposed almost a century ago by~\cite{Thurstone1927} and~\cite{Zermelo1929}, it is assumed that the probability of an observed result, $r$, depends on the performance, $p$, of the agent $i$ and their opponent $j$, expressed as $P(\, r \,|\, p_i, \, p_j \,)$.}
...
%
\en{The model assumes that the agent with the highest performance wins, $r = (p_i > p_j)$, both random variables around their unknown true skill, $\N(p_i\,|\,s_i,\beta^2)$.}
\es{El modelo supone que gana quien haya obtenido mayor rendimiento,  $r = (p_i > p_j)$, ambas variables aleatorias centrado en la verdadera habilidad, $\N(p\,|\,s,\beta^2)$.}
%
\en{Choosing the variance $\beta^2$ only changes the scale, not the estimates: skills separated by a $\beta$ always have a 76\% chance of winning.}
\es{La elección de la varianza $\beta^2$ sólo cambia la escala, no las estimaciones: habilidades separada por un $\beta$ siempre tienen una probabilidad de ganar de 76\%.}
%
\en{Its graphical representation defines a joint probability distribution, from which it is possible to derive, among others, the prediction of the result given the previous estimates, $P(\,r\,|\,s_{i_\text{old}},s_{j_\text{old}}\,)$.}
\es{Su representaci\'on gr\'afica define una distribuci\'on de probabilidad conjunta, a partir de la cual se puede derivar entre otras, la predicción del resultado dadas las estimaciones previas, $P(\,r\,|\,s_{i_\text{old}},s_{j_\text{old}}\,)$.}
...
%
\en{The idea is that the surprise $\Delta$ depends on how accurate are the previous estimates and, therefore, can be used to update them.}
...
%
\en{The likelihood and the evidence are both probabilities of the observed result, so they can be seen as predictions, and since the results are discrete variables, those probabilities are written with capital letters.}
\es{La verosimilitud como la evidencia son ambas probabilidades del resultado observado, por lo que pueden ser vistas como predicciones, y como los resultados son variables discretas, esas probabilidad se escribe con letras mayúsculas.}
%
\en{The only factor that updates our beliefs is the likelihood, because the evidence is the same for all hypotheses.}
\es{El único factor que actualiza nuestras creencias es la verosimilitud, porque la evidencia es la misma para todas las hipótesis.}
%
\en{So, the posterior uncertainty of each hypothesis is proportional to the product of their prior uncertainty and their likelihood.}
\es{Por lo que la incertidumbre a posteriori de cada hip\'otesis es proporcional al producto de su incertidumbre a priori y su verosimilitud.}
...
\es{Por lo tanto, la predicción a priori del resultado observado (o evidencia) es la densidad acumulada, $\Phi$, de todos los valores positivos de la diferencia de rendimientos (Eq.~\ref{eq:evidence}).}
%
...
%
\en{The proportional posterior for the winning case is obtained by multiplying the likelihood by the prior as shown in equation~\ref{eq:posterior_win}.}
\es{El posterior proporcional para el caso ganador se obtiene multiplicando el likelihood por el prior como se muestra en la ecuaci\'on~\ref{eq:posterior_win}.}
%
\en{In section~\ref{sec:exactSolution} we will see in detail how all this equations arises by applying the sum and product rule over the generative model.}
\es{En la secci\'on~\ref{sec:exactSolution} veremos en detalle c\'omo todas estas ecuaciones surge de aplicar las reglas de las suma y el producto sobre el modelo generativo.}
%
...
\es{En otras palabras, la verosimilitud no es más que la predicci\'on a priori del resultado observado suponiendo verdadera la hipótesis de habilidad que estamos evaluando.}
...
\begin{figure}[h!]
    ...
    %
    \en{The proportinal posterior is obtained as the product of the prior (Gaussian distribution) and the likelihood (cumulative Gaussian distribution).}
...
\es{El posterior no es más que la densidad del prior no filtrada por la verosimilitud.}
%    
...
%
\en{The main contribution of the Glicko system~\citep{glikman_gliko_2} was the development of an efficient method to approximate the exact posterior with a Gasussian distribution.}
...
%
\en{However, this offers no guarantee that the selected Gaussian is the one that best approximates it.}
...
%
\en{In our case, in which we need to know how skills change over time, this technique allow us to apply the equation~\ref{eq:posterior_win} iteratively over a sequence of observations.}
\es{En nuestro caso, en el que necesitamos conocer cómo cambian las habilidades a través del tiempo, está técnica nos permite aplicar la ecuación~\ref{eq:posterior_win} iterativamente sobre una secuencia de observaciones.}
%
...
\es{Debido a que el enfoque de filtrado no se surge de ningún modelo probabil\'istico, sufre de un serie de problemas, todos relacionados con el hecho de que la información se propaga en una sola dirección a través del sistema.}
%
\en{The most obvious is that the beginning of any sequence of estimates always has high uncertainty.}
%, making them useless
\es{El más obvio es que el inicio de toda secuencia de estimaciones siempre tiene alta incertidumbre.}
...
%
\en{This kind of graphical models, in any video game, typically involves a complex network between millions of events.}
\es{Este clase de modelos gr\'aficos, en cualquier video juego, implica típicamente una red compleja entre millones de eventos.}
%
\en{For this reason, figure~\ref{fig:smoothing} shows only the closest neighbors of a sequence of estimates.}
\es{Por esta razón, la figura~\ref{fig:smoothing} muestra s\'olo los vecinos m\'as pr\'oximos de una secuencia de estimaciones.}
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
...
%
\en{Now, the prior at each $t$ event depends on all the past and future information.}
\es{Ahora, el prior en cada evento $t$ depende de toda la informaci\'on pasada y futura.}
%
\en{Excluding the dynamic component, $\gamma = 0$, the prior of the agent $i$ at the $t$-eth event is just the product of all their likelihoods, except that of the $t$-eth event.}
\es{Exluyendo el aspecto dinámico, $\gamma = 0$, el prior del agente $i$ en el $t$-ésimo evento es el producto del todas sus verosimilitudes, salvo la del $t$-ésimo evento.}
%
...
%
\en{This mutual dependency between likelihoods, force us to implement an iterative algorithm to solve inference (details at secci\'on~\ref{sec:throguthTime})}
\es{Esta mutua dependencia entre verosimilitudes, nos obliga a implementar un algoritmo iterativo para resolver la inferencia (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
...
\es{En la primera pasada, los priors de cada evento se calculan usando sólo las verosimilitudes pasadas que ya han sido definidas.}
%
...
\es{Las primeras verosimilitudes necesariamente serán incorrectas debido a que los priors habían sido definidos parcialmente.}
%
...
\es{Pero también lo serán las verosimilitudes calculadas en la segunda pasada, debido a que su resutado dependerá parcialmente de las verosimilitudes calculadas en la pasada anterior.}
%
\en{However, this procedure converge with a few linear iterations over the data, scaling to millions of observations in a few minutes.}
\es{Sin embargo, este procedimiento converge con unas pocas iteraciones lineales sobre los datos, escalando a millones de observaciones en pocos minutos.}
