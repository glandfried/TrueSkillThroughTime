...
\en{La clase \texttt{Gaussian} es la base de todos los paquetes.}
%
\en{Se representa mediante sólo dos parámetros, la media y el desvío estandar.}
%
\en{Definimas dos Gaussianas que utilizaremos más adelante.}
%
\begin{lstlisting}[backgroundcolor=\color{all}]
N1 = Gaussian(mu=1., sigma=1.); N2 = Gaussian(1., 2.)  
\end{lstlisting}
%
\en{Esta clase sobreescribe la suma (\texttt{+}), resta (\texttt{-}), producto (\texttt{*}) y división (\texttt{/}) con las principales operaciones requeridas para computar las distribuciones marginales en TrueSkill Throguth Time.}
%
...
\end{equation}
%
\en{A su vez, definimos una constuctor alternativo que representa la siguiente propiedad.}
%
...
\en{Las partidas con dos equipos tienen solución exacta y pueden computarse del siguiente modo en \proglang{Julia}, \proglang{Python} y \proglang{R} respectivamente.}
%
\begin{paracol}{3}
...
\end{paracol}
%
\en{En esta sección vamos mostrar los pasos matemáticos y computacionales para calcular un posterior exacto (\texttt{exact\_posteriors}) en una partida (\texttt{g}) de dos equipos con dos jugadores (2 vs 2).}
%
\en{En presecia de equipos TrueSkill incorpora una variable adicional al modelo que representa el rendimiento del equipo, que se considera es la suma de los rendimientos de sus miembros.}
%
\en{En términos gráficos, la partida 2 vs 2 se representa como} 
%
\begin{figure}[t!]
  \centering
  \scalebox{.9}{
  ...
  \caption{\small Factorización gr\'afica de una partida 2 vs 2.}
