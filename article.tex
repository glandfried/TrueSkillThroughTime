...
\centering \small
    ...
    \es{Modelo generativo en el que las habilidades causan los resultados observables a trav\'es de la diferencia de rendimientos ocultos, $d=p_i-p_j$, ambas variables aleatorias centrado en la verdadera habilidad, $p \sim \N(s,\beta^2)$}
    %
...
%
\en{The model assumes that the one with the highest performance wins, $r = (p_i > p_j)$, which vary between events around the true skill with some constant noise, $\N(p\,|\,s,\beta^2)$.}
\es{El modelo supone que gana quien haya obtenido mayor rendimiento,  $r = (p_i > p_j)$, que var\'ian entre eventos alrededor de la verdadera habilidad con cierto ruido constante, $\N(p\,|\,s,\beta^2)$.}
%
\en{The graphical representation defines a probability distribution.}
\es{Su representaci\'on gr\'afica define una distribuci\'on de probabilidad.}
...
%
\en{The idea is that the surprise $\Delta$ depends on how good the previous estimates are and therefore serves to update them.}
\es{La idea es que la sorpresa $\Delta$ depende de cuan buenas son las estimaciones previas y por lo tanto sirve para actualizarlas.}
%
...
%
\en{The weak point is due to the lack of the notion of uncertainty.}
\es{El punto débil se debe a la falta de la noción de incertidumbre.}
%
...
%
%
\en{From these and the the symmetry property $p(x,y) = p(y,x)$, we immediately obtain the~\ref{eq:bayes_theorem}:}
...
%
\en{The only free variable is the skill hypothesis of agent $i$.}
\es{La única variable libre es la hip\'otesis de habilidad del agente $i$.}
%
\en{The prior quantifies the previous uncertainty, and the posterior reduces it based on the information provided by the data, given the model.}
\es{El prior cuantifica la incertidumbre previa, y el posterior la reduce en base a la información provista por el dato, dado el modelo.}
%
...
%
\en{The evidence does not change when different hypotheses are evaluated.}
\es{La evidencia no cambia su valor cuando se evaluan diferentes hipótesis.}
%
...
\en{As an instance, let's consider a winning case ($p_i > p_j$) using Gaussian priors, $\N(\,s\,|\,\mu, \sigma^2)$.}
\es{A modo de ejemplo, consideremos un caso ganador ($p_i > p_j$) usando priors Gaussianos, $\N(\,s\,|\,\mu, \sigma^2)$.}
%
\en{The difference of perfomances, in these cases, can also be expressed as a Gaussian distribution, centered on the difference of the prior estimate means ($\mu_i -\mu_j$), with a variance that incorporates the uncertainty of both estimates ($\sigma$) and both performances ($\beta$), $\N( d \, | \, \mu_i -\mu_j \, ,\ 2\beta^2 + \sigma_i^2 + \sigma_j^2 \,)$.}
\es{La diferencia de rendimientos, en estos casos, también se puede expresar como una Gaussiana centrada en la diferencia de las medias de las estimaciones a priori ($\mu_i - \mu_j$), con una varianza que incorpora la incertidumbre de ambas estimaciones ($\sigma$) y de ambos rendimientos ($\beta$), $\N(\, d \, | \, \mu_i -\mu_j \, ,\ 2\beta^2 + \sigma_i^2 + \sigma_j^2 \,)$.}
%
\en{Since we observed a winning result, we know from the causal model that the hidden difference of performances was indeed positive.}
\es{Como observamos un resultado ganador, sabemos por el modelo causal que la diferencia de rendimientos oculta fue en efecto positiva.}
%
\en{The prior prediction of the observed result (or evidence) is therefore the cumulative density, $\Phi$, of all positive values of the difference of performances (Eq.~\ref{eq:evidence}).}
\es{La predicción a priori del resultado observado (o evidencia) es entonces la densidad acumulada, $\Phi$, de todos los valores positivos de la diferencia de rendimientos (Eq.~\ref{eq:evidence}).}
%
\en{From now on the role of the model will be left implicit.}
\es{A partir de ahora el rol del modelo se dejrá impl\'icito.}
%
...
\es{La evidencia es una predicción hecha con todas las hipótesis a priori.}
%
\en{The posterior belief distribution for a winning case is shown in equation~\ref{eq:posterior_win}.}
\es{La distribución de creencias a posteriori para el caso ganador se muestra en la ecuaci\'on~\ref{eq:posterior_win}.}
%
\en{In section~\ref{sec:exactSolution} we will see in detail how all this equations arises by applying the sum and product rule over the probabilistic model.}
\es{En la secci\'on~\ref{sec:exactSolution} veremos en detalle c\'omo todas estas ecuaciones surge de aplicar las reglas de las suma y el producto sobre el modelo probabil\'istico.}
%
...
\es{La verosimilitud es la densidad acumulada de los valores positivos de una distribuci\'on Gaussiana centrada en la diferencia entre la hip\'otesis $s_i$ y la estimaci\'on media del oponente $\mu_j$ con una varianza que incluye todas las incertidumbres salvo la de la propia hip\'otesis $s_i$.}
%
