...
%
\en{However, in the example presented in the introduction (figure \ref{fig:smooth_example}) these expressions return much more pessimistic values than the exact solution.}
\es{Sin embargo, en el ejemplo presentado en la introducción (figura \ref{fig:smooth_example}) estas expresiones devuelven valores mucho más pesimistas que la solución exacta.}
%
...
\end{tabular}
\caption{
\en{Three types of a priori predictions evaluated on the example presented in the introduction.}
\es{Evaluación de los tres tipos de predicciones a priori sobre el ejemplo presentado en la introducción.}
%
\en{The $P(\text{D}|\text{M})$ is the exact solution.}
\es{La $P(\text{D}|\text{M})$ es la solución exacta.}
%
\en{This extreme example shows that their approximations do not indirectly use information from the future to artificially improve their predictions.}
\es{En este ejemplo extremo muestra que sus aproximaciones no usan la información del futuro para mejorar artificialmente las predicciones.}
}
\label{tab:evidence}
\end{table}
%
\en{In this extreme case it can be seen that expressions \ref{eq:log_evidence_online} and \ref{eq:log_evidence_online}, that use as prior messages containing converged estimates over all data, are not using information from the future in an indirect way to artificially improve their predictions.}
\es{En este caso extremo se puede ver que las expresiones \ref{eq:log_evidence_online} y \ref{eq:log_evidence_online}, que usan como prior mensajes que contienen estimaciones convergidas sobre todo los datos, no están usando la información del futuro de manera indirecta para artificialmente mejorar sus predicciones.}
%
\en{Both avoid overfitting and penalize sufficiently to avoid selecting unnecessarily complex models.}
\es{Ambas evitan el sobreajuste y penalizan lo suficientemente para evitar seleccionar modelos innecesariamente complejos.}
%
\en{}
\es{De hecho, en este caso, la evidencia conjunta exacta de TrueSkill Through Time coincide con la evidencia conjunta de TrueSkill.}
%
\en{}
