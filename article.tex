...
%
\todo[inline]{Armar una tabla con los par\'ametros default}
...
%
\en{The skills reported by TrueSkill present strong variations between events for each player and also among players for each event.}
...
%
\en{In this example where no one is better than rest, TrueSkill obtains different skill estimates, but they must be the same for all the players, }
...
%
\en{TrueSkill Through Time solves this issue by propagating information throughout the system.}
\es{El modelo TrueSkill Through Time corrige este problema gracias a que la información propaga por todo el sistema.}
%
\en{Our package can also compute the estimates using TrueSkill Through Time by calling \texttt{convergence()} method of the class \texttt{History}.}
...
\end{lstlisting}
%
\en{TrueSkill Through Time computes estimates that are not only the same for all players but also have less uncertainty.}
...
\en{We now analyze a complex example in which a new player joins a large community of already known players.}
...
%
\en{For this purpose, we establish the skill of the target player to change over time following a logistic function. The community is generated ensuring each opponent has similar skill as the target player following her evolution.}
\es{Para eso establecemos una habilidad que cambia en el tiempo siguiendo una función logística, que deberá ser aproximada por el paquete.}
%
...
target = [skill(i, 500, 2, 0.0075) for i in range(1000)]
opponents = normal(target,scale=0.5,size=1000)
\end{lstlisting}
%
\en{We only include the \proglang{Python} version as \proglang{Julia} and \proglang{R} versions can be found in the appendix~\ref{AGREGA-CODE}).}
...
%
\en{The list \texttt{target} has the agent's skills at each moment. The values start at zero grow smoothly until the target player's skill reaches two.}
...
%
\en{The list \texttt{opponents} includes the randomly generated opponents' skills following a normal distribution centered on each of the target's skills and a standard deviation of $0.5$.}
...
results = [[1,0] if normal(target[i]) > normal(opponents[i]) else [0,1] for i in range(1000)]
times = [(i for i in range(1000)]
priors = dict([(str(i), Player(Gaussian(opponents[i], 0.2))) for i in range(1000)])
...
%
\en{To generate the list \texttt{results} we sample the performance following a normal distribution of the skills: the player with the highest performance wins.}
...
\en{The variable \texttt{time} contain a list of integer values ranging from 0 to 999 representing the time for each game.}
\es{La variable \texttt{time} contiene valores enteros de 0 a 999, indicando el período temporal de cada evento.}
%
\en{The class \texttt{History} uses this list to determine if the dynamic uncertainty ($\gamma^2$) should be applied or not. In this example, as all the events have different time, $\gamma^2$ is applied to all of them as we move forward in time.}
...
%
\en{The variable \texttt{priors} is a dictionary which can be used to customize the properties of players.}
...
%
\en{In this example, we assign low uncertainty to the priors of the opponents to simulate we know their skill beforehand.}
...
%
\en{The class \texttt{History} initializes the rest of the players (in this case only the target player) using the default values and a dynamic factor \texttt{gamma=0.015}.}
...
%
\en{Finally, we use the \texttt{convergence()} method to obtain the TrueSkill Through Time estimates and the target's learning curve.}
...
% Parrafo
%
\en{Figure~\ref{fig:logistic} presents the evolution of the estimate of the target player's skill. The solid black line represents the true target's skill, while the dashed black line shows the estimate. The dark grey zone depicts the uncertainty of the skill estimate, while the light grey indicates two times this uncertainty.}
...
\todo[inline]{Aca corresponde discutir qu\'e resultado dio y hacer un cierre diciendo que TTT es bueno para seguir la evolucion de los jugadores}
...
\en{This last example is based on real-life data of all Association of Tennis Professionals (ATP) registered games.}
...
%
\en{We condensed the information in a \texttt{csv} file containing both single and double matches: if the column \texttt{double} has the letter \texttt{t}, the game is a double match.}
...
%
\en{Here we only show the \proglang{Julia} version because it has more performance than (\proglang{Python} and \proglang{R} versions, which can be found in the appendix \ref{AGREGA-CODE}).}
