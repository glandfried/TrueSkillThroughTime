...
\en{Humans develop complex skills through time.}
 ...
 \es{Ninguno de los estimadores habilidad ampliamente utilizados, como TrueSkill, Glicko e Item-Response Theory, modela correctamente el aspecto temporal, lo que impide tener tanto buenas estimaciones iniciales como comparabilidad entre estimaciones separadas en el tiempo y en el espacio.}
 %
 \en{TrueSkill Through corrects the biases of previous models by modeling the entire history of activities using a single Bayesian network.}
 \es{TrueSkill Through Time corrige los sesgos de los modelos anteriores al modelar toda la historia de las actividades mediante única red bayesian.}
 %
 ...
 %
\en{In this paper we offer the first packages for \proglang{Julia}, \proglang{Python}, and \proglang{R}, together with its scientific documentation, supporting the use of reliable learning curve estimates.}
\es{Al ofrecer los primeros paquetes para \proglang{Julia}, \proglang{Python} y \proglang{R}, junto con su documentaci\'on cient\'ifica, apoyando el uso de estimaciones de curvas de aprendizaje confiables.}
...
\es{Conocer c\'omo cambian las habilidades individuales a lo largo del tiempo es esencial para en el sistema educativo y laboral.}
%
...
\es{El campo sigui\'o progresando con los trabajos de \cite{Bradley1952} y~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}, que condujo al gran avance que tuvo lugar cuando~\cite{Elo2008} desarrolló su metodología para la Federaci\'on de Ajedrez de los Estados Unidos (USCF), adoptada hasta el d\'ia de hoy por la Federaci\'on Internacional de Ajedrez (FIDE).}
...
\es{Debido a que el enfoque de filtrado es un procedimiento ad-hoc que no surge de ningún modelo probabilistico, sus estimaciones exhiben una serie problemas  relacionados al hecho de que la información propaga en una sola dirección a través del sistema.}
%
...
\en{The TrueSkill Through Time model~\citep{Dangauthier2007}, being theoretically well-founded, corrects the biases estimation of previous models, making it a fundamental step in the direction of reliable estimates.}
\es{El modelo TrueSkill Through Time~\citep{Dangauthier2007}, al estar teóricamente bien fundado, corrige los sesgos de estimación de los modelos previos, por lo que es un paso fundamental en la dirección de estimaciones confiables.}
%
\en{By having a probabilistic model of the entire history of events, we can perform inference without any ad-hoc procedure by strictly applying the rules of probability, guaranteeing both good initial estimates and the temporal and spatial comparability of the estimates.}
\es{Al tener un modelo probabilísitico de toda la historia de eventos podemos realizar la inferencia sin ningún procedimiento ad-hoc aplicando estrictamente las reglas de la probabilidad, garantizando tanto buenas estimaciones iniciales y como la comparabilidad temporal y espacial de las estimaciones.}
%
...
\es{La conectividad entre eventos se genera por el supuesto básico de que la habilidad de un jugador en un tiempo $t$ depende de su propia habilidad en un tiempo anterior $t-1$.}
%
\en{Then, the temporally adjacent events of the players are linked, generating a network that acquires its structure depending on who participates in each event (similar approaches were developed by \cite{coulom2008-wholeHistoryRating} and \cite{maystre2019-pairwise}).}
\es{Así, los eventos temporalmente adyacentes de los jugadores quedan vinculados, generando un red que adquiere su estructura dependiendo de quienes participan en cada evento (enfoques similares fueron desarrollados por \cite{coulom2008-wholeHistoryRating} y \cite{maystre2019-pairwise}).}
%
...
%
\en{This creates a mutual dependence between likelihoods that forces us to implement an iterative algorithm: in the first round we use only the past likelihoods to calculate the priors of each event, and in each new round (backwards and forwards) we use the last available likelihoods (details in section~\ref{sec:throguthTime}).}
\es{Esto produce una mutua dependencia entre verosimilitudes que nos obliga a implementar un algoritmo iterativo: en la primera vuelta utilizamos sólo las verosimilitudes pasadas para calcular las priors de cada evento, y en cada nueva vuelta (para atrás y para adelante) usamos las últimas verosimilitudes disponibles (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
\en{The procedure converges with a few linear iterations over the data.}
...
%
\en{TrueSkill Through Time recovers the actual differences between hidden skills, unlike TrueSkill which provides biased estimates.}
\es{TrueSkill Through Time recupera las verdaderas diferencias entre habilidades,  a diferencia de TrueSkill que ofrece estimaciones sesgadas.}
%
\en{According to what the data suggests (one win each), the final posteriors are centered on zero, indicating that both players have the same skill.}
\es{De acuerdo a lo que sugieren los datos (una victoria cada uno), el posterior final está centrado en cero, indicando que ambos jugadores tienen misma habilidad.}
%  
...
%
\en{This is a major step towards making learning curve estimates reliable, a necessary prerequisite before they can be used by the scientific community as an empirical basis for decision making.}
\es{Este es un gran paso para hacer que las estimaciones de curvas de aprendizaje sean confiables, un prerequisito necesario antes de que sean usadas por la comunidad científica como base empírica para la toma de decisiones.}
%
...
\en{The implementations made in this work open the possibility of incorporating into the temporal causal model the extensions that were developed over the years for the TrueSkill and Item-Response Theory models.}
\es{Las implementaciones hechas en este trabajo abre la posibilidad de incorporar al modelo causal temporal las extensiones que con los años se fueron desarrollando para los modelos TrueSkill e Item-Response Theory.}
%
...
%
\en{Another example would be to extend the causal model by taking into account that individual skills also depend on socio-cultural factors~\citep{VanDerLinden2016}.}
\es{Otro ejemplo sería extender el modelo causal teniendo en cuenta que las habilidades individuales dependen también de factores socio-cultural~\citep{VanDerLinden2016}.}
%
