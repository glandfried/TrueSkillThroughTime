...
\en{Our exceptional cognitive ability to imitate, combined with human long periods of juvenile dependency and postreproductive life span, allows us to learn things from others and transmit innovations through generations~\citep{Richerson2020}.}
\es{Nuestra extraordinaria capacidad para imitar, combinada con los largos períodos de aprendizaje juvenil y vida posreproductiva, permite a los humanos aprender de los dem\'as y transmitir las innovaci\'on a través de la generaciones~\citep{citar}.}
%
...
\es{Al ser un proceso poblacional, la adaptaci\'on humana también se ve afectada por caracter\'isticas demogr\'aficas, como el tama\~no y estructura de las poblaciones~\citep{Derex2020}.} 
...
% \es{Hacer Conocer c\'omo cambian la habilidades de la personas en el tiempo es crucial en muchas \'areas.}

%

\en{Knowing how individual skills change over time is essential in sports and educational contexts.}
\es{Conocer el cambios en las habilidades individuales a lo largo del tiempo es esencial en los contextos deportivos y educativos.}
%
\en{However they are hidden variable, so in practice the best we can do is estimating them based on its direct observable consequences: the outcome of problem solving and competitions.}
\es{Sin embargo ellas son variables ocultas, por lo que en la pr\'actica lo mejor que podemos hacer es estimarla a partir de sus consecuencias observables directas: el producto de resolución de problemas y competencias.}
%
\en{The naive solution, frequency of positive results, is not an informative measure because it depends also on the difficulty of the challenges.}
\es{La soluci\'on ingenua, frecuencia de resultados positivos, no es un métrica informativa porque depende también de la dificultad de los desaf\'ios.}
%
\en{For this reason, all known successful solution rely on pairwise comparisons.}
\es{Por esta razón, toda solución exitosa conocida se basa en comparaciones por pares.}
%
\en{Since the first generative models, proposed almost a century ago by~\cite{Thurstone1927} and~\cite{zermelo1929-translation}, it is assumed that the probability of an observed result, $r_{ij}$, depends on the difference in performance, $d_{ij}$, between the agent $i$ and target $j$, $P(\, r_{ij} \,|\, d_{ij} \,)$.}
\es{Ya los primeros modelos generativos, propuestos hace casi un siglo por~\cite{Thurstone1927} y~\cite{zermelo1929-translation}, suponen que la probabilidad de un resultado observado, $r$, depende de la diferencia de rendimiento, $d$, entre el agente y su objetivo, $P(\, r_{ij} \,|\, d_{ij} \,)$.}
%
\en{The field continued to progress with the work of~\cite{Bradley1952} and~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}.}
\es{El campo siguió progresando con los trabajos de \cite{Bradley1952} y~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}.}
% 
\en{A breakthrough took place with the methodology developed for the US Chess Federation (USCF) by~\cite{Elo2008}, which is still used by the International Chess Federation (FIDE).}
\es{Un gran avance produjo la metodolog\'ia desarrollada para la Federaci\'on de Ajedrez de los Estados Unidos (USCF) por~\cite{Elo2008}, adoptada hasta el d\'ia de hoy por la Federaci\'on Internacional de Ajedrez (FIDE).}

% Parrafo

\en{The most widely used skill estimators nowadays shared the some variant of the probabilistic model proposed by Elo~\citep{Glickman1999, Herbrich2007, VanDerLinden2016, Fox2010}.}
\es{Los estimadores de habilidades m\'as utilizados en la actualidad comparten alguna variante del modelo probabilístico propuesto por el Elo~\cite{Glickman1999, Herbrich2007, VanDerLinden2016, Fox2010}.}
%
...
\es{Si bien a veces se usen distintas distribuciones \emph{bell-shape} para definir la probabilidad acumulada $P(\, d_{ij} > 0 \,)$, el modelo generativo subyacente es el mismo.}
%
\en{They differ only in the methodology used to solve it.}
\es{Se diferencian sólo en la metodología usada para resolverlo.}
% 
\begin{figure}[h!]
\centering \small
    \tikz{         
    ...
    %
    \en{At each event, agents exhibit a random performance around their unknown true skill, $p \sim \N(s,\beta^2)$.}
    \es{En cada evento, los agentes exhiben una rendimiento aleatorio centrado en la verdadera y desconocida habilidad, $p \sim \N(s,\beta^2)$.}
    %
    \en{The one with the highest performance wins, $r_{ij} = p_i > p_j$.}
    \es{Gana quien haya obtenido mayor rendimiento, $r_{ij} = p_i > p_j$.}
    %
    \en{This graphical representation defines a joint probability distribution.}
    \es{Esta representaci\'on gr\'afica define una distribuci\'on de probabilidad conjunta.}
    %
...
%
%
\en{Figure~\ref{fig:generative_model} provides a graphical representation in which the skill generates observable results.}
...
%
\en{Elo's methodological solution is extremely simple and astute: start with arbitrary estimates and, after seeing new data, update it based on the surprise.}
\es{La solución metodológica de Elo es extremadamente simple y astuta: empezar con estimaciones arbitrarias, y luego de ver nuevos datos, actualizarlas en base a la sorpresa.}
%
\en{Any pair of previous estimates, $s_{i_\text{old}}$ and $s_{j_\text{old}}$, induces a prior prediction that, depending on the observed result, generates a certain surprise.}
\es{Cualquier par de estimaciones previas, $s_{i_\text{old}}$ y $s_{j_\text{old}}$, induce una predicción a priori que, dependiendo del resultado, generan una determinada sorpresa.}
%
\begin{equation} \label{eq:elo_delta}
 \Delta_i = \underbrace{y_{ij}}_{\hfrac{\text{\en{Direction}\es{Dirección}}}{\text{(Sign\es{o})}}}\underbrace{\left(1-P(\,r_{ij}\,|\,s_{i_\text{old}},s_{j_\text{old}}\,)\right)}_{\hfrac{\text{Magnitud}}{\text{(\en{Surprise}\es{Sorpresa})}}}
\end{equation}
%
\en{where $y_{ij}$ is $1$ for winners and $-1$ for losers, and the probability of the observed result given the old estimates can be derived from Fig.~\ref{fig:generative_model} (more details in section~\ref{sec:exactSolution}).}
...
%
\todo{Esteban arreglala un poquito para que sea m\'as simple}
...
\begin{equation}\label{eq:elo_update}
 s_{i_\text{new}} = s_{i_\text{old}} + \Delta_i
\end{equation}
%
...
%
\en{To break the symmetry of equation~\ref{eq:elo_update}, an ad-hoc solution was proposed: reduce the impact of $\Delta$ based on how many times the agent has been seen by the system.}
\es{Para romper la simetr\'ia de la ecuaci\'on~\ref{eq:elo_update}, una soluci\'on ad-hoc fue propuesta: reducir el impacto de $\Delta$ en funci\'on de las veces que el agente ha sido visto por el sistema.}
%
\en{That is the role played by the K-factor used by the FIDE,  $s_{i_\text{new}} = s_{i_\text{old}} + \Delta_i K_i$.}
\es{Ese es rol que desempe\~na el K-factor usado por la FIDE, $s_{i_\text{new}} = s_{i_\text{old}} + \Delta_i K_i$.}

% Cambio de parrafo

%\en{Probability Theory provides us with a consistent framework for quantifying the uncertainty of our beliefs~\citep{bishop2006-PRML}.}
...
%
\en{The use of Probability Theory to handle epistemic uncertainty, known as Bayesian inference, was widely adopted because it has proven to be a extremely useful tool in practice~\citep{bishop2006-PRML}.}
\es{El uso de la Teor\'ia de la Probabilidad para manejar la incertidumbre epistémica, conocida como inferencia bayesiana, se adopt\'o ampliamente porque ha demostrado ser una herramienta extremadamente útil en la pr\'actica~\citep{bishop2006-PRML}.}
%
\en{But it is also a principled choice.}
\es{Pero es también una elección sustentada teóricamente.}
%
\en{The rules of probability are the only ones that guarantee consistent reasoning in the presence of one dimensional epsitemic uncertainty~\citep{Cox1946, VanHorn2003}.}
\es{Las reglas de la probabilidad son las únicas que garantizan un razonamiento consistente en presencia de incertidumbre epsitémica unidimensional~\citep{Cox1946, VanHorn2003}.}
%
...
%
\en{From this rules, together with the symmetry property $p(x,y) = p(y,x)$, we can immediately obtain the~\ref{eq:bayes_theorem},}
\es{De estas reglas, junto con la propiedad de simetr\'ia $p(x,y) = p(y,x)$, podemos obtenemos inmediatamente el~\ref{eq:bayes_theorem},}
%
...
%
\en{Unlike classical statistics that optimizes some ad-hoc cost function to select a single hypothesis, Bayesian inference, by strictly applying the rules of probability, integrates the joint probability distribution to quantify the uncertainty associated with each hypothesis.}
\es{A diferencia de la estadística clásica, que optimiza alguna función de costo ad-hoc para seleccionar una única hipótesis, la inferencia Bayesiana, aplicando estrictamente las reglas de la probabilidad, integra la distribución de probabilidad conjunta para cuantificar la incertidumbre asociada a cada hipótesis.}

% Parrafo

\en{To reduce the epsitemic uncertainty about the hidden skill, using the information provided by the observed result and the described causal model, we need to solve the following equation.}
\es{Para reducir la incertidumbre epistémica sobre la habilidad oculta, usando la información que nos ofrece el resultado observado y el modelo causal descrito, necesitamos resolver la siguiente ecuación.}
%
...
\es{En la secci\'on~\ref{sec:exactSolution} veremos en detalle c\'omo la ecuación~\ref{eq:event_inference} se resuelve en general.}
%
...
\es{Esta fórmula surge de aplicar las reglas de las suma y el producto sobre el modelo probabil\'istico de la figura~\ref{fig:generative_model}.}
%
...
\end{equation}
%
\en{Where the proportional posterior is just the unnormalized posterior, $p(s_i| r_{ij}, s_j) = \frac{p(s_i, r_{ij}|s_j)}{P(r_{ij})}$.}
...
%
\en{The only free variable is the hypothesis $s_i$.}
\es{La única variable libre es la hip\'otesis $s_i$.}
%
\en{The posterior uncertainty of each hypothesis $s_i$ is proportional to the product of their prior uncertainty and thier likelihood.}
\es{La incertidumbre a posteriori de cada hip\'otesis $s_i$ es proporcional al producto de su incertidumbre a priori y su verosimilitud.}
%
\en{Note that the likelihood is just the prior prediction of the observed winning result, made with all the skill opponent's hypotheses, assuming $s_i$ as the true skill of the agent.}
\es{Notar que la verosimilitud no es más que la predicci\'on a priori del resultado observado ganador, realizada con todas las hip\'otesis de habilidad del oponete, suponiendo $s_i$ como la verdadera del agente.}
%
\en{That is what computes the cumulative density of all positive values of a Gaussian distribution centered at the difference between the hypothesis $s_i$ and the opponent's mean estimate $\mu_j$, $\delta(s_i)$, with a variance that includes all uncertainties except the one of $s_i$, $\vartheta_i^2$.}
\es{Esto lo que computa la densidad acumulada de los valores positivos de una distribuci\'on Gaussiana centrada en la diferencia entre la hip\'otesis $s_i$ y la estimaci\'on media del oponente $\mu_j$, $\delta(s_i)$, con una varianza que incluye todas las incertidumbres salvo la de la propia hip\'otesis $s_i$, $\vartheta_i^2$.}
%
...
\en{It is important to stress that the posterior, although similar, is not a Gaussian distribution, preventing us from using the equation~\ref{eq:posterior_win}  iteratively.}
\es{Es importante remarcar que la posterior, aunque se parezca, no es una distribuci\'on Gaussiana, lo que nos impedirá usar la ecuaci\'on~\ref{eq:posterior_win} iterativamente.}
%
...
\es{Por la forma del posterior exacto, una Gaussiana parece una aproximación suficientemente buena, que nos permitiría evitar el costo computacional de la metodolog\'ias de sampleo.}
%
\begin{equation} \label{eq:approx} 
 \widehat{p}(s_i| r_{ij}, s_j) = \underset{q(s_i) = \N(s_i|\mu, \sigma^2)}{\text{ arg min }} \ \ \text{KL}(\, p(s_i| r_{ij}, s_j) \, || \,  q(s_i)\, )
\end{equation}
%
\en{This kind of methods, which minimize the AnKullback-Leibler divergence between the true and the approximate distribution, allows to perform Bayesian inference in situations that otherwise would be impossible.}
\es{Esta clase de métodos, que minizan la divergencia Kullback-Leibler entre la distribución verdadera y aproximada, permiten realizar Inferencia Bayesiana en situaciones que de otra forma no sería posibles.}
%
...
\es{El principal aporte del sistema Glicko~\cite{glikman_gliko_2} fue el desarroll\'o de un método eficiente para aproximar la posterior exacta con una distribuci\'on Gasussiana.}
%
\en{However, this offers no guarantee that the selected Gaussian is the one that best approximates the exact posterior.}
\es{Sin embargo, esta no ofrece garant\'ias de que la Gaussiana seleccionada sea la que mejor aproxima al posterior exacto.}
%
\en{The success of the TrueSkill solution~\citep{Herbrich2007} is based on the application of an efficient method for calculating the Gaussian that best approximates the exact posterior.}
\es{El éxito de la soluci\'on TrueSkill~\citep{Herbrich2007} se basa en la aplicaci\'on de método eficiente para calcular la Gaussiana que mejor aproxima a la posterior exacta.}
%
\en{Methodological details are described in the section~\ref{sec:analytical_posterior}.}
\es{Los detalles metodol\'ogicos se describen en la secci\'on~\ref{sec:analytical_posterior}.}
%
...
%
\en{The approach adopted by TrueSkill, Glicko, and BIRT to treat a sequence of observations, known as \emph{filtering}, uses the posterior as the prior for the next event.}
\es{El enfoque adoptado por TrueSkill, Glicko y BIRT para tratar una secuencia de observaciones, conocido como \emph{filtering}, usan el posterior como prior del siguiente evento.}
%
% \en{Because each prior is now Gaussian, we can reapply the equation~\ref{eq:posterior_win} to calculate the exact posterior and i}
% \es{Gracias a que en cada paso usamos el posterior aproximado como prior podemos aplicar la ecuación~\ref{eq:posterior_win} para calcular el posterior exacto y su \ref{eq:approx}.}
%
\en{For TrueSkill, the posterior calculated at any given time is,}
\es{Para TrueSkill, el posterior calculado en un cada momento es,}
%
...
\es{Donde {\footnotesize $\widehat{\text{Posterior}}_i$} y {\footnotesize $\widehat{\text{Likelihood}}_i$} representan la aproximaciones inducidas por la ecuaci\'on~\ref{eq:approx} en el $i$-ésimo evento.}
%
\en{If we consider the likelihood as a filter of the prior, estimates can be seen as the accumulation of all previous filters.}
\es{Si consideramos la verosimilitud como un filtro del prior, las estimaciones pueden ser vistas como la acumulaci\'on de todos los filtros anterios.}
%
...
\en{The filter approach is very useful, but since its solution does not correspond to any probabilistic model, a set of problems arises.}
\es{El enfoque de filtrado es muy útil, pero como su solución no se  corresponde a ningún modelo probabilístico, surge un conjunto de problemas.}
%
\en{Its weaknesses are due to the fact that information propagates in only one direction througth the system.}
\es{Sus debilidades se deben a que la informaci\'on propaga en un solo sentido por el sistema.}
%
\en{For example, the beginning of any sequence of estimates always has high uncertainty, making them useless.}
\es{Por ejemplo, el inicio de toda secuencia de estimaciones siempre tiene alta incertidumbre, lo que las hace inservibles.}
%
\en{But their estimates suffer others less obvious problems, like ``temporary'' and ``spatial'' decoupling. }
\es{Pero sus estimaciones sufren de otros problemas menos evidentes, como los desacoplamientos ``temporales'' y ``espaciales''.}
%
%Since the results depend only on the difference in skills, and not on their absolute value, i
...
%Dabido a que los resultados dependen sólo de la diferencia de habilidades, y no de su valor absoluto, p
...
\en{To solve the problems of the filter approach, we need a representation of the temporal process: a single causal model that links all historical activities.}
\es{Para resolver los problemas del abordaje de filtro, necesitamos una representaci\'on del proceso temporal: un único modelo causal que vincule todas las actividades hist\'oricas. }
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
  ...
  \es{Representación esquemática de un único modelo causal para toda la historia de eventos.}
  %
  ...
  %
  \en{Its probabilistic solution generates the approach known as \emph{smoothing}, in which historical information flows through the whole system.}
  \es{La solución probabilística da como resultado el abordaje conocido como \emph{smoothing}, en el que la información histórica fluye por todo el sistema.}
  %
...
\end{figure}
%
\en{It is difficult to imagine the complex structure that these types of graphical models can acquire.}
\es{Es dif\'icil imaginar la compleja estructura que pueden adquerir este tipo de modelos gr\'aficos.}
%
\en{Any video game will typically require a network of millions of events.}
\es{Cualquier videojuego requerirá tipicamente una red de millones de eventos.}
%
...
%
\en{By applying the rules of probability to this temporal model, historical information naturally propagate throughout the system, solving the decoupling problems of the filter approach.}
\es{Al aplicar las reglas de la probabilidad sobre este modelo temporal, la informaci\'on hist\'orica propaga naturalmente hacia todo el sistema, resolviendo los problemas de desacoplamiento del enfoque de filtro.}
%
\en{A prior distribution at an event $t$, with $\gamma = 0$, is now the product of all past and future information.}
\es{La distribución a prior en un evento $t$, con $\gamma = 0$, es ahora la multiplicaci\'on de toda la información pasada y futura.}
%
\begin{equation}\label{eq:smooth_prior}
 \text{Prior}_{i_t} = \text{Prior}_{i_0} \underbrace{\prod_{k = 1}^{t-1} \text{\en{Likelihood}\es{Verosimilitud}}_{i_k}}_{\text{\en{Past information}\es{Información pasada}}} \underbrace{\prod_{k = t + 1}^{T} \text{\en{Likelihood}\es{Verosimilitud}}_{i_k}}_{\text{\en{Future information}\es{Información futura}}}
\end{equation}
%
\en{This generates a mutual dependency between likelihoods, forcing us to implement an eficiente iterative algorithm to solve inference (details at secci\'on~\ref{sec:throguthTime})}
%
\es{Esto genera una mutua dependencia entre verosimilitudes, obligándonos a implementar un algoritmo para resolver la inferencia (detalles en la secci\'on~\ref{sec:throguthTime}).}
%
\en{The loopy belief propagation is procedure that converge with a few linear iterations over the data, scaling to millions of observations in a few minutes.}
...
%
\en{At the beginning, the likelihoods that are not yet defined are initialized with a neutral value or simply excluded.}
\es{Al empezar, las verosimilitudes que aun no están definidas se inicializan con un valor neutro o simplemente se las excluyé.}
%
...
\begin{figure}[h!]
  ...
  \caption{
  \en{Convergence of TrueSkill Throguth Time in a history with only two events and two agents}
  \es{Convergencia de TrueSkill Throguth Time en una historia con sólo dos eventos y dos agentes.}
  %
  ...
  \es{La primera (la más clara) se correponde a la solución TrueSkill.}
  %
  \en{The last one (the darkest) corresponds to the TrueSkill Throguth Time solution.}
  \es{La última (la más oscura) se correponde a la solución TrueSkill Throguth Time.}
  }
...
%
\en{At the end of the convergence the posterior is centered on 0 for both players, indicating that they have the same skill.}
\es{Al final de la convergencia el posterior se centra en 0 para ambos jugadores, indicando que tienen misma habilidad.}
%  
...
\es{Impulsado por la creciento en la capacidad de cómputo ocurrida, en las últimas cinco décadas los investigadores han desarrollado miles de algoritmos exitosos de aprendizaje de automático, hechos a medida de conjuntos de problemas específicos.}
%
\en{In contrast, probabilistic inference offers a generally applicable framework~\citep{Bishop2013}.}
\es{Por el contrario, la inferencia probabilística ofrece un marco de aplicación general~\citep{Bishop2013}.}
%
...
\es{Las suposiciones hechas por los algoritmos estándar de aprendizaje automático son generalmente difíciles de recordar y a menudo difíciles de percibir.}
%
\en{Similarly, the algorithms used vary from application to application, forcing us to learn a variety of methods.}
\es{Del mismo modo, los algoritmos empleadas varían de aplicación en aplicación, obligándonos a aprender una gran variedad de métodos.}
%
...
\es{En lugar de tener que transformar el problema para que encaje en algún algoritmo estándar, la inferencia probabilística ofrece los bloques de construcción que permiten resolver cualquier problema específico.}
%
...
\es{Todos los supuestos se hacen explícitos a través de un modelo generativo (o causal), que puede ser especificado intuitivamente de forma gráfica.}
%
\en{Inference, on the other hand, is always solved by applying the rules of probability (the rules of the sum and the product) to the joint probability represented by the model.}
\es{Y toda inferencia se hace aplicando las reglas de la probabilidad, de la suma y del producto, sobre el modelo representado como probabilidad conjunta.}

% Parrafo

\en{In this section we will explain all the details of the probabilistic framework in the context of the TrueSkill Throguth Time problem.}
\es{En esta secci\'on explicaremos todos los detalles del marco probabilísitico en el contexto del problema TrueSkill Throguth Time.}
%
\en{In the section~\ref{sec:sumProductAlgorithm} we will introduce the \emph{sum-product algorithm}, which allows to efficiently compute any marginal probability distributions from a joint probability distributions.}
\es{En la sección~\ref{sec:sumProductAlgorithm} introduciremos el \emph{sum-product algorithm}, que permite computar eficientemente cualquier distribuciones de probabilidad marginal a partir de una distribuciones de probabilidad conjunta.}
%
...
\es{En la sección~\ref{sec:propiedades} enumeramos las propiedades que necesitaremos para derivar las distribuciones marginales de interés.}
%
...
\es{En las secciones \ref{sec:Gasussian}, \ref{sec:rating} y \ref{sec:game} introducimos los detalles de implementación de las clases \texttt{Gaussian}, \texttt{Rating} y \texttt{Game}.}
%
\en{In the sections \ref{sec:exactSolution}, \ref{sec:empate}, \ref{sec:approximate _posterior}, and \ref{sec:iterative_posterior} we show respectively how to solve the prior probability and the exact posterior of an event, we modified the model to include ties, we explain how to approximate the posterior, and we give the general multi-team solution.}
\es{En las secciones \ref{sec:exactSolution}, \ref{sec:empate}, \ref{sec:approximate _posterior}, y \ref{sec:iterative_posterior} mostramos respectivamente cómo resolver la probabilidad a priori y el posterior exacto de un evento, modificamos el modelo para que incluya empates, explicamos cómo aproximar el posterior y damos la solución general multi-equipos.}
%
FALTA HISTORY
% %
...
\es{El \emph{sum-product algorithm}~\citep{Kschischang2001} es un procedimiento general que aprovecha la estructura de la disstribución de probabilidad conjunta que impone el modelo causal para aplicar eficientemente las reglas de la probabilidad, la~\ref{eq:sum_rule} y la~\ref{eq:product_rule}.}
%
\en{Represented as joint probability distributions, the models can be factored into the product of simple conditional probabilities.}
\es{Representados como distribuciones de probabilidad conjunta, los modelos pueden factorizarse en el producto de probabilidades condicionales simples.}
%
...
\es{En la figura~\ref{fig:factor_graph} mostramos gráficamente la factorización.}
%
...
\es{Los \emph{factor graph} son gráfos bipartitos, compuestos por nodos variables $v$ y nodos funciones (o factores) $f$.}
%
...
\es{Los ejes entre variables y funciones representan la relación matemática ``la variable $v$ es argumento de la función $f$''.}
%
\begin{figure}[h!]
\centering \small
    \tikz{         
...
% 	\node[const, above=of nfr] (dfr) {\large $\mathbb{I}(d >0)$}; %
    ...
    %\node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     %
     \en{Los cuadrados negros representan las funciones, los círculos blancos representan las variable, y los ejes entre ellos representan la relaci\'on matem\'atica ``la variable es argumento de la función''.}
     %
     }
...
\es{La estructura codifica la mínima cantidad de pasos que se requieren para calcular cualquier distribución de probabilidad marginal.}
%
...
\es{El \emph{sum-product algorithm} es una forma general de descomponer las reglas de la probabilidad como mensajes que se envían localmente las variables del \emph{factor graph}.}
%
...
\es{Los mensajes que envían los nodos variables codifican una porción de la regla del producto.}
%
...
\es{En pocas palabras, los mensajes que envía una variables $v$ es simplemente la multiplicación de los mensajes que recibió del resto de sus vecinos $h \in n(v)$ salvo $f$.}
%
\en{And the messages sent by the function nodes encode a portion of the sum rule.}
\es{Y los mensajes que envían los nodos funciones codifican una parte de la regla de la suma.}
%
...
\es{Donde $\bm{h} = n(f)\setminus \{v\}$ es el conjunto de todos los vecinos de $f$ salvo $v$, y $f(\bm{h},v)$ represeta la función $f$, evaluada en todos sus argumentos.}
%
...
\es{En pocas palabras, los mensajes que enviado por una función $f$ a una variable vecina $v$ es simplemente la integración sobre $\bm{h}$ del producto de si mismo con todos los mensajes que $f$ recibe del resto de sus vecinos $\bm{h}$ salvo $v$.}
%
\en{The composition of messages generates a partial computation.}
\es{La composición de mensajes va generando un computo parcial.}
%
...
\es{Finalmente, la distribución de probabilidad marginal de una variable $v$ es simplemente la multiplicación de los mensajes que $v$ recibe de sus vecinos.}
%
...
\subsection{\en{Mathematical properties and notation}\es{Propiedades matemáticas y notación}}\label{sec:propiedades}

\en{The efficiency of TrueSkill Throguth Time is due to the fact that margins, whether exact or approximate, are solved analytically.}
\es{La eficiencia de TrueSkill Throguth Time se debe a que las marginales, sean exactas o aproximadas, se resuelven de forma analítica.}
%
...
\es{En esta sección enumeramos las propiedades que usaremos para derivar los mensajes exactos que surgen del \emph{sum-product algorithm}.}
%
...
\es{Sea $\N$ la ditribución de probabilidad Gaussiana, $\Phi$ la acumulada de una distribucón Gaussiana, $\mathbb{I}$ la función indicadora.}
%
...
\es{Debido a que desarrollamos paquetes en tres lenguajes de programación, al mostrar código identificaremos cada sintaxis del siguiente modo.}
%
...
\es{La clase \texttt{Gaussian()} realiza la mayor parte del cómputo en todos los paquetes.}
%
\en{It is represented by two parameters, the mean and the standard deviation.}
\es{Se representa mediante dos parámetros, la media y el desvío estandar.}
%
...
\end{paracol}
%
\en{The class overwrites the operators addition (\texttt{+}), subtraction (\texttt{-}), product (\texttt{*}) and division (\texttt{/}) with the main properties required to compute the marginal distributions in the TrueSkill Throguth Time model.}
\es{La clase sobreescribe los operadores suma (\texttt{+}), resta (\texttt{-}), producto (\texttt{*}) y división (\texttt{/}) con las principales propiedades requeridas para computar las distribuciones marginales en el modelo TrueSkill Throguth Time.}
%
...
\es{La clase \texttt{Rating()} se representa con cuatro atributos: la media (\texttt{mu}) y el desvío (\texttt{sigma}) de la distribución Gaussiana a priori; el devío de los rendimientos (\texttt{beta}); y la volatilidad temporal de las habilidades (\texttt{gamma}).}
%
...
\es{Por cuestión de eficiencia, el prior Gaussiano se guarda precomputado.}
%
...
\es{En la figura~\ref{fig:modelo_trueskill_2vs2} mostramos su factorización gráfica.} 
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
  ...
  \es{Factorización gr\'afica de una partida 2 vs 2.}
  }
...
\es{Todo partida con dos equipos tiene solución exacta.}
%
...
\es{y los likelihoods exactos pueden computarse en \proglang{Julia}, \proglang{Python} y \proglang{R} mediante el método \texttt{exact\_likelihoods()}.}
%
...
\es{En esta sección vamos a mostrar los pasos para calcular la evidencia  y un likelihood exacto de una partida con dos equipos de dos jugadores.}
%
...
\es{Debido a las reglas del algoritmo de suma-producto y a la factorización del modelo, los mensajes de los factores de habilidad $f_{s_i}$ a su variable $s_i$ no son otra cosa más que el prior.}
%
...
\es{Ya hemos definimos la variable \texttt{prior} en el código~\ref{lst:prior}.}
%
\en{The next message, that the variable $s_i$ sends to the performance factor $f_{p_i}$ will also be the prior.}
\es{El siguiente mensaje, que la variable $s_i$ le envía al factor rendimiento $f_{p_i}$ también será el prior.}
%
...
\es{Recordar que los mensajes que envían las variables es tan solo la multiplicación de los mensajes que esa variable recibe del resto de los factores.}
%
\en{Since they are trivial, we will not make these types of messages explicit.}
\es{Por ser de trivial solución, no haremos explícitos este tipo de mensajes.}
%
\en{Let's see then the message that the performance factors $f_{p_i}$ send to their variable $p_i$.}
\es{Veamos entonces el mensaje que envían los factores rendimiento $f_{p_i}$ a su variable $p_i$.}
%
...
\es{En general (demostración en sección anexa~\ref{suma_normales_induccion})}.
%
\begin{equation*}
...
\es{Al suponer conocida la habilidad, debemos remplazar su estimación media $\mu_1$ por la hipótesis $s_1$ en la diferencia esperada $\delta$, y remover su propia incertidumbre de la incertidumbre total $\vartheta^2$.}
%
...
\subsection{\en{Elementary draw model}\es{Modelo básico de empates}} \label{sec:empate}
...
\es{En la figura~\ref{fig:draw_a} se puede ver en términos gráficos las probabilidades de los tres resultados posibles.}
%
\en{The elementary model requires determining the value of the margin.}
\es{Este modelo básico requiere determinar el valor del margen.}
%
\en{The original paper~\citep{Herbrich2007} proposed to use the empirical frequency of ties.}
\es{El artículo original~\citep{Herbrich2007} proponía usar la frecuencia empírica de empates.}
%
...
\es{En la sección~\ref{sec:ttt-d} presentamos la solución Bayesiana al modelo de empates.}
%
...
\es{En cualquier caso, suponiendo que podemos definir esa la ``probabilidad de empate entre equipos con misma habilidad'', es importante tener en cuenta que el margen también depende de la cantidad de jugadores.}
%
...
\es{En la figura~\ref{fig:draw_b} se puede ver que para mantener el área de empates constante, es necesario adaptar el margen dependiendo de la incertidumbre.}
%
%\en{This is because the actual distribution of performance differences depends on how many players are in the game.}
%\es{Esto es así porque la distribuci\'on de diferencias de rendimientos real depende de cuántos jugadores hay en la partida.}
%
\begin{figure}[h!]
\centering
...
\es{Así es que podemos definir una ecuación que vincula el margen con la probabilidades de empate.}
%
\begin{equation}
...
% \es{En cualquier caso, una alternativa es usar la estrategia clásica de optimización: elegir el valor de $\varepsilon$ que maximiza alguna función de costo.}
% %
% \begin{equation*}
% \varepsilon = \underset{\varepsilon}{\text{arg max}} \ p(\text{Data}|\text{Model}_{\varepsilon})
% \end{equation*}
% %
\subsection{\en{Optimal approximation of the exact posterior}\es{Aproximación óptima del posterior exacto}} \label{sec:approximate _posterior}

\en{In the section~\ref{sec:exactSolution} we have seen how to find the exact posterior.}
\es{En la sección~\ref{sec:exactSolution} hemos visto que el posterior exacta, si bien se parece a un Gaussiano, no lo es, .}
%
...
% %
% 
% \en{Dado que la forma exacta se parece a una Gaussiana (figura~\ref{fig:posterior_win}), usar una Gaussiana para representar el posterior no parecería ser tan grave en términos de aproximación y en cambio nos permitiría realizar estimaciones sucesivas.}
% 
% % Parrafo
...
%\es{Los métodos para aproximar una distribución intratable mediante una distribución perteneciente a una familia conocida se conocen en inferencia bayesiana como ``variacionales''.}
%
...
\es{En esta sección mostraremos cómo encontrar la distribución Gaussiana que mejor aproxima al posterior exacto, considerando la posibilidad de empates.}
%
\en{The packages solve it with the following two lines of code.}
\es{Los paquetes lo resuelven con las siguientes dos líneas de código.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{Recuerden que \texttt{teams}, \texttt{results}, y \texttt{p\_draw}, fueron definidas en los códigos \ref{lst:game} y \ref{lst:draw}.}
...
\es{La necesidad de aproximar el posterior ocurre debido a que la distribución de probabilidad de la diferencia es una Gasussian truncada (Eq.~\ref{eq:p_d}).}
%
...
\es{En esta sección veremos como encontrar la mejor aproximación al posterior exacto en casos en los que tenemos más de dos equipos.}
%
...
\es{La asignación de equipos se representa como una partición del conjunto de agentes, $A$, en $k$ subconjuntos disjuntos, $A_i$.}
%
...
\es{Para ese propósito definimos el orden de equipos que surge del resultado observdo, $o := (o_1, \dots, o_k)$, donde $o_1$ indica el equipo ganador, y en general $o_i = e$ indica que el equipo $e$ quedó ubicado en la posición $i$.}
%
\begin{figure}[t!]
  \centering
  \scalebox{.9}{
  ...
  \es{Factorizaci\'on del modelo de multiequipos genérico.}
  %
  \en{Plates indicate replication.}
  \es{Las placas indican replicación.}
  %
  }
...
\es{En la figura~\ref{fig:factorGraph_trueskill} mostramos la factorización del modelo general de TrueSkill.}
%
...
\es{La idea básica es actualizar repetidamente hacia adelante y hacia atrás todos los mensajes en el camino más corto entre dos marginales $p(d_i)$ hasta la convergencia, como se muestra en la figura~\ref{fig:ep_ts}.}
%
\begin{figure}[H]
  \centering
  \scalebox{.9}{
...
\caption{
 ...
 \es{Factorización de una partida con 3 equipos.}
 %
 \en{We only show factors from the teams to the results.}
 \es{Mostramos sólo factores desde los equipos hasta los resultados.}
 %
 \en{The names will be used to explain the iterative procedure known as loopy belief propagation.}
 \es{Los nombres se usarán para explicar el procedimiento iterativo conocido como \emph{loopy belief propagatiion}.}
}
\label{fig:ep_ts}
\end{figure}
%
...
\es{Los mensajes que aún no están definidos se inicializan con una forma neutra, como una distribución gaussiana con varianza infinita.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{Calculamos el los margenes para cada comparación $d_1$.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{El mensahe \texttt{lhood\_d1\_approx} nos permite actualizar la distribución de rendimietos $t_b$.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{Luego computamos los mensajes de la siguiente distirbución de diferencia $d_2$.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{El mensaje \texttt{lhood\_d2\_approx} nos permite actualizar la distribución de rendimiento $t_b$.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{Entonces, repite los códigos \ref{lst:d1}, \ref{lst:tb_lose}, \ref{lst:d2}, \ref{lst:tb_win} hasta alcanzar convergencia.}
%
\en{Once the iteration is finished, we send the last ascending message to the teams on the sides.}
\es{Finalizada la iteración, enviamos los últimos mensajes ascendentes a los equipos de los bordes.}
%
\begin{lstlisting}[backgroundcolor=\color
...
\es{En esta sección vamos a ver cómo se obtiene la mejor aproximación al posterior exacto en una historia de eventos en la que $n$ agentes compiten durante $T$ pasos temporales o \emph{batches} (e.g. día, semana, mes, año).}
% 
...
\es{Grafo de factorización de una historia de eventos alrededor de la habilidad $s$ de un agente $i$ en el paso temporal $t$}
%
\en{The variables $ p_ {i_t} (j) $ represents the performance $ p $ that player $i$ had in their $j$-th game within the time step $ t $.}
\es{Las variables $p_{i_t}(j)$ representa el rendimiento $p$ que ese jugador tuvo en la $j$-ésima partida al interior del paso temporal $t$.}
%
...
\es{En la figura~\ref{fig:history} mostramos representación esquemática del grapho de factorización de un historia de eventos.}
%
...
\es{Y las verosimilituides se computan siguiendo la sección~\ref{sec:iterative_posterior}, usando como prior toda la información salvo la de la partida de interés.}
%
 \begin{equation}
