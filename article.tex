...
composition_g = [ r.double == "t" ? [[r.w1_id, r.w1_id*r.ground, r.w2_id, r.w2_id*r.ground],[r.l1_id, r.l1_id*r.ground, r.l2_id, r.l2_id*r.ground]] : [[r.w1_id, r.w1_id*r.ground],[r.l1_id, r.l1_id*r.ground]] for r in eachrow(data) ]   

h_ground = History(composition=composition_g, times = times, sigma = 1.6, gamma = 0.036)
convergence(h_ground,epsilon=0.01, iterations=10)
\end{lstlisting}
%
\en{In Figure~\ref{fig:atp_ground} we show the skill difference that Nadal and Federer have in each of the three main types of ground.}
\es{En la figura~\ref{fig:atp_ground} mostramos la diferencia de habilidad que Nadal y Federer tiene entre los tres tipos de piso principales.}
%
\begin{figure}[ht!]
    \centering
    ...
    \end{subfigure}
    \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[page={2},width=\linewidth]{figures/atp_ground}
    \caption{Federer}
    \end{subfigure}
    \caption{
...
\end{figure}
%
\en{We can see that Nadal has a big skill difference between grounds, unlike Federer who has very similar skills on all three types of ground.}
\es{Podemos ver que Nadal tiene una diferencia de habilidad grande entre suelos, a diferencia de Federer que tiene habilidades muy similares en los tres tipos de suelo.}
%
\en{The Nadal's skill difference between clay and grass gorunds is two or more $\beta$, which means a 92\% difference in probability of winning compared to itself.}
\es{La diferencia de habilidad entre piso de polvo de ladrillo y pasto de dos o más $\beta$, lo que significa una diferencia de 92\% de probabilidad de ganar si lo comparamos consigo mismo.}
%
...
%
\en{In Federer's case, it seems reasonable to summarize it in a single dimension.}
\es{En el caso de Federer parece razonable resumir la habilidad en una única dimensión.}
%
