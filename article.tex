...
%
\en{This method allow as to efficiently apply equation~\eqref{eq:posterior_win} iteratively over a sequence of observations, which would otherwise be infeasible.}
...
\begin{figure}[H]
    ...
    %
    \en{The dark and light gray areas show onnce and twice the uncertainty of one of the estimates.}
...
%
\en{The information contained in the \texttt{csv} files has both single and double matches: if the column \texttt{double} has the letter \texttt{t}, the game is a double match.}
\es{La información contenida en los archivos \texttt{csv} contiene tanto partidas simples como dobles, indicado por la columna \texttt{double}.}
%
...
\es{Aquí mostramos solo la versión de \proglang{Julia} debido a que es la más eficiente, las versiones de \proglang{Python} y \proglang{R} se pueden entonrar en el anexo \ref{AGREGA-CODIGO}.}
%
...
\es{Al inicializar la clase \texttt{History} modificamos los valores por defecto de \texttt{sigma} y \texttt{gamma}, basados en un procedimiento de optimización realizado previamente (más detalles a continuación).}
%
...
\es{Finalmente, convergemos la historia para obtener las estimaciones de TrueSkill Through Time, indicando explícitamente el criterio de corte: cuando el cambio entre iteraciones sea menor a $0.01$, o cuando se alcanzan las $10$ iteraciones.}
%
...
%
\en{Similarly, the dirac delta function, $\delta(\cdot=\cdot)$, which is used to represent probability distributions of non-random continuous variables, such as the difference of performances given the agents' performances $p(d|\bm{p})$, can also be used to reduce dimensionality of some integrals.}
\es{De la misma forma, la función delta de dirac, $\delta(\cdot=\cdot)$, que se usa para representar distribuciones de probabilidad de variables continuas no aleatoria como la diferencia de rendimiento dados rendimientos $p(d|\bm{p})$, también puede usarse para reducir la dimensionalidad de algunas integrales.}
%
...
\subsection{\en{The team model and the exact solution}\es{El modelo de equipos y la solución exacta}}\label{sec:2vs2}

\en{TrueSkill also allows modeling team games.}
\es{TrueSkill también permite modelar partidas con equipos.}
%
...
\es{Siguiendo la regla~\ref{eq:m_f_v} del \emph{sum-product algorithm} y la factorizaci\'on del modelo presentado en la figura~\ref{fig:modelo_trueskill_2vs2}, los mensajes de los factores de habilidad $f_{s_i}$ a su variable $s_i$ no son otra cosa m\'as que el priors.}
%
...
\end{equation}
%
\en{We have access to these messages through the \texttt{performance()} method of the \texttt{Agent} class.}
\es{Tenemos acceso a estos mensajes mediante el método \texttt{performance()} de la clase \texttt{Agent}.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:performance, caption={\en{Computing the prior performance}\es{Computando el desempeño a priori}}, belowskip=-1.0 \baselineskip, aboveskip=0.0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
p1 = performance(a1)
...
\begin{split}
 m_{f_{t_e} \rightarrow t_e}(t_e) &= \iint \delta(t_e = p_i + p_j) \N(p_i|\mu_i,\beta^2 + \sigma_i^2)\N(p_j|\mu_j,\beta^2 + \sigma_j^2) dp_idp_j  \\ &=  \N(t_e|\mu_i+\mu_j,2\beta^2 + \sigma_i^2 + \sigma_j^2)
\end{split}
\end{equation}
%
\en{Thanks to the property of Gaussians, the integral has an analytical solution being again a Gaussian distribution.}
\es{Gracias a la propiedad de las gaussianas, la integral tiene solución analítica siendo a su vez una distribución Gaussiana.}
%
\en{We have access to this message when we use the operator \texttt{+} of the class \texttt{Gaussian} to sum the agents' performances, \texttt{ta = p1 + p2}.}
\es{Tenemos acceso a este mensaje cuando usamos el operador \texttt{+} de la clase \texttt{Gaussian} para sumar los desempeños de los agentes, \texttt{ta = p1 + p2}.}
%
...
\begin{equation}\label{eq:m_fd_d} \tag{\texttt{d = ta - tb}}
 \begin{split} 
  m_{f_{d_1} \rightarrow d_1}(d_1) & = \iint \delta(d_1 = t_a - t_b) \N(t_a| \mu_a, \sigma_a^2)  \N(t_b| \mu_b, \sigma_b^2)  dt_adt_b \\[0.25cm]
  & = \N\big( d_1 | \underbrace{\mu_a - \mu_b}_{\hfrac{\text{\en{Expected}\es{Differencia}}}{\text{\en{difference}\es{esperada}}}:  \ \psi}, \underbrace{\sigma_a^2 +\sigma_b^2}_{\hfrac{\text{\en{Total}\es{incertidumbre}}}{\text{\en{uncertainty}\es{total}}} : \ \vartheta^2}  \big) = \N(d_1 | \psi, \, \vartheta^2)
 \end{split}
\end{equation}
%
\en{We have access to this message when we use the operator \texttt{-} of the class \texttt{Gaussian} to get de difference of teams' performances, \texttt{d = ta - tb}.}
...
\begin{split}
 m_{f_{r_1} \rightarrow r_1}(r_1) = \int \mathbb{I}(d_1 > 0) \N(d_1 | \psi, \vartheta^2)  dd_1 = 1 - \Phi(0|\psi, \vartheta^2)
\end{split}
\end{equation}


\begin{lstlisting}[backgroundcolor=\color{white},label=lst:difference, caption=\relax, belowskip=-1.0 \baselineskip, aboveskip=0.0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
e = 1.0 - cdf(d, 0.0)
