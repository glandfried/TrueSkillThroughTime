...
%
\en{Suppose we know the true ability of all players.}
...
priors = dict([(str(k), Player(Gaussian(other[k], 0.2))) for k in range(1000)])

h = History(composition, results, times, priors, sigma=6.0, gamma=0.015)
h.convergence()
mu = [tp[1].mu for tp in h.learning_curves()["a"]] 
\end{lstlisting}
%
\en{The variable \texttt{composition} contains all the games in which the agent participates against the 1000 different opponents.}
\es{La variable \texttt{composition} contiene todas las partidas en la que participa el agente contra los 1000 distintos oponentes.}
%
\en{The variable \texttt{results} contains the results randomly generated from the true skill, the player with the highest performance wins.}
\es{La variable \texttt{results} es una lista con los resultados generados aleatoriamente a partir de la verdadera habilidad, el juegador que saca mayor rendimiento gana.}
%
\en{The variable \texttt{time} contains integer values from 0 to 999.}
\es{La variable \texttt{time} contiene valores enteros de 0 a 999.}
%
\en{When passed to the class \texttt{History}, this will indicate the time period in which each of the games was played, and therefore the amount of dynamic uncertainty ($\gamma$) to be added between adjacent games.}
\es{Al pasarselo a la clase \texttt{History}, esto indicará el período temporal en los que se jugó cada una de las partidas, y por lo tanto la cantidad de incertidumbre dinámica ($\gamma$) que se debe agregar entre partidas adyacentes.}
%
\en{The variable \texttt{priors} is a dictionary indexed by the names of the players, which will indicate in this case the custom priors of the opponents (all estimates with low uncertainty)}
\es{La variable \texttt{priors} es un diccionario indexado por los nombres que indicará a la clase Historia los priors personalizados de los oponetes (todas las estimaciones con baja incertidumbre).}
%
\en{The class \texttt{History} is initialized with these four parameters, and dynamic fator \texttt{gamma=0.015} less than the defualt value.}
\es{La clase \texttt{History} la inicializamos con estos parámetros, y un factor dinámico \texttt{gamma=0.015} menor al valor por defecto.}
%
\en{Then we converge the class \texttt{History} to obtain the TrueSkill Through Time estimates to finally obtain the mean of the estimated learning curve.}
\es{Luego convergemos la clase \texttt{History} para obtener las estimaciones de TrueSkill Through Time para finalmente obtener la media de la curva de aprendizaje estimada.}
