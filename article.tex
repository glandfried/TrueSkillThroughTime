...
%
\en{For example, in sports development and education, knowing the skill and how it changes can support avoiding the exposition of agents to imbalanced challenges, maintaining the interest and engagement, and monitoring the learning and improvement process.}
...
%
\en{In practice, since skill is a hidden variable, the best we can do is estimating it from its direct observable consequences.}
...
%
\en{The most widely used information sources in education and sports are the outcomes of problem solving and competition results.}
...
%
\en{The naive solution (i.e. frequency of winning or passing the exam) is not a good estimator because it depends on the difficulty of the challenges.}
...
%
\en{The estimation should rely on pairwise comparisons.}
...
%
\en{The field revitalized $20$ years later by the publications of~\cite{Bradley1952} and~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}.}
\es{El campo revitaliz\'o 20 a\~nos despu\'es gracias a las publicaciones de \cite{Bradley1952} y~\cite{Mosteller1951a,Mosteller1951b,Mosteller1951c}.}
...
% %
\en{The main difference between them is the methodology to solve the shared probabilistic model.}
\es{Las principales diferencias entre ellos son metodol\'ogicas: el procedimiento para resolver el modelo probabil\'istico compartido.}
% Cambio de parrafo.
\begin{figure}[h!]
\centering \small
    \tikz{         
...
%
\en{This problem can be addressed through Markov chain, as~\cite{fox2010-bayesianIRT} does to solve the Bayesian Item-Response Theory (BIRT) variants.}
\es{Este problema puede ser tratado mediante cadenas de Markov, tal como hace~\cite{fox2010-bayesianIRT} para resolver las variantes Bayesianas de Item-Response Theory (BIRT).}
%
\en{While sampling methods allow intractable integrals to be solved, the associated computational complexity imposes severe limits on the applications.}
...
%
\en{Due the shape of the exact posterior, a Gaussian seem good enough approximation and it would also allow us to make successive estimates avoiding the computational cost of the sampling methodologies.}
\es{Por la forma del posterior exacto, una Gaussiana parece una aproximación suficientemente buena y nos permitiría además realizar estimaciones sucesivas evitando el costo computacional de la metodolog\'ias de sampleo.}
%
\begin{equation} \tag{\text{Gaussian approximation}}
 \underset{\widehat{\mu}, \widehat{\sigma}}{\text{ arg min }} \ \ \text{KL}(\, p(s_i| r_{ij}, s_j) \, || \,  \N(s_i|\widehat{\mu}_i, \widehat{\sigma}_i^2)\, )
\end{equation}
%
\en{This kind of methods, which minimize the Kullback-Leibler divergence between the true and the approximate distribution, allows the Bayesian inference to be a growing field in the main conferences of artificial intelligence.}
\es{Esta clase de métodos, que minizan la divergencia Kullback-Leibler entre la distribución verdadera y aproximada, permiten hoy que la inferencia Bayesiana sea un campo creciente en las principales conferencia de inteligencia artificial.}
%
...
%
\es{El enfoque m\'as sencillo trata las secuencia de observaciones como una lista ordenada de modelos causales independientes.}
%
\en{The adopted approach used by TrueSkill, Glicko, and BIRT is treating the sequence of observations (i.e. games) as an ordered list of events and using the information obtained in one event in the next one:}
\begin{equation}\label{eq:filter}
 \text{Posterior}_t \propto \text{Likelihood}_t  \overbrace{\text{Likelihood}_{t-1} \dots \text{Likelihood}_{2} \underbrace{\text{Likelihood}_{1} \text{Prior}_1}_{\ \text{Prior}_2} }^{\text{Prior}_{t}} 
\end{equation}
...
%     \tikz{ %
%       \node[latent] (s1) {} ;
%       \node[const, above=of s1 ] (ns1) {$s_{i_1}$} ;
%       \node[latent, below=of s1] (p1) {};
%       \node[const, right=of p1 ] (np1) {$p_{i_1}$} ;
%       
%       \node[latent, right=of s1] (s2) {} ;
%       \node[const, above=of s2 ] (ns2) {$s_{i_2}$} ;
%       \node[latent, below=of s2] (p2) {};
%       \node[const, right=of p2 ] (np2) {$p_{i_2}$} ;
%       
%       \node[const, right=of s2, xshift=0.5cm] (s3) {\ $\dots$} ;
% 
%       \node[latent, right=of s3, xshift=-0.4cm] (s1n) {} ;
%       \node[const, above=of s1n ] (ns1n) {$s_{i_{n-1}}$} ;
%       \node[latent, below=of s1n] (p1n) {};
%       \node[const, right=of p1n ] (np1n) {$p_{i_{n-1}}$} ;
%       
%       \node[latent, right=of s1n] (sn) {} ;
%       \node[const, above=of sn ] (nsn) {$s_{i_n}$} ;
%       \node[latent, below=of sn] (pn) {};
%       \node[const, right=of pn ] (npn) {$p_{i_n}$} ;
%       
%       \edge {s1} {p1};
%       \edge {s2} {p2};
%       \edge {s1n} {p1n};
...
%   %
%   \en{Each pair $s_{i_t} \rightarrow p_{i_t}$ represents a fragment of the causal model presented in Figure~\ref{fig:generative_model}.}
%   \es{Cada par $s_{i_t} \rightarrow p_{i_t}$ representa un fragmento del modelo causal presentado en el Figura~\ref{fig:generative_model}.}
%   %
