...
% %
\en{Knowing people's abilities is crucial in the area of education and sports.}
\es{Conocer las habilidades de las personas es crucial en el \'area de la educaci\'on y los deportes.}
%
...
%
\en{Since the first generative models, proposed almost a century ago by~\cite{thurstone1927-comparativeJugement} and~\cite{zermelo1929-translation}, it is assumed that the probability of an observed result, $r$, depends on the difference in strength, $d$, between the agent and their target, $P(\, r \,|\, d \,)$.}
\es{Ya los primeros modelos generativos, propuestos hace casi un siglo por~\cite{thurstone1927-comparativeJugement} y~\cite{zermelo1929-translation}, suponen que la probabilidad de un resultado observado, $r$, depende de la diferencia de escala, $d$, entre el agente y su objetivo, $P(\, r \,|\, d \,)$.}
%
...
%
\en{The main difference in respect to their representations is limited to the choice between two very similar cumulative probability distributions to define the likelihood $P(\, r \,|\, d \,)$: the logistic or the Gaussian cumulative function.}
\es{La principal diferencia respecto a sus representación se limita a la elección entre dos distribuciones de probabilidad acumulativas muy similares para definir la verosimilitud $P(\, r \,|\, d \,)$: la función logística o la Gaussiana acumulativa.}
%
...
\begin{equation}\label{eq:inference}
 \underbrace{p(\overbrace{\text{\en{Skills}\es{Habilidades}}}^{\text{\en{Hidden}\es{Oculta}}}|\overbrace{\text{\en{Result}\es{Resultado}}}^{\text{\en{Observed}\es{Observado}}}, \text{\en{Model}\es{Modelo}})}_{\text{Posterior\es{i}}} = \frac{\overbrace{P(\,\text{\en{Result}\es{Resultado}}\,|\,\text{\en{Skills}\es{Habilidades}}\,,\text{\en{Model}\es{Modelo}})}^{\text{\en{Likelihood}\es{Verosimilitud}}}\overbrace{p(\text{\en{Skills}\es{Habilidades}})}^{\text{Prior\es{i}}}}{\underbrace{P(\text{\en{Result}\es{Resultado}}\,|\,\text{\en{Model}\es{Modelo}})}_{\text{Evidenc\en{e}\es{ia}}}}
\end{equation}
%
\en{Where the hypotheses are the hidden skills, the data is given by the observed results, and the model is defined by the figure~\ref{fig:generative_model}.}
\es{Donde las hip\'otesis son las habilidades ocultas, los datos son los resultados y el modelo est\'a definido por la figure~\ref{fig:generative_model}.}
%
\en{The probability distributions that represent our previous beliefs about people's abilities can be updated by the use of the sum and product rules.}
\es{Las distribuciones de probabilidad que representen nuestras creencias previas sobre las habilidades de las personas se pueden actualizar mediante el uso de las reglas de la suma y el producto.}
%
\en{Using a Gaussian prior, let's see the exact belief update for the winning case,}
\es{Usando un prior Gaussiano, veamos la actualizaci\'on de creencias exacta para el caso ganador,}
%
...
% 
\en{A proportional posterior distribution is obtained by the product of the prior and the likelihood.}
\es{Una distribuci\'on proporcional al posterior se obtiene mediante el producto de la distribuci\'on a priori y la verosimilitud.}
%
...
\begin{figure}[h!]
    ...
    %
    \en{The proprotional posterior distribution is obtained as the product of the prior and the likelihood.}
    \es{La distribuci\'on a posteriori proporcional se obtiene como el producto de la distribuci\'on a priori y la verosimilitud.}
    %
...
\end{figure}
%
\en{It is interesting to note how the likelihood works as a filter for the prior beliefs.}
\es{Es interesante notar cómo la verosimilitud funciona como filtro de las creencias a priori.}
%
\en{In the regions of very high ability hypothesis, where a winning result would not generate any surprise, the proportional posterior distribution remains unaltered with respect to the prior.}
\es{En las regiones de hipótesis de muy alta habilidad, donde un resultado ganador no generaría sorpresa alguna, la distribución a posteriori proporcional no sufre alteración respecto del prior.}
%
...
\textbf{Ahora bien, nosotros necesitamos extender esta inferencia a toda una serie de eventos, de modo tal de estimar curvas de aprendizajes completas.}
%
...
\begin{figure}[h!]
  ...
  \caption{The \emph{filter} approach to modelling a sequence of observations treat each graphical model as independent, without links.}
  \label{fig:filter}
\end{figure}
%
En este este abordaje conocido como \emph{filtrado}, las estimaciones a posteriori de un tiempo $t-1$ se usan como prior del tiempo $t$.
%
En nuestro caso, la habilidad de las personas es una variable dinámica que cambia en el tiempo.
%
Por ese motivo, en cada pasos se agrega incertidumbre alrededor del subsequent prior mediante una distribuci\'on Gaussian centrada en el posterior anterior.
%
\begin{equation}\label{eq:dynamic_factor}
 s_i^t \sim \N(s_i^{t-1}, \gamma^2)
 \end{equation}
%
Utilizar como posterior del tiempo $t-1$ la Gaussiana que mejor la aproxima permitiría que el prior del tiempo $t$, definido en la ecuación~\ref{eq:dynamic_factor}, sea también Gaussiano, lo que permitiría aplicar repetidas veces la actualización de creencias definida en la ecuación~\ref{eq:posterior_win}.
% 
...
%
El método, conocido como \emph{expectation propagation}, permite computar de forma eficiente la Gaussiana que minimiza la diveregencia Kullback-Leibler.
%
...
%
No solo superó los resultados obetnidos mediante Elo.
%
...
\textbf{Porque el abordaje de filtro es malo.}
%
...
However, due to its fully probabilistic formulation it is possible to extend Trueskill to perform smoothing on a time series of player skills.
\end{quotation}
%
\begin{figure}[h!]
  \centering
  ...
  \caption{The \emph{smoothing} approach treat the sequential data as a unique Markov chain of latent variables. This graphical structure forms the foundation both for the hidden Markov model and for linear dynamical systems.}
  \label{fig:smoothing}
\end{figure}
...
\begin{quotation}
Although such models are tractable, they are also severely limited. We can obtain a more general framework, while still retaining tractability, by the introduction of latent variables, leading to \emph{state space models}.
%
Here we focus on one important state space models known as \emph{linear dynamical systems} in which the latent variables are Gaussian.
%
In order to motivate the concept of linear dynamical systems, let us consider the following simple problem, which often arises in practical settings.
Suppose we wish to measure the value of an unknown quantity z using a noisy sensor that returns a observation x representing the value of z plus zero-mean Gaussian noise.
our estimate for z by taking lots of measurements and averaging them,
%
Now let’s make the situation more complicated by assuming that we wish to measure a quantity z that is changing over time.
We can take regular measurements of $x$ so that at some point in time we have obtained $x_1 , \dots , x_N$ and we wish to find the corresponding values $z_1 , \dots , z_N$.
If we simply average the measurements, we will introduce a source of error.
%
Intuitively, we could imagine doing a bit better as follows. To estimate the value of $z_N$, we take only the most recent few measurements, say $x_{N −L} , \dots , x_N$ and just average these.
Perhaps we could do even better if we take a weighted average, in which more recent measurements make a greater contribution than less recent ones.
Although this sort of intuitive argument seems plausible, it does not tell us how to form a weighted average, and any sort of hand-crafted weighing is hardly likely to be optimal
%
Fortunately, we can address problems such as this much more systematically by defining a probabilistic model that captures the time evolution and measurement processes and then applying the inference and learning methods developed in earlier chapters.
Here we shall focus on a widely used model known as a linear dynamical system.
