...
%
\todo[inlin]{Esteban arreglala un poquito para que sea m\'as simple}
...
%
\en{In this section we will see how to find the best approximate posterior in cases where we have more than two teams (Figure \ref{fig:factorGraph_trueskill}).}
\es{En esta sección veremos como encontrar el mejor posterior aproximado en casos en los que tenemos más de dos equipos (figura \ref{fig:factorGraph_trueskill}).}
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
  ...
  \caption{
  \en{Factorization of the multi-team model.}
  \es{Factorizaci\'on del modelo de multiequipos.}
  %
  \en{}
  \es{Los equipos son Sean $n$ agentes $\{1,\dots,n\}$ en un evento con $k$ equipos.}
  %
  \en{}
  \es{Los equipos son conjuntos de$A_e$ define con $k$ conjuntos disjuntos no vacíos.}
  %
  
  %
  \en{}
  \es{Notar que si tenemos $|A|$ equipos, solo hacemos $|A|-1$ comparaciones $d_i$.}
  }
...
%
\en{The basic idea is to update repeatedly forward and backward all messages in the shortest path between any two marginals $p(d_i)$ until convergence.}
\es{La idea básica es actualizar repetidamente hacia adelante y hacia atrás todos los mensajes en el camino más corto entre dos marginales $p(d_i)$ hasta la convergencia.}
%
