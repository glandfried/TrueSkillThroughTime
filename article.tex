...
\es{Nuestra extraordinaria capacidad para imitar, combinada con los largos per\'iodos de aprendizaje juvenil y vida posreproductiva, permite a los humanos aprender de los dem\'as y transmitir las innovaci\'on a trav\'es de la generaciones~\citep{citar}.}
%
...
%
\en{Only considering the frequency of positive results as an indicator of the individuals' ability could lead to wrong approximations, mainly because the outcome also depends on the difficulty of the challenge.}
...
\es{Todos los estimadores de habilidad ampliamente utilizados actualmente comparten alguna variante del modelo probabil\'istico propuesto por el Elo~\cite{Glickman1999, Herbrich2007, VanDerLinden2016, Fox2010}.}
%
\begin{figure}[h!]
\centering \small
    \tikz{         
...
\es{El modelo supone que el resultado es consecuencia directa de los rendimientos, que var\'ian entre eventos alrededor de la verdadera habilidad con cierto ruido constante, $\N(p\,|\,s,\beta^2)$.}
%
...
\en{Elo's methodological solution is simple and smart: update both previous estimates, $s_{i_\text{old}}$ and $s_{j_\text{old}}$, based on the surprise (i.e. the complement of the prediction of the observed result).}
...
%
\en{The idea is that the surprise, $\Delta$, is related to how good are the previous estimates and therefore serves to update them.}
...
%
\en{Because new agents start with arbitrary estimates (the same initial value is used for all individuals), they tend to generate high surprise and abruptly modify estimates that had already converged.}
...
%
\en{The weak point of the methodological solution proposed by Elo is the lack of the notion of uncertainty.}
\es{El punto débil de la solución metodológica propuesta por Elo es la falta de una noción de incertidumbre.}
%
\en{To solve this problem, an ad-hoc solution was proposed: reducing the impact of the surprise based on the number of times the agent has participated previously.}
...
%
%
\en{From these rules and jointly with the symmetry property $p(x,y) = p(y,x)$, we can immediately obtain the~\ref{eq:bayes_theorem}:}
...
%
\en{To reduce the uncertainty about the hidden skill using the information provided by the observed result and the described causal model, we need to solve:}
\es{En nuestro caso, para reducir la incertidumbre sobre la habilidad oculta usando la informaci\'on que nos ofrece el resultado observado y el modelo causal descrito:}
%
...
\es{Debido a que el enfoque de filtrado no se surge de ningún modelo probabil\'istico, sufre de un serie de problemas.}
% %
...
%   \es{La soluci\'on probabil\'istica da como resultado el abordaje conocido como \emph{smoothing}, en el que la informaci\'on hist\'orica fluye por todo el sistema.}
%   %
...
%
\en{The expectation propagation defines a procedure that converge with a few linear iterations over the data, scaling to millions of observations in a few minutes.}
...
\es{Impulsado por la creciento en la capacidad de c\'omputo ocurrida, en las últimas cinco d\'ecadas los investigadores han desarrollado miles de algoritmos exitosos de aprendizaje de automático, hechos a medida de conjuntos de problemas espec\'ificos.}
%
...
\es{Las suposiciones hechas por los algoritmos estándar de aprendizaje automático son generalmente dif\'iciles de recordar y a menudo dif\'iciles de percibir.}
%
\en{Similarly, the algorithms used vary from application to application, forcing us to learn a variety of methods.}
\es{Del mismo modo, los algoritmos empleadas var\'ian de aplicaci\'on en aplicaci\'on, obligándonos a aprender una gran variedad de m\'etodos.}
%
...
\es{En lugar de tener que transformar el problema para que encaje en algún algoritmo estándar, la inferencia probabil\'istica ofrece los bloques de construcci\'on que permiten resolver cualquier problema espec\'ifico.}
%
...
\es{Todos los supuestos se hacen expl\'icitos a trav\'es de un modelo generativo (o causal), que puede ser especificado intuitivamente de forma gráfica.}
%
...
\es{En esta secci\'on explicaremos todos los detalles del marco probabil\'isitico en el contexto del problema TrueSkill Throguth Time.}
%
...
\es{La eficiencia de TrueSkill Throguth Time se debe a que las marginales, sean exactas o aproximadas, se resuelven de forma anal\'itica.}
%
...
\es{Se representa mediante dos parámetros, la media y el desv\'io estandar.}
%
...
\es{El siguiente mensaje, que la variable $s_i$ le env\'ia al factor rendimiento $f_{p_i}$ tambi\'en será el prior.}
%
...
%\es{Esto es as\'i porque la distribuci\'on de diferencias de rendimientos real depende de cuántos jugadores hay en la partida.}
%
\begin{figure}[h!]
\centering
