...
\en{Humans develop complex skills because of a special integration of biological, cognitive, and social processes~\citep{Koster2020}.}
...
\en{Our exceptional cognitive ability to imitate, combined with human periods of juvenile dependency and postreproductive life span, allows us to learn things from others and transmit innovations through generations~\citep{Richerson2020}.}
...
%
\en{However, the skill is a hidden variable, so in practice the best we can do is estimating it from its direct observable consequences: the outcome of problem solving and competitions.}
...
\todo[inline]{Poner por ac\'a el concepto de performance como una hidden variable ``alrededor'' del skill cuando se lo presenta a continuaci\'on}
%
...
\en{The most widely used skill estimators nowadays are based on the probabilistic model proposed by Elo~\citep{Glickman1999,Herbrich2007,VanDerLinden2016,Fox2010}.}
...
%
% \en{Although they sometimes choose alternative bell-shape distributions to define the cumulative probability $P(\, d_{ij} > 0 \,)$, .}
...
%
\en{They differ only in the methodology used to solve it, but the underlying generative model is the same.}
\es{Se diferencian s\'olo en la metodología usada para resolverlo, aunque el modelo generativo subyacente es el mismo.}
% 
\begin{figure}[h!]
\centering \small
    \tikz{         
...
% %
\en{However, the performance depends on an undetermined number of unknown factors, which are modeled as a constant noise, $\N(p\,|\,s,\beta^2)$.}
...
%
\en{Then, the observed result is a direct consequence of the difference of players' performance, $d_{ij}=p_i - p_j$.}
...
\es{Pero es tambi\'en una elecci\'on sustentada te\'oricamente.}
%
...
\es{Las reglas de la probabilidad son las únicas que garantizan un razonamiento consistente en presencia de incertidumbre epsit\'emica unidimensional~\citep{Cox1946, VanHorn2003}.}
%
...
\es{El uso inferencial del teorema de Bayes juega un rol central en las t\'ecnicas modernas de aprendizaje estad\'istico.}
%
...
\es{A partir de ahora el rol del modelo se dejará impl\'icito, de acuerdo con convenciones de notaci\'on.}
%
...
\end{equation}
%
\en{This generates a mutual dependency between likelihoods, forcing us to implement an eficiente iterative algorithm to solve inference (details at secci\'on~\ref{sec:throughTime})}
%
\es{Esto genera una mutua dependencia entre verosimilitudes, obligándonos a implementar un algoritmo para resolver la inferencia (detalles en la secci\'on~\ref{sec:throughTime}).}
%
...
\begin{figure}[h!]
  ...
  \es{La última (la más oscura) se correponde a la soluci\'on TrueSkill Through Time.}
  }
...
\es{En esta secci\'on explicaremos todos los detalles del marco probabilísitico en el contexto del problema TrueSkill Through Time.}
%
...
\es{La eficiencia de TrueSkill Through Time se debe a que las marginales, sean exactas o aproximadas, se resuelven de forma analítica.}
%
...
\subsection{\en{Multiples teams}\es{Varios equipos}} \label{sec:iterative_posterior} 
%
\begin{figure}[h!]
  \centering
  \scalebox{.9}{
...
\es{En esta secci\'on veremos como encontrar el mejor posterior aproximado en casos en los que tenemos más de dos equipos.}
%
...
\subsection{History convergence}~\label{sec:throughTime}
...
%cambiar nombre de through time por trueskill
