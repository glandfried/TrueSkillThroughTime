...
%
\en{We know, however, that the ability of players can vary significantly depending on the type of ground.}
\es{Sabemos, sin embargo, que la habilidad de los jugadores puede variar significativamente según el tipo de suelo.}
%
...
%
\en{One option is to keep one skill variable per player that we include in all games, and one skill variable per ground that we add as a teammate depending on the type of game.}
\es{Una opción es mantener una variable de habilidad respectiva al jugador que incluimos en todas las partidas, y una variable de habilidad por suelo que agregameos como compañero de equipo según el tipo de partida.}
%
\begin{lstlisting}[backgroundcolor=\color{julia},label=lst:atp_ground, caption={\en{Modeling multi-dimensional skills in ATP history}\es{Modelando habilidades multi-dimensionales en la historia de la ATP}}, belowskip=0.1cm, aboveskip=0.1cm]
...
h_ground = History(composition=composition_ground, times = times, sigma = 1.0, gamma = 0.01, beta = 0.0, priors = priors)
...
%
\en{The Nadal's skill difference between clay and grass gorunds is more than one $\beta$, which means at least 76\% difference in probability of winning compared to itself.}
\es{La diferencia de habilidad entre piso de polvo de ladrillo y pasto de más de un $\beta$, lo que significa una diferencia de al menos 76\% de probabilidad de ganar si lo comparamos consigo mismo.}
%
\en{In the case of Nadal it seems important to model the skill's multi-dimensionality.}
\es{En el caso de Nadal parece importante modelar la multi-dimensionalidad de la habilidad.}
%
\en{In Djokovic's case, it seems reasonable to summarize it in a single dimension.}
\es{En el caso de Djokovic parece razonable resumir la habilidad en una única dimensión.}
%
\en{Perhaps in tennis it is sufficient to summarize the skill in one dimension.}
\es{Quizás en tenis sea suficiente resumir la habilidad en una dimensión.}
%
\en{In other examples where the multidimensionality of the skills is more relevant, it may be appropriate to separate the estimates into different components.}
\es{En otros ejemplos donde la multidimensionalidad de las habilidades sea más relevante es posible que sea conveniente separar las estimaciones en diferentes componentes.}
%
\en{To assess whether the complexity added by modeling multidimensionality is appropriate in general terms, we can compare the a priori prediction of the models.}
\es{Para evaluar si la complejidad que se agrega al modelar la multidimensional conviene en términos generales, podemos comparar la predicción a priori de los modelos.}
...
\en{In this section we provide the complete mathematical documentation for implementing the TrueSkill Through Time model.}
\es{En esta secci\'on ofrecemos la documentación matemática completa para implementar el modelo TrueSkill Through Time.}
%
\en{The advantage of TrueSkill Through Time over other models lies in the strict application of probability theory: all the assumptions are made explicit through a generative model, and inference is solved by applying the sum and product rules over model.}
\es{La ventaja de TrueSkill Through Time respecto de otros modelos reside en la aplicación estricta la teoría de la probabilidad: todos los supuestos se hacen expl\'icitos a trav\'es de un modelo generativo, y la inferencia se resuelve aplicando las reglas de la suma y el producto sobre ese modelo.}
%
\en{In section~\ref{sec:sumProductAlgorithm} we introduce the \emph{sum-product algorithm}, which allows us to efficiently apply the sum and product rules to compute the marginal distribution, e.g. the posterior and the prior predicition.}
\es{En la secci\'on~\ref{sec:sumProductAlgorithm} introducimos el \emph{sum-product algorithm}, que nos permite aplicar eficientemente las reglas de la suma y el producto para computar las distribuciones marginal, e.g. el posterior y la predicción a priori.}
%
...
%
\en{In sections \ref{sec:2vs2}, \ref{sec:empate}, and \ref{sec:approximate_posterior} we show respectively how to solve the prior prediction and the exact posterior of an event, we introduce the darw model, we explain how to approximate the posterior in events with two teams.}
\es{En las secciones \ref{sec:2vs2}, \ref{sec:empate}, \ref{sec:approximate_posterior}, mostramos respectivamente c\'omo resolver la predicci\'on a priori y el posterior exacto de un evento, incorporamos empate al modelo y explicamos c\'omo aproximar el posterior en partidas con dos equipos.}
%
...
\es{En la sección \ref{sec:iterative_posterior} explicamos la soluci\'on general multi-equipos que requiere la apliación de un algoritmo iterativo.}
%
\en{In section~\ref{sec:throguthTime} we justify the mathematical steps required to solve the full TrueSkill Through Time model.}
\es{En la secci\'on~\ref{sec:throguthTime} justificamos los pasos matemáticos requeridos para resolver el modelo TrueSkill Through Time completo.}
...
%
\en{The need to approximate the posterior occurs because the probability distribution of the difference is a truncated Gaussian (Eq.~\ref{eq:p_d}).}
\es{La necesidad de aproximar el posterior ocurre debido a que la distribuci\'on de probabilidad de la diferencia es una gaussian truncada (Eq.~\ref{eq:p_d}).}
%
...
\subsection{\en{The History class}\es{La clase History}} \label{sec:throguthTime}
