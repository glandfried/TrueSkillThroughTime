...
Knowing people's abilities is crucial in the area of education and sports among others.
Since it is a complex variable, it is generally impossible to measure it directly (aka hidden variable).
In practice, the best we can do is estimate it from its most direct observable consequences.
The most widely used sources of information in education and sports are the results of problem solving and competition between opponents, respectively.
The naive solution, frequency of positive results, is not a good indicator essentially because it also depends on the difficulty of the challenges.
For that reason, all conventional models are underpinned by pairwise comparisons.
Since the first generative models, proposed almost a century ago~\cite{glickman2013-zermelo1928}, it is assumed that the probability of observing a positive result, $r=1$, depends on the \emph{difference in strength} between the agent and their target, $d_{ab} = s_a - s_b$.
% 
\begin{equation}\label{eq:pairwise-comparisions}
p(\,r=1 \,|\, d_{ab}\,)
\end{equation}
% elo1961-uscf
After the seminal works of \cite{thurstone1927-comparativeJugement} and \cite{zermelo1929-translation}, the field was revitalized 25 years later by the publications of \cite{bradley1952-pairedComparisions} and \cite{mosteller1951-remarks}.
The most widely used skill estimators today are those proposed by~\cite{elo1961-uscf},~\cite{glikman_gliko_2}, \cite{herbrich2006-trueskill}, and some of the Item-Response Theory variants compiled by~\cite{vanDerLinden2016-handbookIRT_1} and~\cite{fox2010-bayesianIRT}.
%\cite{elo1978-pastPresent}
Interestingly, they all share the same underlying generative model.
The main difference between them may be the choice of one of two similar cumulative probability distributions, the cumulative Gaussian or the logistic function, to define equation~\ref{eq:pairwise-comparisions}.



La otra diferencia entre es m\'etodo para resolver el modelo probabilísitico.



% 
% %La diferencia m\'as importante entre todas las propuestas son dos: a) la distribuci\'on de probabilidad del resultado; b) m\'etodo para resolver el modelo probabilísitico.
% 
% \
% 
% The use of probabilty to represent uncertainty, however, is not an
% ad-hoc choice.
% Cox~\cite{cox} showed that if numerical values are used to represent degrees of belief, then a simple set of axioms encoding ``common sense'' that leads uniquely to a set of rules for manipulating degrees of belief that are equivalent to the sum and product rules of probability.
% 
% 
...
\centering \small
    ...
    \node[invisible, right=of p2, xshift=4.35cm] (s-dist) {};
     ...
     \end{subfigure}
     %\caption{Modelo causal que vincula resultados observables ($r$) con habilidades ocultas ($s$) a trav\'es de rendimientos aleatorios ocultos ($p$). Los c\'irculos representan variables continuas, los cuadrados variables discretas. Las variables observables se pintan de gris y las variable ocultas se dejan en blanco. Las flechas representan relaciones causales y las funciones definen esas relaci\'on causales. El rendimiento aleatorio est\'a centrado en la verdadera habilidad con ruido $\beta$, $\N(s,\beta^2)$. El jugador que tuvo mayor rendimiento es el ganador.}
\label{modelo}
\end{figure}

% The main idea of the Elo system is to model the probability of an outcome game based on players' skill $s_i$, $s_j$.
...
% It is assumed that the player who exhibits the greater performance is the winner.
...
% Ambas propuestas pueden representarse con el siguiente modelo causal.
% 
% 
% 
% 
