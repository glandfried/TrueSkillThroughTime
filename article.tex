...
\es{Con una distribución de probabilidad que represenste nuestras creenccias a priori sobre las habilidad de las personas, podemos obtener la distribución de creencias a posterori exacta usando tan solo las reglas de la suma y el producto.}
%
\en{}
...
\es{Actualización de creencias exacta para el caso ganador.}
%
\en{The prior is a Gaussian distribution.}
\es{La distribución a priori es una Gaussiana.}
%
\en{The likelihood is a cumulative Gaussian distribution.}
\es{La verosimilitud es la acumulada de una distribución Gaussiana.}
%
\en{The posterior distribution is obtained as the product of the prior and the likelihood.}
\es{La distribución a posteriori se obtiene como el producto de la distribución a priori y la verosimilitud.}
%
\en{Note that the distributions are not on the same scale: the prior integrates 1, while the likelihood goes from 0 to 1.}
\es{Notar que las distribuciones no están en la misma escala: la distribución a priori integra 1, mientras que la verosimilitud va de 0 a 1.}
%
...
\es{En las regiones de habilidad muy alta, donde un resultado ganador observado no generaría ninguna sorpresa, la distribución a posteriori se queda junto al prior.}
%
...
\es{En las regiones de habilidad muy bajas, donde un resultado observado ganador sería muy sorprendente, la distribución a posteriori pierde casi toda su densidad.}
%
...
%\es{La figura~\ref{fig:posterior_win} muestra, en términos gráficos, el procedimiento de actualización que se realiza en la ecuación~\ref{eq:posterior_win}.}
% 
\en{The posterior distribution is obtained as the product of the prior and the likelihood.}
\es{La distribución a posteriori se obtiene como el producto de la distribución a priori y la verosimilitud.}
%
...
\es{Conseguir la Gaussiana que mejor la aproxima nos permitiría extender la inferencia iterativamente en el tiempo a tavés del uso de las posteriors como priors de las siguientes observaciones.}
%
