...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:performance, caption={\en{Computing the indivudal prior performance}\es{Computando el desempeño individual a priori}}, belowskip=-1.0 \baselineskip, aboveskip=0.0 \baselineskip]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
p1 = performance(a1)
...
\tikz{ %        
        ...
        \path[draw, ->, fill=black!50,sloped] (tb) edge[bend left,draw=black!50] node[midway,below,color=black!75] {\scriptsize \texttt{ \ posterior\_lose\_tb}} (fd1);
        ...
        \path[draw, ->, fill=black!50,sloped] (tb) edge[bend right,draw=black!50] node[midway,below,color=black!75] {\scriptsize \texttt{posterior\_win\_tb}} (fd2);
...
\en{We will solve the convergence corresponding to the example shown at code~\ref{lst:multiple_team_game}.}
\es{Vamos a resolver la convergencia correpodientes al ejemplo del código~\ref{lst:multiple_team_game}.}
%
\en{First we define the prior performance of the teams.}
\es{Primero definimos el desempeño a prior de los equipos.}
%
...
\end{lstlisting}
%
\en{Messages that are not yet defined must be initialized with a neutral form, such as a Gaussian distribution with infinite variance.}
...
\end{lstlisting}
%
\en{We compute the margins for each comparison, $d_j$.}
\es{Calculamos el los margenes para cada comparaci\'on $d_j$.}
%
\en{Since there are 3 players in both comparisons, we adjust both margins to that size.}
\es{Como en ambas comparaciones hay 3 jugadores, ajustamos ambos margenes a ese tamaño.}
%
\begin{lstlisting}[backgroundcolor=\color{all},belowskip=0.1cm]
margin = compute_margin(p_draw, sqrt(3)*beta)
\end{lstlisting}
%
\en{Let's start the iterative process by approximating the distributions $d_1$.}
\es{Empezmos el proceso iterativo aproximando la distribuciones $d_1$.}
%
...
\end{lstlisting}
%
\en{The message \texttt{lhood\_d1\_approx} allows us to update the distribution of performance $p(t_b)$.}
\es{El mensahe \texttt{lhood\_d1\_approx} nos permite actualizar la distribuci\'on de rendimietos $p(t_b)$.}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:tb_lose, caption={\en{Updating the messages of $t_b$  with the last approximation of $d_1$}\es{Actualizando la distribución $t_b$ con la última aproximación de $d_1$}},aboveskip=0.0 \baselineskip]
lhood_lose_tb = posterior_win_ta - lhood_d1_approx 
\end{lstlisting}
%
\en{Then we approximate the distribution $d_2$ using the updated message \texttt{lhood\_lose\_tb}.}
\es{Luego aproximamos la distribución $d_2$ usando el mensaje \texttt{lhood\_lose\_tb } actualizado.}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:d2, caption={\en{Approximating de distribution $d_2$ with the last messages of $t_b$}\es{Aproximando la distribución $d_2$ con lo últimos mensajes de $t_b$}},aboveskip=0.0 \baselineskip]
...
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:tb_win, caption={\en{Updating la distribución $t_b$ with the approximation of $d_2$}\es{Actualizando la distribución $t_b$  con la aproximación de $d_2$}},aboveskip=0.0 \baselineskip]
