...
%
\en{We present both TrueSkill and TrueSkill Through Time solutions, and the methods to obtain the posteriors, the learning curves, and the prior prediction of the observed data (i.e. evidence).}
\es{Presentamos la soluciones TrueSkill y TrueSkill Through Time, y los m\'etodos para obtener los posteriors, las curvas de aprendizaje, y la predicci\'on a priori (i.e. evidencia) del dato observado.}
%
...
\en{To solve an event we use the class \texttt{Game()}.}
\es{Para resolver un único evento utilizamos la clase \texttt{Game()}.}
\en{To perform the analysis we first need to define the features of the players.}
\es{Para hacer el análisis necesitamos definir primero las caracterísiticas de los jugadores.}
%
...
%
\en{Before we begin, we offer some comments on the value of the parameters.}
\es{Antes de empezar, ofrecemos algunos comentarios respecto de los valores de los parámetros.}
%
\begin{lstlisting}[backgroundcolor=\color{all},label=lst:parameters, caption={\en{Parameters}\es{Par\'ametros}},aboveskip=0.1cm]
mu = 0.0; sigma = 6.0; beta = 1.0; gamma = 0.03; p_draw = 0.0
\end{lstlisting}  
%
\en{The absolute value of the mean \texttt{mu} is irrelevant because it is the difference in skills what really matters.}
\es{El valor absoluto de la media \texttt{mu} es irrelevante porque es la diferencia de habilidades lo que realmente importa.}
%
\en{The prior's standard deviation \texttt{sigma} must be sufficiently large so as not to leave out possible skill hypotheses.}
\es{El desvío estandar del prior \texttt{sigma} debe ser suficientemente grande para no dejar por fuera posibles hip\'otesis de habilidad.}
%
\en{This value depends fundamentally on the standard deviation of the performances  \texttt{beta}, which works as the scale of the estimates.}
\es{Ese valor depende fundamentalmente del desvío estandar de los rendimientos \texttt{beta}, que funciona como la escala del sistema.}
%
\en{A real skill difference of one $\beta$, $ s_i - s_j = \beta $, is equivalent to 76\% probability of winning whatever the value of \texttt{beta}.}
\es{Una diferencia de habilidad real de un $\beta$, $s_i - s_j = \beta$, es equivale a 76\% de probabilidad de ganar, cualquiera sea el valor de \texttt{beta}.}
%
\en{Being the unit of measurement of the system, we choose \texttt{beta =1.0}.}
\es{Al ser la unidad de medida del sistema, elegimos \texttt{beta=1.0}.}
% \en{The other element to consider when choosing the \texttt{sigma} is the number of ``levels'' that may exist in the game to be analyzed.}
% \es{El otro elemento a considerar para elegir el \texttt{sigma} es la cantidad de ``niveles'' que pueden exisitir en el juego que va a ser analizado.}
%
...
%
\en{And to initialize the draw probability \texttt{p\_draw} of the game, rule of thumb proposes to select the observed frequency of draws.}
\es{Y para inicializar la probabilidad de empate \texttt{p\_draw} del juego, la regla general propone seleccionar la frecuencia observada de empates.}
%
\en{These are the default values for the three packages.}
\es{Estos son los valores por defecto de los tres paquetes.}
%
...
\end{lstlisting}
%
\en{The lists of likelihoods and posteriors are returned keeping the order in which players and teams were loaded during the initilization of the class \texttt{Game()}.}
\es{Las listas de verosimilitudes y posteriors se devuelven manteniendo el orden en el que los jugadores y equipos fueron cargados durante la inicialización de la clase \texttt{Game()}.}

% Parrafo

\en{Let's look at a more complex example in which the same four players participate in a game organized in 3 teams, ending with a single winning team and the losing teams tied with each other.}
\es{Veamos un ejemplo más complejo en el que los mismos cuatro jugadores participan en una partida organizados en 3 equipos, finalizando con un único equipo ganador y los equipos perdedores empatados entre sí.}
%
\en{Unlike the previous example, now we need to specify the result and the draw probability of the game.}
\es{A diferencia del ejemplo anterior, ahora necesitamos especificar el resultado y la probabilidad de empate del juego.}
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={\en{Initialization of teams and games}\es{Inicializaci\'on de los equipos y el juego}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
...
\end{lstlisting}
\todo{Revisar que se aplique gamma después de la partida.}
%
\en{where the team with the highest value beats the rest, and the teams with the same value are tied.}
\es{donde el equipo con el mayor valor le gana al resto, y los equipos con mismo valor empatan.}
%
...
%
\en{As an example, we initialize a history containing three players (\texttt{\small "a", "b", "c"}) and three games.}
...
\en{Upon initialization, the history immediately triggers the computation of TrueSkill estimates.}
\es{Al inicializarse, la historia inmediatamente activa el computo de las estimaciones TrueSkill.}
%
\en{In this example, where all agents beat each other and their skills do not change over time, it would be reasonable for the agent's estimates to be the same after observing the data.}
\es{En este ejemplo, en el que todos los agentes se ganan mutuamente y sus habilidades no cambian en el tiempo, sería razonable que las estimaciones de los agentes sean las mismas luego de observar los datos.}
%
