...
\es{¿Cu\'an difíciles son los desafíos que enfrentan?}
%
...
\es{Con fines de investigaci\'on, las calificaciones pueden ponerse a disposici\'on de los sujetos y del p\'ublico general, estimulando así el inter\'es y la participaci\'on.}
%
...
\es{El uso de la teoría de la probabilidad para manejar la incertidumbre, conocida como inferencia bayesiana, est\'a hoy adopta de manera generalizada en el \'ambito de la inteligencia artificial y el aprendizaje autom\'atico fundamentalmente porque ha mostrado ser un instrumento sumamente útil en la pr\'actica.}
%
...
\es{Cualquier sistema que represente grados de creencia mediante un valor num\'erico, consistente con la l\'ogica deductiva cl\'asica y un conjunto de axiomas que codifican el razonamiento de sentido común,  ser\'a necesariamente isom\'orfico a la teoría de la probabilidad~\citep{cox1946-reasonableExpectation,vanHorn2003-guideCoxTheorem}.}
%
...
\es{Y toda inferencia probabilística, no importa cuan compleja ella sea, puede ser resuelta a trav\'es de dos simples ecuaciones: la~\ref{eq:sum_rule} y la~\ref{eq:product_rule}.}
%
...
\es{El uso inferencial del teorema de bayes juega un rol central en las t\'ecnicas modernas de aprendizaje estadísitico.}
%
\en{This formula allows us to optimally update our beliefs about a set of hypotheses, given a model and the data.}
\es{Esta f\'ormula nos permite actualizar de forma \'optima nuestras creencia sobre ciertas hip\'otesis, dado un modelo y los datos.}
%
...
\es{Definiendo una distribuci\'on de probabilidad que represenste nuestraa creenccias a priori sobre las habilidad de las personas, podemos obtener la distribuci\'on de creencias a posterori exacta usando tan solo las reglas de la suma y el producto (detalles en~\ref{}).}
%
\en{}
...
\es{\begin{itemize}
		\item Inferencia entre un bache temporal (día, semana, mes) depende del orden aleatorio elegido para la actualizaci\'on. De todas formas el resultado deberia ser independiente del orden de los juegos dentro de un bache temporal.
...
\es{Los mensajes que no est\'an todavía definidos, por ejemplo el prior inverso en la primer pasada hacia adelante, es remplazado por una forma neutral como un escalar unidad o una distribuci\'on gausiana con infinita varianza.}
\vspace{0.3cm}
