...
%
\begin{lstlisting}[backgroundcolor=\color{all}]
difference = performance(team_a) - performance(team_b)  
\end{lstlisting}
%
\begin{equation}\label{eq:m_fd_d} \tag{\texttt{difference}}
 \begin{split} 
...
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia}]
delta = differencia.mu
theta = differencia.sigma
evidencia = cdf(delta/theta) 
\end{lstlisting}  
 \switchcolumn
\begin{lstlisting}[backgroundcolor=\color{python}]
delta = differencia.mu
...
La probabilidad de la diferencia de rendimientos de los equipos es una Gasussian truncada (Eq.~\ref{eq:p_d}).
%
En caso de victoria la diferencia de habilidad solo debe ser positiva
%
\begin{equation}\label{eq:p_d}
p(d_1) = \N(d_1|\delta,\vartheta^2) \mathbb{I}(d_1 > 0)
\end{equation}
%
Para obtener la posterior Gaussiana lo que se hace es un buscar la que mejor aproxima a esta Gaussiana truncada, para luego propogar los mensajes.
%
...
\end{equation}
%
Luego, la Gaussiana que mejor aproxima a $p(d_1)$ en el caso de empate
%
\begin{equation}\label{eq:p*_d}
 \widehat{p}(d_1) = \N\Bigg(d1 \,  \bigg| \, \underbrace{ E(d_1 | -\varepsilon < d_1 < \varepsilon ) \,}_{\hfrac{\text{\scriptsize Media}}{\text{\scriptsize aproximada}} \, \text{\small $\widehat{\delta}$}} , \,  \underbrace{V(d_1 |  -\varepsilon < d_1 < \varepsilon ) }_{\hfrac{\text{\scriptsize Varianza}}{\text{\scriptsize aproximada}} \,\text{\small $\widehat{\vartheta}^{\,2}$}} \, \Bigg)
\end{equation}
%
...
%
\begin{equation}\label{eq:p*_d}
 \widehat{p}(d_1) = \N\Bigg(d1 \,  \bigg| \, \underbrace{ E(d_1 | d_1 > -\varepsilon ) \,}_{\hfrac{\text{\scriptsize Media}}{\text{\scriptsize aproximada}} \, \text{\small $\widehat{\delta}$}} , \,  \underbrace{V(d_1 | d_1 > -\varepsilon ) }_{\hfrac{\text{\scriptsize Varianza}}{\text{\scriptsize aproximada}} \,\text{\small $\widehat{\vartheta}^{\,2}$}} \, \Bigg)
\end{equation}
%
...
\subsection{approximate iterative posterior} \label{sec:iterative_posterior}
