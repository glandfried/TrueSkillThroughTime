...
\es{La soluci\'on ingenua, frecuencia de resultados positivos, no es un m\'etrica informativa porque depende tambi\'en de la dificultad de los desaf\'ios.}
%
...
%
\en{Since the first generative models, proposed almost a century ago by~\cite{Thurstone1927} and~\cite{zermelo1929-translation}, it is assumed that the probability of an observed result, $r_{ij}$, depends on the difference in performance, $d_{ij}$, between the agent $i$ and target $j$, which is expressed with the expression $P(\, r_{ij} \,|\, d_{ij} \,)$.}
...
\en{The most widely used skill estimators nowadays are based on variations of the probabilistic model proposed by Elo~\citep{Glickman1999,Herbrich2007,VanDerLinden2016,Fox2010}.}
...
\label{modelo}
\end{figure}
\todo[inline]{En el caption de la figura, no hay ninguna funci\'on, te refer\'is a las flecas?}
...
% %
\en{The performance depends on an undetermined number of unknown factors, which are modeled as a constant noise, $\N(p\,|\,s,\beta^2)$.}
...
\en{Elo's methodological solution is extremely simple and astute: start with arbitrary estimates and, after seeing new data, update it based on the surprise generated by the result (i.e a weak player beats a top one).}
...
%
\en{The prior prediction induced by the previous estimates $s_{i_\text{old}}$ and $s_{j_\text{old}}$, defines the generated surprise:}
...
\end{equation}
%
\en{where $\Delta_i$ is the generated surprise, $y_{ij}$ is $1$ if agent $i$ wins and $-1$ in the other case, and the magnitude of the surprise is related with how probable is the obtained outcome (see Fig.~\ref{fig:generative_model}). 
If the result of the event is the \emph{expected} one, then the generated surprise will be low (more details in section~\ref{sec:exactSolution}). 
On the contrary, when an unexpected outcome is found, then the surprise will be high.}
...
The surprise is used to update the current skill estimations of each player:
\begin{equation}\label{eq:elo_update}
 s_{i_\text{new}} = s_{i_\text{old}} + \Delta_i
\end{equation}
where $s_{i_\text{new}}$ is the updated skill, $s_{i_\text{old}}$ is the previous estimation, and $\Delta_i$ is the generated surprise. 
A similar update rule is followed for the agent $j$ using the surprise $\Delta_i$ with the opposite sign.

\en{An unlikely result generates a great surprise and, thus, a higher update in the player skill. On the contrary, when the expected result is found, the generated surprise is low and the change on the estimated skill is also low.}
...
%
\en{The update rule shown in equation~\ref{eq:elo_update} is symmetric: what one agent loses is obtained by the opponent.}
...
%
\en{As you cannot have an accurate estimation of new players (an arbitrary fixed value is used for all the new players), the first games tend to generate high surprises. 
These games can abruptly modify skills that had already been accurately estimated.}
...
%
\en{To break this symmetry in equation~\ref{eq:elo_update}, an ad-hoc solution was proposed: reducing the impact of the generated surprise $\Delta_i$ based on the number of times the agent has participated previously.}
...
%
\en{This is the role played by the K-factor used by FIDE,  $s_{i_\text{new}} = s_{i_\text{old}} + \Delta_i K_i$.}
...
%
\en{The use of Probability Theory to handle epistemic uncertainty, known as Bayesian inference, was widely adopted because it has proven to be an extremely useful tool in practice~\citep{bishop2006-PRML}.}
\es{El uso de la Teor\'ia de la Probabilidad para manejar la incertidumbre epist\'emica, conocida como inferencia bayesiana, se adopt\'o ampliamente porque ha demostrado ser una herramienta extremadamente útil en la pr\'actica~\citep{bishop2006-PRML}.}
%
\en{The rules of probability are the only ones that guarantee consistent reasoning in the presence of one dimensional epistemic uncertainty~\citep{Cox1946, VanHorn2003}.}
\es{Las reglas de la probabilidad son las únicas que garantizan un razonamiento consistente en presencia de incertidumbre epist\'emica unidimensional~\citep{Cox1946, VanHorn2003}.}
%
...
%
\en{This expression allows us to optimally update our beliefs about the hypotheses, given a model and the data.}
...
\todo[inline]{Falta contar en palabras esta expresi\'on, as\'i tirada no va. Tambi\'en me parece que habr\'ia que dar las nociones operacionales (c\'omo haces para usar esto como introducci\'on}
%
