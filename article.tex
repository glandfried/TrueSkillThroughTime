...
\en{Human adaptation depends on a special integration of biological, cognitive and social processes~\citep{_Koster2020}.}
...
%
\en{Knowing how they change over time is crucial in many areas.}
\es{Conocer cómo cambian en el tiempo es crucial en muchas \'areas.}
%
...
%
\en{At equation~\ref{eq:posterior_win} we can see the belief update for the winning case using Gaussian priors.}
\es{En la ecuación podemos ver la actualizaci\'on de creencias para el caso ganador usando Gaussiano como priors.}
%
...
%
\en{From now on the role of the model is left implicit, according to the notation conventions.}
\es{A partir de ahora el rol del modelo se deja implícito, de acuerdo con las convenciones de notación.}
%
...
%
\en{Therefore, the likelihood is all the mass up to the value 0 of a Gaussian distribution centered on the difference between the skill hypothesis and the opponent's mean estimate, ($\delta(s_i) = s_i - \mu_j$), with a variance that includes all uncertainties except the one of $s_i$, ($\vartheta_i^2$).}
\es{Por lo tanto, la verosimilitud es toda la masa hasta el 0 de una distribución Gaussiana centrada en la diferencia entre la hipótesis de habilidad $s_i$ y la estimación media del oponente $\mu_j$, ($\delta(s_i)$), con una varianza que incluye todas las incertidumbres salvo la de la propia hipótesis $s_i$, ($\vartheta_i^2$).}
%
\en{This is just the a priori prediction of the observed result, made using all the skill hypotheses of the opponent $s_j$, evaluated in the interest hypothesis $s_i$.}
\es{Esto no es más que la predicción a priori del resultado observado, realizada usando todas las hipótesis de habilidad del oponete $s_j$, evaluada en la hipótesis de interés $s_i$.}
%
\en{}
\es{La sorpresa que no hubiera generado el resultado observado es lo que funciona como filtro del prior.}
%
\en{If we had assumed a priori that the winning agent was much more skilled than his opponent, then the prediction of the winning result would have been close to 1.}
\es{Si hubieramos supuesto a priori que el agente ganador era mucho más habil que su oponente, entonces la predicción del resultado ganador hubiera sido cercana a 1.}
%
\en{}
\es{Y viceversa, si hubieramos supuesto a priori que la persona que al final resultó ganadora era muy poco habil, entonces no hubiésemos llevado una gran sorpresa porque l resultado observado lo hubiramos predicho con probabilidad cercana a 0.}
%
\en{}
\es{La proporción de masa que se filtra para cada hipótesis $s_i$, es equivalente al tamaño de la sorpresa, definida como el complemento de la verosimilitud, $(1-\text{verosimilitud})$}
%
\en{}
\es{El posterior recibe la masa del prior que no fue filtrada por la verosimilitud.}
%
