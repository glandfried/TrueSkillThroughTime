...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:game, caption={\en{Initialization of teams and games}\es{Inicializaci\'on de los equipos y el juego}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
...
\end{lstlisting}
%
\en{The likelihoods and posteriors are returned keeping the order in which players and teams were loaded during the initilization of the class \texttt{Game()}.}
...
\en{Assume now that the same four players participate in a game organized in 3 different teams, two of them draw and one wins.}
\es{Suponga ahora que esos mismo cuatro jugadores participan en una partida organizados en 3 equipos diferentes, dos de ellos empatan y uno gana.}
%
...
\en{The class \texttt{History()} is used to compute the posteriors and evidence of a history of events.}
\es{La clase \texttt{History()} se usa para computar los posteriors y la evidencia de una historia de eventos.}
%
...
%
\begin{lstlisting}[backgroundcolor=\color{white},label=lst:history, caption={\en{History initialization}\es{Inicializaci\'on de la historia}}, belowskip=-1.0 \baselineskip, aboveskip=0.1cm]
\end{lstlisting}
\begin{paracol}{3}
\begin{lstlisting}[backgroundcolor=\color{julia},belowskip=-0.77 \baselineskip]
c1 = [["a"],["b"]]
...
\end{lstlisting}
%
\en{where the variable \texttt{c1} indicates the composition of the first game using the names (i.e. identifiers) of the agents, the variable \texttt{composition} is a list containing the three game compositions, and the parameter \texttt{gamma} when set at zero indicate that agent's skill does not change between games.}
\es{donde la variable \texttt{c1} indica la composici\'on de la primer partida usando los nombres (i.e. identificadores) de los agentes, la variable \texttt{composition} es una lista que contiene la composición de la tres partidas y el par\'ametro \texttt{gamma} inicializado en cero indica que la habilidad de los agetes no cambia entre partidas.}
%
\en{Again the results are implicitly defined by the order in which compositions were initialized: the teams appearing first in the list beat those appearing later.}
\es{Otra vez el resultado de la partida queda definido implícitamente por el orden en el que las composiciones fueron inicializadas: los equipos que aparecen primero en la lista (menor índice) le ganan a los que aparecen después (mayor índice)).}
%
\en{Other default values are the mean (\texttt{mu = 0.}) and standard deviation (\texttt{sigma = 6.}) of the priors, the standard deviation of the performances (\texttt{beta = 1.}), and the draw probability (\texttt{p\_draw = 0.}).}
\es{Otros valores por defecto son la media (\texttt{mu = 0.}) y el desvio estándar (\texttt{sigma = 6.}) de los priors, el desvio estándar de los rendimientos (\texttt{beta = 1.}), y la probabilidad de empate (\texttt{p_draw = 0.}).}

% Parrafo

\en{History initilization triggers the computation of TrueSkill estimates.}
\es{La inicializaci\'on de la historia activa el computo de las estimaciones TrueSkill.}
%
\en{In this example, where agents beat each other and their skills do not change over time, it would be reasonable for the agent's estimates to be the same after observing the data.}
\es{En este ejemplo, en el que los agentes se ganan mutuamente y sus habilidades no cambian en el tiempo, sería razonable que las estimaciones de los agentes sean las mismas luego de observar los datos.}
%
\en{To access them we can use the method \texttt{learning curves()}, which returns a dictionary that is indexed by the names of the agents.}
\es{Para acceder acceder a ellas podemos utilizar el m\'etodo \texttt{learning\_curves()}, que devuelve un diccionario indexado por los nombres de los agentes.}
%
...
\end{lstlisting}
%
\en{Individual learning curves are lists of tuples: the first element indicates the time, and the second the estimate corresponding to that time.}
\es{Las curvas de aprendizaje individuales son listas de tuplas: el primer elemento indica el tiempo, y el segundo la estimaci\'on correpondiente a ese tiempo.}
%
...
\subsection{\en{Parameters}\es{Par\'ametros}}

\en{Before solving a history of events it is necessary to review the set of hyperparameters of the class \texttt{History}: the mean \texttt{mu} and standard deviation \texttt{sigma} of the priors; the standard deviation of the performances \texttt{beta}; the dynamic factor \texttt{gamma}; the draw probability \texttt{p\_draw}; and the convergence conditions \texttt{epsilon} and \texttt{iterations}.}
\es{Antes de resolver una historia de eventos es necesario revisar el conjunto de hiperparámetros de la clase (\texttt{History()}): la media \texttt{mu} y el desvío estandar \texttt{sigma} de los priors; el desvío estandar de los desempe\~nos \texttt{beta}; el factor dinámico \texttt{gamma}; la probabilidad de empate \texttt{p\_draw}; y las condiciones de convergencia \texttt{epsilon} e \texttt{itertions}.}
%
...
\end{lstlisting}
\begin{lstlisting}[backgroundcolor=\color{all}]
MU = 0.; SIGMA = 6.; BETA = 1.; GAMMA = .05; P_DRAW = 0.; EPSILON = 1e-6; ITERATIONS = 40
\end{lstlisting}
%
\en{The values used in the code \ref{lst:environment} are the default values of the class \texttt{History()}.}
\es{Los valores usados en el c\'odigo \ref{lst:environment} son los valores por defecto de la clase \texttt{History()}.}
%
