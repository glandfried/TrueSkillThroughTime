...
\title{Skill estimation using TrueSkill Through Time: \\ the \proglang{Julia}, \proglang{Python} and \proglang{R} packages (Draft)}
\Plaintitle{Skill estimation using TrueSkill Through Time: the Julia, Python and R packages (Draft)}
...
\Abstract{
\en{Humans develop and acquire complex skills through time. }%
\es{Los humanos desarrollamos y adquirimos habilidades complejas a través del tiempo. }%
%
\en{Knowing how individual abilities change is essential in a wide range of activities. }%
\es{Saber cómo cambian las capacidades individuales es esencial en diversas actividades. }%
%
\en{However, skill cannot be directly measured and we have to estimate it based on problem-solving and competition outcomes. }%
\es{Sin embargo, las habilidades no pueden medirse directamente y deben estimarse a partir de la resoluci\'on de problemas y competencias. }%
%
\en{Without properly considering the temporal aspect of the learning processes, we cannot obtain reliable initial estimates and ensure comparability between distant estimates. }%
\es{Sin considerar adecuadamente el aspecto temporal del proceso de aprendizaje, no podemos obtener estimaciones iniciales fiables ni garantizar la comparabilidad entre estimaciones distantes. }%
%
\en{TrueSkill Through Time fulfills these objectives by modeling the entire history of activities using a single Bayesian network. }%
\es{TrueSkill Through Time satisface estos objetivos modelando toda la historia de actividades mediante una única red bayesiana. }%
%
\en{In this paper, we present TrueSkill Through Time's first implementation in \proglang{Julia}, \proglang{Python}, and \proglang{R}, together with the complete technical documentation. }%
\es{En este artículo, ofrecemos los primeros paquetes de TrueSkill Through Time para \proglang{Julia}, \proglang{Python} y \proglang{R}, junto con su documentaci\'on técnica completa. }%
%
\en{We illustrate how to use them with typical cases and show the actual learning curves of historical players of the Association of Tennis Professionals (ATP), supporting the adoption of a reliable learning estimator. }%
\es{Ilustramos cómo usarlos en casos típicos, y mostramos las curvas de aprendizaje reales de los jugadores históricos de la Asociación de Tenistas Profesionales (ATP), apoyando el uso de un estimador de aprendizaje confiable. }%
%
...
\es{Hay una buena concordancia entre ambas metodologías respecto de la detección de quién es el mejor jugador en cada momento. }%
%
\en{The ATP's ranking take time to identify the best player because the points are dropped a year after being awarded. }%
\es{La clasificación de la ATP tarda en identificar al mejor jugador debido que su método retira los puntos un año después de ser otorgados. }%
\en{Perhaps more interesting are the differences with respect to the historical ranking shown in Table~\ref{tab:atp_weeks}. }%
\es{Quizás más interesante sean las diferencia respecto del ranking histórico que se muestra en la tabla~\ref{tab:atp_weeks}. }%
%\en{Notwithstanding, there are some differences between players' skills and the ATP's ranking, especially concerning the historical ranking shown in Table~\ref{tab:atp_weeks}. }%
%
\en{For example, Lleyton Hewitt's position in the historical ranking is the product of a window of opportunity opened around the year 2000, since his ability is relatively low in historical terms. }%
...
\end{lstlisting}
%
\en{In this example we keep the same prior as before for all the generic players, but this time we define them using the variable \texttt{priors}. }%
...
%
\en{Just as in the previous code (\ref{lst:atp}), we create the teams depending on whether the game is double or single, but now adding the specific skills of each player as their teammate (we use the operator \texttt{*} to concatenate strings). }%
...
%
\en{As the specific skills are not defined in the dictionary \texttt{prior}, they will be initialized with the default values indicated in the class \texttt{History}. }%
...
%
\en{We choose a \texttt{sigma} that we consider sufficiently large and a dynamic factor \texttt{gamma} that represents \SI{1}{\percent} of the a priori uncertainty. }%
...
%
\en{To assess whether the complexity added by modeling multidimensionality is appropriate in general terms, we can compare the joint prior prediction of the models, calling the method \texttt{log\_evidence()} of the class \texttt{History}. }%
...
%
\en{In tennis it is sufficient to summarize the skills in a single dimension, since the a prior prediction is maximized when the parameters of the ground factors, $\sigma$ and $\gamma$, vanish. }%
...
\end{lstlisting}
%
\en{Where the agents \texttt{a} were defined in the code~ \ref{lst:player}. }%
\es{Los agentes \texttt{a} fueron definidos en el código~\ref{lst:player}. }%
%
...
\end{lstlisting}
%
\en{In the first line we initialize the message that the variable $t_b$ sends to the factor node $f_{d_2}$}
...
%
\en{When we specify the time of the events, as we did in the code \ref{lst:atp}, the adjacency depends on those times and the dynamic uncertainty depends on the elapsed time between two adjacent variables $s_t$ and $s_{t-1}$. }%
...
%
\en{In large database, the joint probability of the data (equation~\eqref{eq:evidencia_conjunta}) is always close to $0$ because it is the product of probabilities ranging from $0$ to $1$. }%
...
%
\en{The \texttt{log\_evidence()} method of the \texttt{History} class returns by default the product of all these elements in logarithmic scale. }%
...
%
\en{To compute the a priori prediction using only the past information, it is necessary to send only the \emph{forwardPrior} as a message to the events. }%
...
%
\en{The variable \texttt{online} of the method \texttt{log\_evidence()}, when assigned a true value, returns in logarithmic scale the product of the predictions made with past information. }%
...
%
\en{Someone might suspect that the expressions \ref{eq:log_evidence_default} and \ref{eq:log_evidence_online} are unreliable because they might indirectly use information from the future to correctly predict the result of events. }%
...
%
\en{Conversely, in the example presented in the introduction (figure \ref{fig:smooth_example}) these expressions return more pessimistic values than the exact solution. }%
...
\end{table}
%
\en{In the table \ref{tab:evidence} we can see that the exact solution $P(\text{D}|\text{M})$ uses as prior of the events messages containing completely ignorant estimates of the future: the likelihood$_1^1$ is obtained from the first event in the first iteration of the convergence, before the information from the second event has been incorporated. }%
...
%
\en{In this extreme case the exact joint evidence of TrueSkill Through Time and TrueSkill are the same, so there is no preference for any of the models. }%
...
%
\en{In this extreme case, if we use some of them to approximate the a priori predictions we would believe that the TrueSkill model is better, even though TrueSkill Through Time recovers the correct posteriors. }%
...
%
\en{If we extend the example, adding one game at a time, alternating the wins between the two players, the situation is quickly reversed: both the exact solution and its approximations provide better prior predictions than TrueSkill. }%
...
%
\en{In Figure \ref{fig:log_evidence_behavior} we can see the prediction difference between the exact solution with respect to its approximations and TrueSkill. }%
...
\begin{figure}[ht!]
    \centering
    \includegraphics[page={1},width=.7\linewidth]{figures/prior_predictions}
    \caption{
    \en{Prediction difference between the exact solution $ P (D | M) $ with respect to TrueSkill and the approximate solutions, {\small$\widehat{\text{LOOCV}}$} and $ \widehat{P}(D | M) $, as we add more events to the example presented in the introduction (Figure \ref{fig:smooth_example}). }%
    \es{Diferencia de predicción entre la solución exacta $P(D|M)$ respecto de TrueSkill y las solcuiones aproximadas, {\small$\widehat{\text{LOOCV}}$} y $\widehat{P}(D|M)$, a medida que agregamos más eventos al ejemplo presentado en la introducción (figura \ref{fig:smooth_example}). }%
    }
    \label{fig:log_evidence_behavior}
\end{figure}
%
\en{It is interesting to note that the exact solution is always greater than TrueSkill and $\widehat{P}(D|M)$, and even though the solution {\small$\widehat{\text{LOOCV}}$} slightly exceeds it in a moment it is at the same time the one that best approximates it. }%
...
%
\en{This example illustrate that the \ref{eq:log_evidence_default} and \ref{eq:log_evidence_online} expressions do not use information about the future indirectly to artificially improve their predictions. }%
...
\section{Summary and discussion} \label{sec:summary}
 ...
 \en{Estimating how individual skills evolve through time is essential in the educational system or in the labor market. }%
 ...
 %
 \en{The models commonly used in industry and academia (TrueSkill, Glicko, Item-Response Theory) are based on an ad-hoc solution that prevents them both from having good initial estimates and from guaranteeing comparability between estimates separated in time. }%
 ...
 %
 \en{Although this improvement was published more than a decade ago, it had not been implemented until now in the programming languages noted for having the largest scientific software development communities. }%
...
\en{The advantage of TrueSkill Through Time lies in its temporal causal model, that links all historical activities in the same Bayesian network, allowing the information to propagate correctly throughout the system, which guarantees both good initial estimates and the temporal and spatial comparability of the estimates. }%
...
%
\en{Our \proglang{Python} package solves individual events 10 times faster than the original \pkg{trueskill}~$0.4.5$ \citep{lee-trueskill} package. }%
\es{Nuestro paquete de \proglang{Python} resuelve los eventos individuales 10 veces más rápido que el paquete original \pkg{trueskill}~$0.4.5$ \citep{lee-trueskill}. }%
%
\en{In turn, our \proglang{Julia} package solves event sequences $10$ times faster than our \proglang{Python} package. }%
\es{A su vez, nuestro paquete de \proglang{Julia} resuelve las secuencias de eventos $10$ veces más rápido  que nuestro paquete de \proglang{Python}. }%
%
...
%
\en{Bias correction and computational efficiency are critical for estimates to be used in sensitive areas of industry and academia. }%
...
%
\en{The fact that it has not been implemented before seems to be a consequence of the scarcity of previously available documentation. }%
...
\en{The implementations made in this work open the possibility of incorporating the extensions that were developed over the years for the TrueSkill and Item-Re
sponse Theory models. }%
...
%
\en{For example, it would be possible to use as observable data not only the ordinal values of the results (lose/tie/win), but also the difference in points that those results had~\citep{Guo2012}. }%
...
%
\en{By offering the packages in three of the main scientific programming languages together with the complete mathematical documentation, we are strongly supporting the collaboration and unification of the scientific area of skill estimation, still scattered in the use of different models, some of them already obsolete. }%
