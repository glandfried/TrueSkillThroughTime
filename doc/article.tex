...
%
\en{All widely used skill estimators, such as TrueSkill, Glicko and Item-Response Theory, are based on the outcome of problem-solving and competitions. }%
\es{Todos los estimadores de habilidades ampliamente utilizados, como TrueSkill, Glicko e Item-Response Theory, se basan en el producto de resolucion de problemas y competencias. }%
%
\en{However, they do not properly model the temporal aspect, which prevents them from achieving both good initial estimates and comparability between distant estimates. }%
\es{Sin embargo, ellos no modelan apropiadamente el aspecto temporal, lo que les impide alcanzar tanto buenas estimaciones iniciales como comparabilidad entre estimaciones distantes. }%
%
\en{TrueSkill Through Time correct those issues by modeling the entire history of activities using a single Bayesian network. }%
\es{TrueSkill Through Time corrige esos sesgos modelando toda la historia de actividades mediante una única red bayesiana. }%
%
\en{In this paper we offer the first packages for \proglang{Julia}, \proglang{Python}, and \proglang{R}, together with the scientific documentation, supporting the adoption of a reliable learning estimator. }%
\es{En este artículo ofrecemos los primeros paquetes para \proglang{Julia}, \proglang{Python} y \proglang{R}, acompañada con su documentaci\'on cient\'ifica, apoyando el uso de un estimador de aprendizaje confiable. }%
%
\en{We illustrate how to use them in typical cases, and show the actual learning curves of historical players of the Association of Tennis Professionals (ATP). }%
\es{Ilustramos cómo usarlos en casos típicos, y mostramos las curvas de aprendizaje reales de los jugadores históricos de la Asociación de Tenistas Profesionales (ATP).  }%
%
\en{This algorithm requires only a few iterations over the data, allowing it to scale to millions of observations in a few seconds. }%
\es{Este algoritmo requiere sólo unas pocas iteraciones sobre los datos, permitiendo escalar a millones de observaciones en pocos segundos. }%
}
\Keywords{%
...
%
\en{One of the main contributions of the Glicko system~\citep{glikman_gliko_2} is the development of an efficient method to approximate the exact posterior using a Gaussian distribution. }%
\es{Uno de los principales aportes del sistema Glicko~\citep{glikman_gliko_2} fue el desarroll\'o de un m\'etodo eficiente para aproximar la posterior exacta con una distribuci\'on gaussiana. }%
%
...
%
\en{Similar algorithms were implemented by \cite{coulom2008-wholeHistoryRating} and \cite{maystre2019-pairwise} based on Laplacian approximations and Gaussian processes, respectively. }%
\es{Algoritmos similares fueron implementados por \cite{coulom2008-wholeHistoryRating} y \cite{maystre2019-pairwise} basados en aproximaciones laplacianas y procesos gaussianos respectivamente. }%
%
...
%
\en{This example exhibit that TrueSkill Through Time is able to follow the evolution of new player's skill. }%
...
%
\en{Here we only show the \proglang{Julia} code because it is far more efficient than \proglang{Python} and \proglang{R} versions (the \proglang{Python} and \proglang{R} codes can be found in appendix \ref{sec:appendix_atp_code}, and the performance comparison can be found in section \ref{sec:computationDetails}). }%
...
%
\en{To assess whether the complexity added by modeling multidimensionality is appropriate in general terms, we can compare the joint priori prediction of the models, calling the method \texttt{log\_evidence()} of the class \texttt{History}. }%
