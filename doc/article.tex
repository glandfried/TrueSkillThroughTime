...
TrueSkill Through Time: reliable initial skill estimates and historical comparability in \proglang{Julia}, \proglang{Python} and \proglang{R}
}
\Plaintitle{TrueSkill Through Time: enabling reliable initial skill estimates and historical comparability}
...
%
\en{Here we provide the first implementations of TrueSkill Through Time for \proglang{Julia}, \proglang{Python} and \proglang{R}, a model that solves all these problems by propagating historical information correctly, outperforming even more complex approaches. }%
\es{Aquí ofrecemos las primeras implementaciones de TrueSkill Through Time para \proglang{Julia}, \proglang{Python} y \proglang{R}, un modelo que resuelve todos estos problemas propagando correctamente la información histórica, superando incluso a enfoques similares más complejos. }%
%
...
%
\en{In this paper we offer a technical report accessible to the general public without omitting any theoretical and technical details, material that is not available in the scientific literature. }%
\es{En este trabajo ofrecemos una reporte técnico accesible al público en general sin omitir ningún detalle téoricos y técnicos, material que no está disponible hasta ahora en la literatura científica. }%
%
...
%
\en{A complete scientific documentation allows scientists to make sense of all epistemological and technical aspects. }%
\es{Una documentación científica completa permite a los científicos dar sentido a todos los aspectos epistemológicos y técnicos. }%
...
\en{To make the comparison we will follow the methodology employed by \cite{Maystre2019} over the four databases on which they report their prior predictions: ATP (tennis), NBA (basketball), FIFA (football), and FIDE (chess). }%
...
\en{This methodology consists of splitting the data in two subsets: the initial \SI{70}{\percent} used to train the hyperparameters and the last \SI{30}{\percent} to evaluate the models. }%
...
%ttt: 0.576047 %ks: 0.5758245946309177
 & & & & & & & & & & \\
...
%ttt: 0.5317903539908228 %ks: 0.532802033269822 %ks_integrado: 0.5327914819          
  & & & & & & & & & & \\
 ...
 %ttt: 0.358379593836443  %ks: 0.358397563993023 %ks_integrado: 0.3583680733
 & & & & & & & & & \\
 ...
 %ttt: 0.39627670749811733 ks: 0.39629097832609717 ks_integrado: 0.39619785744480546
  & & & & & & & & & & \\ \hline
  \end{tabular}
  \caption{
  \en{Comparisons between TTT, KickScore, TrueSkill, Elo and Constant models in four databases. }%
  ...
  %
  \en{The $\text{log}_2$BF columns report the Bayes Factor (Eq.~\ref{eq:bayes_factor}) between TTT and the other models in logarithmic scale. }%
  ...
  %
  \en{For reference, LOOCV column reports the geometric mean of the predictions using all historical data, $p(d_i| d_1, \dots, d_{i-1}, d_{i+1}, \dots, d_n , M)$. }%
  ...
  %
  \en{The geometric mean of the models indicated with the symbol $\dagger$ were computed by \cite{Maystre2019}. }%
  \es{La media geométrica de los modelos señalados con el símbolo $\dagger$ fueron computadas por \cite{Maystre2019}. }%
  }
  \label{Tab:Models}
\end{table}
%
...
%
\en{With no more than 3 intuitive hyperparameters, the TTT model achieves similar or even better results than KickScore more efficiently, because the latter must optimize on a much larger hyperparameter space, which includes the definition of a Kernel (a statistical trick not always intuitive\footnote{\href{https://stats.stackexchange.com/questions/152897/how-to-intuitively-explain-what-a-kernel-is}{Stackexchange: ``How to intuitively explain what a kernel is.'' (question 152897)}}) and then the hyperparameters specific to that Kernel. }%
\es{Con no más de 3 hiperparámetros intuitivos, el modelo TTT logra resultados similares y hasta mejores que KickScore de forma más eficiente, debido a que el último debe optimizar en un espacio de parámetros mucho más grande, que incluye la definción de un Kernel (un truco estadísitico no siempre intuitivo\footnote{\href{https://stats.stackexchange.com/questions/152897/how-to-intuitively-explain-what-a-kernel-is}{Stackexchange: ``How to intuitively explain what a kernel is.'' (question 152897)}}) y luego los hiperparámetros específicos a ese Kernel. }%
...
\es{En un problema tan sensible para las personas, como es la estimación de su habilidad es imperativo ofrecer una explicación que sea al mismo tiempo intuitiva y completa. }%
%
\en{There is a very large population, both in the video game industry and in the educational systems, that needs to know how the algorithm that measures their skills works. }%
...
%
\en{This paper aims to provide a technical report, with all the theoretical and technical details, in the most accessible way possible for the general public. }%
...
%
\en{With no more than 3 intuitive hyperparameters, the TTT model achieves similar or even better results than KickScore~\citep{Maystre2019} more efficiently, because the latter must optimize on a much larger hyperparameter space, which includes the definition of a Kernel (a statistical trick not always intuitive). }%
\es{Con no más de 3 hiperparámetros intuitivos, el modelo TTT logra resultados similares y hasta mejores que KickScore~\citep{Maystre2019} de forma más eficiente, debido a que el último debe optimizar en un espacio de parámetros mucho más grande, que incluye la definción de un Kernel (un truco estadísitico no siempre intuitivo). }%
...
%
\en{The TTT model implements an iterative message-passing algorithm that efficiently solve the inference process. }%
\es{El modelo TTT implementa un algoritmo iterativo de pasaje de mensajes que resuelve de forma eficiente el proceso de inferencia. }%
%
...
The observable data used to feed the model can come directly from the teachers’ evaluations and the model will consider the variable performance of students and teachers. }%
